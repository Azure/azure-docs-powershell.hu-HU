...
## Áttekintés
Módosítja az integrációs fiók kódösszeállítását.
...
## Leírás
A **set-AzIntegrationAccountAssembly** parancsmag módosította az integrációs fiók kódösszeállítását.

## Példák

### Példa 1: kódösszeállítás módosítása helyi fájl használatával
```powershell
...
Módosítja a "sampleAssembly" nevű kódösszeállítást a "$localAssemblyFilePath" fájl elérési útján található fájl elérési útjának helyi fájlja segítségével.

### 2. példa: a kódösszeállítás módosítása a byte Data függvénnyel
```powershell
...
Módosította az "sampleAssembly" nevű kódösszeállítást az "$assemblyContent"-ban található bájtos tömb használatával.

### 3. példa: kódösszeállítás módosítása a tartalmi hivatkozás használatával
```powershell
...
Módosította az "sampleAssembly" nevű kódösszeállítást a "$assemblyUrl" URL-címen található bájtos adatértékek használatával. Ez a javasolt módszer nagy méretű összeállítások készítéséhez.

## PARAMÉTEREK

### -AssemblyData
Az integrációs fiók összeszerelési bájtjának adatait.
...
### -AssemblyFilePath
Az integrációs fiók kódösszeállítás-fájljának elérési útja.
...
### -ContentLink
Nyilvánosan elérhető hivatkozás az integrációs fiókok összeállítási adatszolgáltatásához.
...
### -InputObject
Egy integrációs fiók kódösszeállítása.
...
### -Metadata
Az integrációs fiók kódösszeállításának metaadatai.
...
### -Name (név)
Az integrációs fiók kódösszeállításának neve.
...
### -ResourceId
Az integrációs fiók erőforrás-azonosítója.
...
### Microsoft. Azure. Command. LogicApp. models. PSIntegrationAccountAssembly
