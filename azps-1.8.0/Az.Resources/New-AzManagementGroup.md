...
## Leírás
A **New-AzManagementGroup** parancsmag létrehoz egy felügyeleti csoportot.
...
Új csoport létrehozása `DisplayName` és `ParentId` beállítása `null` Az `DisplayName` ugyanaz lesz, mint a `GroupName` csoport anyavállalata lesz a bérlő.  

### 2. példa: felügyeleti csoport létrehozása megjelenítendő névvel
```
...
Ebben az esetben a csoport szülője lesz a bérlő, és a `DisplayName` megjelenő érték az adott értékre lesz állítva.

### 3. példa: felügyeleti csoport létrehozása szülővel és megjelenített névvel
```
...
### 4. példa: felügyeleti csoport létrehozása szülővel (szülő objektum használatával)
```
...
### -DisplayName
A felügyeleti csoport megjelenítendő neve
...
### -ParentId
A felügyeleti csoport fölérendelt azonosítója
