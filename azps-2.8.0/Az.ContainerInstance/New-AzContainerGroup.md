...
## Leírás
A **New-AzContainerGroup** parancsmagok tároló csoportot hoznak létre.
...
### 3. példa: a befejezésre kerülő tároló csoportot hozza létre.
```
...
Ez a parancs létrehoz egy tároló csoportot, amely a "helló" és a leállást nyomtatja.

### 4. példa: tároló-csoport létrehozása az Azure Container beállításjegyzékben lévő képpel
```
...
### Példa 5: a Container-csoport létrehozása kép használatával az egyéni tároló képnyilvántartásban
```
PS C:\> $secpasswd = ConvertTo-SecureString "PlainTextPassword" -AsPlainText -Force
...
### 6. példa: az Azure-fájlok csatlakoztatására szolgáló tároló csoport létrehozása
```
PS C:\> $secpasswd = ConvertTo-SecureString "PlainTextPassword" -AsPlainText -Force
PS C:\> $mycred = New-Object System.Management.Automation.PSCredential ("username", $secpasswd)
...
Ez a parancs létrehoz egy tároló csoportot, amely a megadott Azure-fájlmegosztás csatlakoztatását tartalmazza `/mnt/azfile` .
...
Ez a parancs létrehoz egy tároló csoportot a rendszerhez rendelt identitással a legújabb Nginx képen, és nyilvános IP-címet kér a 8000-as port megnyitásával.
...
Ez a parancs létrehoz egy tároló csoportot a rendszer kiosztott és a felhasználó által kiosztott identitással a legújabb Nginx képen, és a 8000-as port megnyitásával nyilvános IP-címet kér.

## PARAMÉTEREK

### -AssignIdentity
A rendszer által kiosztott identitás engedélyezése
...
Az Azure-fájlmegosztás tárolási fiókjának hitelesítő adatai, ahol a Felhasználónév a tárolási fiók neve, a kulcs pedig a tárolási fiók kulcsa.
...
Az Azure-fájl kötetének csatlakozási útvonala.
...
A Mount Azure-fájl megosztásának neve.
...
### -IdentityId
A felhasználó által kiosztott azonosítók azonosítói
...
### -Port
A megnyitni kívánt port (ok) Alapértelmezett: [80]
...
### -RestartPolicy
A tároló újraindítása házirend. Alapértelmezett: mindig
