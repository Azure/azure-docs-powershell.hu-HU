---
Module Name: Az.Websites
Module Guid: cc69c625-e961-43f4-8b50-0061eba6e4b6
Download Help Link: https://docs.microsoft.com/en-us/powershell/module/az.websites
Help Version: 1.0.0.0
Locale: en-US
content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Websites/Websites/help/Az.Websites.md
original_content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Websites/Websites/help/Az.Websites.md
ms.openlocfilehash: 174da8669c84442f24866b8ab8831ee70d2d239d
ms.sourcegitcommit: 0b94b9566124331d0b15eb7f5a811305c254172e
ms.translationtype: MT
ms.contentlocale: hu-HU
ms.lasthandoff: 10/15/2019
ms.locfileid: "93665552"
---
# Az. websites modul
## Leírás
ARM (Azure Resource Manager) webalkalmazás és app Service Plan terv parancsai.

## Az. websites parancsmagok
### [Edit-AzWebAppBackupConfiguration](Edit-AzWebAppBackupConfiguration.md)


### [ENTER-AzWebAppContainerPSSession](Enter-AzWebAppContainerPSSession.md)
Távoli PowerShell-munkamenet megnyitása egy adott webhelyen vagy a bővítőhelyen és az adott erőforráscsoporthoz megadott Windows-tárolóban

### [Get-AzAppServicePlan](Get-AzAppServicePlan.md)
Egy Azure app-szolgáltatási csomagot kap a megadott erőforráscsoport számára.

### [Get-AzAppServicePlanMetric](Get-AzAppServicePlanMetric.md)


### [Get-AzDeletedWebApp](Get-AzDeletedWebApp.md)
Törölt webalkalmazásokat kap az előfizetésben.

### [Get-AzWebApp](Get-AzWebApp.md)
A megadott erőforráscsoport Azure Web Apps-t kap.

### [Get-AzWebAppBackup](Get-AzWebAppBackup.md)


### [Get-AzWebAppBackupConfiguration](Get-AzWebAppBackupConfiguration.md)


### [Get-AzWebAppBackupList](Get-AzWebAppBackupList.md)


### [Get-AzWebAppCertificate](Get-AzWebAppCertificate.md)
Azure Web App-tanúsítványt kap.

### [Get-AzWebAppContainerContinuousDeploymentUrl](Get-AzWebAppContainerContinuousDeploymentUrl.md)
Get-AzWebAppContainerContinuousDeploymentUrl a tároló folyamatos telepítő URL-címét adja vissza

### [Get-AzWebAppMetric](Get-AzWebAppMetric.md)
Azure Web App metrikáit kapja.

### [Get-AzWebAppPublishingProfile](Get-AzWebAppPublishingProfile.md)
Az Azure Web App közzétételi profilját kapja.

### [Get-AzWebAppSlot](Get-AzWebAppSlot.md)
Azure Web App-bővítőhelyet kap.

### [Get-AzWebAppSlotConfigName](Get-AzWebAppSlotConfigName.md)
A Web App-bővítőhely konfigurációs neveinek listája

### [Get-AzWebAppSlotMetric](Get-AzWebAppSlotMetric.md)
Egy Azure Web App-bővítőhely metrikáit kapja.

### [Get-AzWebAppSlotPublishingProfile](Get-AzWebAppSlotPublishingProfile.md)
Beilleszt egy Azure Web App bővítőhely-közzétételi profilt.

### [Get-AzWebAppSnapshot](Get-AzWebAppSnapshot.md)
Elérhetővé válik a webes alkalmazások számára elérhető Pillanatképek.

### [Get-AzWebAppSSLBinding](Get-AzWebAppSSLBinding.md)
Az Azure Web App tanúsítványának SSL-kötését kapja.

### [Új – AzAppServicePlan](New-AzAppServicePlan.md)
Azure app-szolgáltatási csomagot hoz létre egy adott földrajzi helyen.

### [Új – AzWebApp](New-AzWebApp.md)
Azure webalkalmazást hoz létre.

### [Új – AzWebAppAzureStoragePath](New-AzWebAppAzureStoragePath.md)
Létrehoz egy olyan objektumot, amely az Azure-tárterület elérési útját jelöli egy webalkalmazásban való csatlakoztatáshoz. A program paraméterként (-AzureStoragePath) használja Set-AzWebApp és Set-AzWebAppSlot

### [Új – AzWebAppBackup](New-AzWebAppBackup.md)


### [Új – AzWebAppContainerPSSession](New-AzWebAppContainerPSSession.md)
New-AzWebAppContainerPSSession új távoli PowerShell-munkamenetet hoz létre egy adott webhelyen vagy bővítőhelyen vagy erőforrás csoportban megadott Windows-tárolóban.

### [Új – AzWebAppDatabaseBackupSetting](New-AzWebAppDatabaseBackupSetting.md)


### [Új – AzWebAppSlot](New-AzWebAppSlot.md)
Azure Web App-bővítőhelyet hoz létre.

### [Új – AzWebAppSSLBinding](New-AzWebAppSSLBinding.md)
Az Azure Web App-hoz létrehoz egy SSL-tanúsítványt.

### [Közzététel – AzWebApp](Publish-AzWebApp.md)
Az Azure Web App telepítése ZIP-, JAR-vagy háborús fájlból a zipdeploy segítségével.

### [Remove-AzAppServicePlan](Remove-AzAppServicePlan.md)
Az Azure app szolgáltatás tervének eltávolítása.

### [Remove-AzWebApp](Remove-AzWebApp.md)
Azure Web App eltávolítása.

### [Remove-AzWebAppBackup](Remove-AzWebAppBackup.md)


### [Remove-AzWebAppSlot](Remove-AzWebAppSlot.md)


### [Remove-AzWebAppSSLBinding](Remove-AzWebAppSSLBinding.md)
SSL-kötés eltávolítása feltöltött tanúsítványból.

### [Reset-AzWebAppPublishingProfile](Reset-AzWebAppPublishingProfile.md)


### [Reset-AzWebAppSlotPublishingProfile](Reset-AzWebAppSlotPublishingProfile.md)


### [Újraindítás – AzWebApp](Restart-AzWebApp.md)
Az Azure Web App újraindítása.

### [Újraindítás – AzWebAppSlot](Restart-AzWebAppSlot.md)


### [Restore-AzDeletedWebApp](Restore-AzDeletedWebApp.md)
Törölt webalkalmazás visszaállítása új vagy meglévő webalkalmazásra.

### [Restore-AzWebAppBackup](Restore-AzWebAppBackup.md)


### [Restore-AzWebAppSnapshot](Restore-AzWebAppSnapshot.md)
Egy webalkalmazás pillanatképének visszaállítása

### [Set-AzAppServicePlan](Set-AzAppServicePlan.md)
Azure-alkalmazási szolgáltatási csomagot állít be.

### [Set-AzWebApp](Set-AzWebApp.md)
Azure Web App módosítása

### [Set-AzWebAppSlot](Set-AzWebAppSlot.md)
Azure Web App-bővítőhely módosítása

### [Set-AzWebAppSlotConfigName](Set-AzWebAppSlotConfigName.md)
A Web App-bővítőhely konfigurációs neveinek beállítása

### [Start-AzWebApp](Start-AzWebApp.md)
Azure Web App indítása.

### [Start-AzWebAppSlot](Start-AzWebAppSlot.md)
Azure Web App-bővítőhelyet indít el.

### [Stop-AzWebApp](Stop-AzWebApp.md)
Azure Web App leállítása

### [Stop-AzWebAppSlot](Stop-AzWebAppSlot.md)
Azure Web App-bővítőhely leállítása

### [Switch-AzWebAppSlot](Switch-AzWebAppSlot.md)
Két bővítőhely cseréje webalkalmazással

### [Get-AzWebAppAccessRestrictionConfig](Get-AzWebAppAccessRestrictionConfig.md)
Az Azure Web App hozzáférési korlátozási beállításait kapja meg.

### [Update-AzWebAppAccessRestrictionConfig](Update-AzWebAppAccessRestrictionConfig.md)
Az SCM-webhely öröklődési konfigurációjának frissítése az Azure Web App alkalmazásban.

### [Add-AzWebAppAccessRestrictionRule](Add-AzWebAppAccessRestrictionRule.md)
Az Azure Web App alkalmazáshoz való hozzáférés korlátozását adja hozzá.

### [Remove-AzWebAppAccessRestrictionRule](Remove-AzWebAppAccessRestrictionRule.md)
Az Azure Web App hozzáférési korlátozásának eltávolítása