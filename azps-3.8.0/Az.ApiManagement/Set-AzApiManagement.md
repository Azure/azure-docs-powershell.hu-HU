...
A **set-AzApiManagement** parancsmag egy Azure API-kezelési szolgáltatást frissít.

## Példák

### Példa 1 API-kezelési szolgáltatás beszerzése és átméretezése a prémium verzióhoz és a régió hozzáadásához
```powershell
...
### 2. példa: a frissítés telepítése (külső VNET)
```powershell
...
### 3. példa: a PsApiManagementCustomHostNameConfiguration-példányok létrehozása és inicializálása titkos kulccsal
```powershell
...
### 4. példa: a Publisher e-mail-címének frissítése, NotificationSender-és szervezeti név
```powershell
...
### -PassThru
Frissített PsApiManagement küld a csővezetéknek, ha a művelet sikeres.
