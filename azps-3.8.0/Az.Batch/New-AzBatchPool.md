...
## Leírás
A **New-AzBatchPool** parancsmag létrehoz egy készletet az Azure kötegelt szolgáltatásban a *BatchContext* paraméterrel megadott fiók alapján.

## Példák

### Példa 1: új készlet létrehozása a TargetDedicated paraméterrel az CloudServiceConfiguration használatával
```
...
A medence úgy van konfigurálva, hogy a STANDARD_D1_V2 virtuális gépeket használja a négy család operációs rendszerének verziójával.

### 2. példa: új készlet létrehozása a TargetDedicated paraméterrel az VirtualMachineConfiguration használatával
```
...
A készlet úgy van konfigurálva, hogy a Windows-2016-Datacenter operációs rendszerű virtuális gépeket STANDARD_D1_V2 használja.

### 3. példa: új készlet létrehozása az automéretarány paraméterérték használatával
```
PS C:\>$imageReference = New-Object -TypeName "Microsoft.Azure.Commands.Batch.Models.PSImageReference" -ArgumentList @("WindowsServer", "MicrosoftWindowsServer", "2016-Datacenter", "*")
PS C:\>$configuration = New-Object -TypeName "Microsoft.Azure.Commands.Batch.Models.PSVirtualMachineConfiguration" -ArgumentList @($imageReference, "batch.node.windows amd64")
...
A készlet úgy van konfigurálva, hogy a Windows-2016-Datacenter operációs rendszerrel rendelkező virtuális gépeket STANDARD_D1_V2 használja, valamint a számítási csomópontok kitűzött számát az automéretarány képlet határozza meg.

### Példa 4: egy alhálózatban csomópontokat tartalmazó Pool létrehozása
```
PS C:\>$imageReference = New-Object -TypeName "Microsoft.Azure.Commands.Batch.Models.PSImageReference" -ArgumentList @("WindowsServer", "MicrosoftWindowsServer", "2016-Datacenter", "*")
PS C:\>$configuration = New-Object -TypeName "Microsoft.Azure.Commands.Batch.Models.PSVirtualMachineConfiguration" -ArgumentList @($imageReference, "batch.node.windows amd64")
...
### Példa 5: egyéni felhasználói fiókokat tartalmazó készlet létrehozása
```
PS C:\>$imageReference = New-Object -TypeName "Microsoft.Azure.Commands.Batch.Models.PSImageReference" -ArgumentList @("WindowsServer", "MicrosoftWindowsServer", "2016-Datacenter", "*")
PS C:\>$configuration = New-Object -TypeName "Microsoft.Azure.Commands.Batch.Models.PSVirtualMachineConfiguration" -ArgumentList @($imageReference, "batch.node.windows amd64")
...
### -ApplicationLicenses
Az alkalmazásspecifikus licencek listája, amelyet a kötegelt szolgáltatás a készlet minden számítási csomópontjára elérhetővé tesz.
...
### -MountConfiguration
A készlet minden csomópontján a csatlakoztatni kívánt fájlrendszerek listája. Ez a funkció támogatja az Azure-fájlokat, az NFS-t, a CIFS/SMB-t és a Blobfuse.
...
A kijelölt számítási csomópontok cél számát adja meg a készletre való kiosztáshoz.
...
A minimális prioritású számítási csomópontok cél számát adja meg a készletre való kiosztáshoz.
