...
## Áttekintés
Módosítja az integrációs fiók kötegének konfigurációját.
...
## Leírás
A **set-AzIntegrationAccountBatchConfiguration** parancsmag módosította az integrációs fiók kötegének konfigurációját.

## Példák

### Példa 1: kötegfájl-konfiguráció módosítása helyi fájl használatával
```powershell
...
Módosítsa a "sampleBatchConfig" nevű köteg-konfigurációt a "$batchConfigurationFilePath" fájl elérési útján található fájl elérési útján található helyi fájl segítségével.

### 2. példa: a kötegek konfigurációjának módosítása JSON-karakterlánc használatával
```powershell
...
Módosítsa a "sampleBatchConfig" nevű köteg-konfigurációt a "$batchConfigurationContent" nevű JSON-karakterlánc használatával.

### 3. példa: a kötegek konfigurációjának módosítása paraméterek használatával
```powershell
...
Módosítsa a "sampleBatchConfig" nevű köteg-konfigurációt a szükséges paraméterek mindegyikének manuális megadásával.

## PARAMÉTEREK

### -BatchConfigurationDefinition
Az integrációs fiók kötegének konfigurációs definíciója.
...
Az integrációs fiók kötegfájl-konfigurációs fájljának elérési útja.
...
### -BatchGroupName
Az integrációs fiók kötegének konfigurációs csoportjának neve.
...
### -BatchSize
Az integrációs fiók kötegének konfigurációját tartalmazó köteg mérete.
...
### -MessageCount
Az integrációs fiók kötegelt beállítási üzeneteinek száma.
...
### -Metadata
Az integrációs fiók kötegelt konfigurációs metaadata.
...
### -ScheduleFrequency
Az integrációs fiók kötegelt konfigurációja ütemtervének gyakorisága.
...
### -ScheduleInterval
Az integrációs fiók kötegének ütemezési intervalluma.
...
### -ScheduleStartTime
Az integrációs fiók kötegelt beállítási ütemezése kezdési időpontja.
...
### -ScheduleTimeZone
Az integrációs fiók kötegének konfigurációs ütemezése időzóna.
