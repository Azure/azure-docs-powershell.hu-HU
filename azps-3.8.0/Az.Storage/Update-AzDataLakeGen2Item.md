...
## Leírás
Az **Update-AzDataLakeGen2Item** parancsmag frissíti a tulajdonságokat, a metaadatokat, az engedélyeket, az ACL-t és a tulajdonosokat tartalmazó fájlt vagy könyvtárat.
...
### 1. példa: ACL-objektum létrehozása 3 ACL-bejegyzéssel és az ACL frissítése a fájlrendszer összes eleméhez rekurzívan
```
...
Ez a parancs először egy ACL-objektumot hoz létre a 3 ACL-bejegyzéssel (a-InputObject paraméterrel az ACL-bejegyzést a meglévő ACL-objektumhoz), majd beolvashatja a fájlrendszer összes elemét, és frissítheti az ACL-elemeket.

### 2. példa: a fájl összes tulajdonságának frissítése és megjelenítése
```
...
Ez a parancs a fájl összes tulajdonságát frissíti (ACL, engedély, tulajdonos, csoport, metaadatok, tulajdonság frissíthető bármely conbination), és megjelenítheti őket a PowerShell-konzolban.

### 3. példa: ACL-bejegyzés hozzáadása egy címtárhoz
```
## Get the origin ACL
...
Ez a parancs beolvassa az ACL-t egy címtárból, frissítéseket/egy ACL-bejegyzést, és visszaállítja a címtárat.
Ha az ACL-bejegyzés ugyanazzal a AccessControlType/EntityId/DefaultScope nem létezik, akkor új ACL-bejegyzést fog hozzáadni, különben a meglévő ACL-bejegyzés engedélye.

## PARAMÉTEREK

### -ACL
A POSIX hozzáférés-vezérlési jogosultságait fájlok és könyvtárak esetében állítja be.
Hozza létre ezt az objektumot új AzDataLakeGen2ItemAclObject.
...
### -Csoport
Beállítja a blob tulajdonosi csoportját.
...
### -InputObject
Azure Datalake Gen2 elem objektum frissítése
...
### -Metadata
A címtár vagy a fájl metaadatait adja meg.
...
### -Path (elérési út)
A megadott fájlrendszerben frissítendő elérési út.
A "címtár/file.txt" vagy a "directory1/directory2/" formátumban lehet fájl vagy könyvtár.
Nem adja meg, hogy ez a paraméter frissíti a fájlrendszer gyökérkönyvtárát.
...
### – Engedély
A fájl tulajdonosának, a fájl tulajdonosi csoportjának és a többieknek a POSIX-hozzáférési engedélyének beállítása.
Minden osztály olvasási, írási vagy végrehajtási engedélyt kaphat.
A szimbolikus (rwxrw-RW-) támogatott.
Érvénytelen az ACL-vel együtt.
...
### -Tulajdonság
A címtár vagy a fájl tulajdonságait adja meg. A fájlok támogatott tulajdonságai: CacheControl, ContentDisposition, ContentEncoding, ContentLanguage, ContentMD5, ContentType.
