---
Module Name: Az.Compute
Module Guid: 0a83c907-1ffb-4d87-a492-c65ac7d7ed37
Download Help Link: https://docs.microsoft.com/en-us/powershell/module/az.compute
Help Version: 3.5.0.0
Locale: en-US
content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Compute/Compute/help/Az.Compute.md
original_content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Compute/Compute/help/Az.Compute.md
ms.openlocfilehash: e0cb2feccbafae5d914047bcddcf9e3eb6933e9d
ms.sourcegitcommit: 1de2b6c3c99197958fa2101bc37680e7507f91ac
ms.translationtype: MT
ms.contentlocale: hu-HU
ms.lasthandoff: 10/13/2020
ms.locfileid: "94024164"
---
# Az. számítási modul
## Leírás
Ez a témakör az Azure számítási parancsmagok súgótémaköröket jeleníti meg.

## Az. számítási parancsmagok
### [Add-AzContainerServiceAgentPoolProfile](Add-AzContainerServiceAgentPoolProfile.md)
Elhelyez egy tároló szolgáltatás-ügynök készlet-profilt.

### [Add-AzImageDataDisk](Add-AzImageDataDisk.md)
Adatlemez felvétele a képobjektumba.

### [Add-AzVhd](Add-AzVhd.md)
Virtuális merevlemezt tölt fel egy helyszíni virtuális gépről egy blobra egy felhőalapú tároló fiókban az Azure-ban.

### [Add-AzVMAdditionalUnattendContent](Add-AzVMAdditionalUnattendContent.md)
Információt ad a Windows felügyelet nélküli telepítési fájljához.

### [Add-AzVMDataDisk](Add-AzVMDataDisk.md)
Adatlemez felvétele virtuális gépre.

### [Add-AzVMNetworkInterface](Add-AzVMNetworkInterface.md)
Hálózati felületet ad hozzá egy virtuális géphez.

### [Add-AzVMSecret](Add-AzVMSecret.md)
Titkos kulcsot ad hozzá egy virtuális géphez.

### [Add-AzVmssAdditionalUnattendContent](Add-AzVmssAdditionalUnattendContent.md)
Információt ad a Windows felügyelet nélküli telepítési fájljához.

### [Add-AzVmssDataDisk](Add-AzVmssDataDisk.md)
Adatlemez felvétele a VMSS.

### [Add-AzVmssDiagnosticsExtension](Add-AzVmssDiagnosticsExtension.md)
Diagnosztikai bővítményt ad a VMSS.

### [Add-AzVmssExtension](Add-AzVmssExtension.md)
Hozzáad egy bővítményt a VMSS.

### [Add-AzVMSshPublicKey](Add-AzVMSshPublicKey.md)
A virtuális gép nyilvános kulcsait hozzáadja az SSH-hoz, ha csak a VM-et hozza létre.

### [Add-AzVmssNetworkInterfaceConfiguration](Add-AzVmssNetworkInterfaceConfiguration.md)
Hálózati illesztőfelület-konfiguráció felvétele a VMSS.

### [Add-AzVmssSecret](Add-AzVmssSecret.md)
Titkot ad a VMSS.

### [Add-AzVmssSshPublicKey](Add-AzVmssSshPublicKey.md)
Az SSH-beli nyilvános kulcsok felvétele a VMSS.

### [Add-AzVmssVMDataDisk](Add-AzVmssVMDataDisk.md)
Adatlemez felvétele a Vmss VM-be

### [Add-AzVmssWinRMListener](Add-AzVmssWinRMListener.md)
Hozzáadja a WinRM-figyelőt a VMSS.

### [ConvertTo-AzVMManagedDisk](ConvertTo-AzVMManagedDisk.md)
A virtuális gépet blob-alapú lemezekkel egy virtuális gépre konvertálja, a kezelt lemezekkel.

### [Disable-AzVMDiskEncryption](Disable-AzVMDiskEncryption.md)
Letiltja a titkosítást egy IaaS virtuális gépen.

### [Disable-AzVmssDiskEncryption](Disable-AzVmssDiskEncryption.md)
Letiltja a lemez titkosítását a VM-léptékű készleteken.

### [Exportálás – AzLogAnalyticRequestRateByInterval](Export-AzLogAnalyticRequestRateByInterval.md)
Az adott időablakban az előfizetés API-kéréseit megjelenítő naplók exportálása a szabályozási tevékenységekre.

### [Exportálás – AzLogAnalyticThrottledRequest](Export-AzLogAnalyticThrottledRequest.md)
Olyan naplók exportálása, amelyek az adott időablakban az előfizetéshez tartozó összes szabályozott API-kérést megjelenítik.

### [Get-AzAvailabilitySet](Get-AzAvailabilitySet.md)
Az Azure elérhetőségi készleteit egy erőforráscsoport kapja meg.

### [Get-AzComputeResourceSku](Get-AzComputeResourceSku.md)
Az összes számítási erőforrás listázása

### [Get-AzContainerService](Get-AzContainerService.md)
Tároló szolgáltatást kap.

### [Get-AzDisk](Get-AzDisk.md)
A felügyelt lemezek tulajdonságait kapja meg.

### [Get-AzDiskAccess](Get-AzDiskAccess.md)
A lemezek elérésének tulajdonságai

### [Get-AzDiskEncryptionSet](Get-AzDiskEncryptionSet.md)
A lemez-titkosítási készletek beolvasása és listázása

### [Get-AzGallery](Get-AzGallery.md)
Beolvashatja vagy listázhatja a gyűjteményeket.

### [Get-AzGalleryImageDefinition](Get-AzGalleryImageDefinition.md)
Képdefiníciók beolvasása vagy listázása

### [Get-AzGalleryImageVersion](Get-AzGalleryImageVersion.md)
Beolvashatja vagy listázhatja a Képtár képverzióit.

### [Get-AzHost](Get-AzHost.md)
Beolvashatja vagy listázhatja az állomásokat.

### [Get-AzHostGroup](Get-AzHostGroup.md)
Beolvashatja vagy listázhatja az állomásokat.

### [Get-AzImage](Get-AzImage.md)
A kép tulajdonságainak beolvasása

### [Get-AzProximityPlacementGroup](Get-AzProximityPlacementGroup.md)
Beolvashatja vagy listázhatja a Proximity-hely erőforrás (oka) t.

### [Get-AzRemoteDesktopFile](Get-AzRemoteDesktopFile.md)
. Rdp fájlok beolvasása

### [Get-AzSnapshot](Get-AzSnapshot.md)
A pillanatképek tulajdonságainak beolvasása

### [Get-AzVM](Get-AzVM.md)
Egy virtuális gép tulajdonságait kapja meg.

### [Get-AzVMAccessExtension](Get-AzVMAccessExtension.md)
Információt kap az VMAccess bővítményről.

### [Get-AzVMADDomainExtension](Get-AzVMADDomainExtension.md)
Információt AD a tartomány kiterjesztéséről.

### [Get-AzVMAEMExtension](Get-AzVMAEMExtension.md)
Információt kap az agrár-és az agrár-mellékről.

### [Get-AzVMBootDiagnosticsData](Get-AzVMBootDiagnosticsData.md)
Rendszerindítási diagnosztikai adatot kap egy virtuális géphez.

### [Get-AzVMChefExtension](Get-AzVMChefExtension.md)
Információt kap a szakács kibővítéséről.

### [Get-AzVMCustomScriptExtension](Get-AzVMCustomScriptExtension.md)
Információt kap az egyéni parancsfájl-kiterjesztésről.

### [Get-AzVMDiagnosticsExtension](Get-AzVMDiagnosticsExtension.md)
A diagnosztikai bővítmény beállításait egy virtuális gépen kapja meg.

### [Get-AzVMDiskEncryptionStatus](Get-AzVMDiskEncryptionStatus.md)
A virtuális gép titkosítási állapotát kapja meg.

### [Get-AzVMDscExtension](Get-AzVMDscExtension.md)
Egy adott virtuális gépen a DSC bővítmény beállításait kapja.

### [Get-AzVMDscExtensionStatus](Get-AzVMDscExtensionStatus.md)
Egy virtuális gép DSC-bővítményének állapotát kapja meg.

### [Get-AzVMExtension](Get-AzVMExtension.md)
A virtuálisgép-bővítmények tulajdonságait a virtuális gépen telepítette.

### [Get-AzVMExtensionImage](Get-AzVMExtensionImage.md)
Az Azure-bővítmény minden verziójának beolvasása

### [Get-AzVMExtensionImageType](Get-AzVMExtensionImageType.md)
Az Azure-bővítmény típusa.

### [Get-AzVMImage](Get-AzVMImage.md)
Beolvassa a VMImage összes verzióját.

### [Get-AzVMImageOffer](Get-AzVMImageOffer.md)
Beolvassa a VMImage kínálati típust.

### [Get-AzVMImagePublisher](Get-AzVMImagePublisher.md)
Megkapja a VMImage-közzétevőket.

### [Get-AzVMImageSku](Get-AzVMImageSku.md)
Megkapja a VMImage SKU-ot.

### [Get-AzVMRunCommandDocument](Get-AzVMRunCommandDocument.md)
Futtathatja a Futtatás parancs dokumentumát.

### [Get-AzVMSize](Get-AzVMSize.md)
Elérhetővé válik a virtuális gépek mérete.

### [Get-AzVMSqlServerExtension](Get-AzVMSqlServerExtension.md)
A virtuális gép SQL Server-bővítményének beállításait kapja meg.

### [Get-AzVmss](Get-AzVmss.md)
Beolvassa a VMSS tulajdonságait.

### [Get-AzVmssDiskEncryption](Get-AzVmssDiskEncryption.md)
A VM-léptékű készlet lemez-titkosítási állapotát jeleníti meg.

### [Get-AzVmssRollingUpgrade](Get-AzVmssRollingUpgrade.md)
A virtuálisgép-készlet legújabb, folyamatos frissítésének állapotát jeleníti meg.

### [Get-AzVmssSku](Get-AzVmssSku.md)
A VMSS elérhető SKU-ket kapja.

### [Get-AzVmssVM](Get-AzVmssVM.md)
A VMSS virtuális gép tulajdonságait kapja meg.

### [Get-AzVmssVMDiskEncryption](Get-AzVmssVMDiskEncryption.md)
A VMs-rendszer merevlemez-titkosítási állapotát jeleníti meg VM-méretarányban.

### [Get-AzVMUsage](Get-AzVMUsage.md)
Beilleszti a virtuális gép alapvető számát a helyhez.

### [Grant-AzDiskAccess](Grant-AzDiskAccess.md)
Hozzáférést biztosít a lemezhez.

### [Grant-AzSnapshotAccess](Grant-AzSnapshotAccess.md)
Hozzáférést ad egy pillanatképhez.

### [Meghívó-AzVMPatchAssessment](Invoke-AzVMPatchAssessment.md)
A virtuális gép patch állapotának felmérése

### [Meghívó-AzVMReimage](Invoke-AzVMReimage.md)
Egy Azure Virtual Machine Reimage-et.

### [Meghívó-AzVMRunCommand](Invoke-AzVMRunCommand.md)
Futtassa a parancsot a VM-ben.

### [Meghívó-AzVmssVMRunCommand](Invoke-AzVmssVMRunCommand.md)
A Futtatás parancs a virtuális gép méretarány beállítása VM.

### [Új – AzAvailabilitySet](New-AzAvailabilitySet.md)
Azure elérhetőségi készletet hoz létre.

### [Új – AzContainerService](New-AzContainerService.md)
Tároló szolgáltatást hoz létre.

### [Új – AzContainerServiceConfig](New-AzContainerServiceConfig.md)
Helyi konfigurációs objektum létrehozása tároló szolgáltatáshoz.

### [Új – AzDisk](New-AzDisk.md)
Felügyelt lemezt hoz létre.

### [Új – AzDiskAccess](New-AzDiskAccess.md)
Disk Access-erőforrás létrehozása

### [Új – AzDiskConfig](New-AzDiskConfig.md)
Konfigurálható lemezkezelő-objektumot hoz létre.

### [Új – AzDiskEncryptionSet](New-AzDiskEncryptionSet.md)
Lemez-titkosítási készletet hoz létre.

### [Új – AzDiskEncryptionSetConfig](New-AzDiskEncryptionSetConfig.md)
Konfigurálható lemezkezelő set objektum létrehozása.

### [Új – AzDiskUpdateConfig](New-AzDiskUpdateConfig.md)
Konfigurálható Disk Update-objektumot hoz létre.

### [Új – AzGallery](New-AzGallery.md)
Gyűjtemény létrehozása

### [Új – AzGalleryImageDefinition](New-AzGalleryImageDefinition.md)
Gyűjtemény képének definíciójának létrehozása

### [Új – AzGalleryImageVersion](New-AzGalleryImageVersion.md)
Gyűjtemény képének létrehozása

### [Új – AzHost](New-AzHost.md)
Egy állomást hoz létre.

### [Új – AzHostGroup](New-AzHostGroup.md)
Egy állomás-csoportot hoz létre.

### [Új – AzImage](New-AzImage.md)
Képet hoz létre.

### [Új – AzImageConfig](New-AzImageConfig.md)
Konfigurálható képobjektumot hoz létre.

### [Új – AzProximityPlacementGroup](New-AzProximityPlacementGroup.md)
A közelségi hely létrehozása erőforrás létrehozása

### [Új – AzSnapshot](New-AzSnapshot.md)
Pillanatfelvételt hoz létre.

### [Új – AzSnapshotConfig](New-AzSnapshotConfig.md)
Konfigurálható pillanatkép-objektumot hoz létre.

### [Új – AzSnapshotUpdateConfig](New-AzSnapshotUpdateConfig.md)
Konfigurálható Snapshot Update-objektumot hoz létre.

### [Új – AzVM](New-AzVM.md)
Virtuális gépet hoz létre.

### [Új – AzVMConfig](New-AzVMConfig.md)
Konfigurálható virtuálisgép-objektumot hoz létre.

### [Új – AzVMDataDisk](New-AzVMDataDisk.md)
Helyi adatlemez-objektumot hoz létre egy virtuális géphez vagy egy Vmss VM-hez.

### [Új – AzVMSqlServerAutoBackupConfig](New-AzVMSqlServerAutoBackupConfig.md)
Konfigurációs objektum létrehozása az SQL Server automatikus biztonsági mentéséhez.

### [Új – AzVMSqlServerAutoPatchingConfig](New-AzVMSqlServerAutoPatchingConfig.md)
Létrehoz egy konfigurációs objektumot egy virtuális gépen történő automatikus javításhoz.

### [Új – AzVMSqlServerKeyVaultCredentialConfig](New-AzVMSqlServerKeyVaultCredentialConfig.md)
Létrehoz egy konfigurációs objektumot az SQL Server Key Vault hitelesítő adatairól egy virtuális gépen.

### [Új – AzVmss](New-AzVmss.md)
Létrehoz egy VMSS.

### [Új – AzVmssConfig](New-AzVmssConfig.md)
Létrehozza a VMSS konfigurációs objektumát.

### [Új – AzVmssIpConfig](New-AzVmssIpConfig.md)
IP-konfigurációt hoz létre egy VMSS hálózati felületéhez.

### [Új – AzVmssIpTagConfig](New-AzVmssIpTagConfig.md)
IP-címke típusú objektumot hoz létre egy VMSS hálózati felületéhez.

### [Új – AzVmssVaultCertificateConfig](New-AzVmssVaultCertificateConfig.md)
A fő pince-tanúsítványok konfigurációját hozza létre.

### [Közzététel – AzVMDscConfiguration](Publish-AzVMDscConfiguration.md)
DSC-parancsfájl feltöltése Azure Blob-tárhelyre

### [Remove-AzAvailabilitySet](Remove-AzAvailabilitySet.md)
Az Azure-ról származó elérhetőségi készlet eltávolítása.

### [Remove-AzContainerService](Remove-AzContainerService.md)
Tároló szolgáltatás eltávolítása.

### [Remove-AzContainerServiceAgentPoolProfile](Remove-AzContainerServiceAgentPoolProfile.md)
Egy ügynök-gyűjtő profil eltávolítása a tároló szolgáltatásból.

### [Remove-AzDisk](Remove-AzDisk.md)
Lemez eltávolítása

### [Remove-AzDiskAccess](Remove-AzDiskAccess.md)
A Disk Access-erőforrás eltávolítása.

### [Remove-AzDiskEncryptionSet](Remove-AzDiskEncryptionSet.md)
A Lemezkarbantartó-készlet eltávolítása.

### [Remove-AzGallery](Remove-AzGallery.md)
Gyűjtemény törlése

### [Remove-AzGalleryImageDefinition](Remove-AzGalleryImageDefinition.md)
Gyűjtemény képdefiníciójának törlése

### [Remove-AzGalleryImageVersion](Remove-AzGalleryImageVersion.md)
Gyűjtemény képének törlése

### [Remove-AzHost](Remove-AzHost.md)
Egy állomás eltávolítása.

### [Remove-AzHostGroup](Remove-AzHostGroup.md)
Egy állomás csoportjának eltávolítása.

### [Remove-AzImage](Remove-AzImage.md)
Eltávolít egy képet.

### [Remove-AzImageDataDisk](Remove-AzImageDataDisk.md)
Egy képobjektumból eltávolítja az adatlemezt.

### [Remove-AzProximityPlacementGroup](Remove-AzProximityPlacementGroup.md)
Törölje a közelségi hely erőforrást.

### [Remove-AzSnapshot](Remove-AzSnapshot.md)
Pillanatkép eltávolítása

### [Remove-AzVM](Remove-AzVM.md)
Eltávolítja a virtuális gépet az Azureról.

### [Remove-AzVMAccessExtension](Remove-AzVMAccessExtension.md)
Eltávolítja a VMAccess-bővítményt egy virtuális gépről.

### [Remove-AzVMAEMExtension](Remove-AzVMAEMExtension.md)
Eltávolítja az agrár-AEM bővítményt egy virtuális gépről.

### [Remove-AzVMBackup](Remove-AzVMBackup.md)
Eltávolítja a virtuális gép biztonsági másolatát.

### [Remove-AzVMChefExtension](Remove-AzVMChefExtension.md)
Eltávolítja a séf kiterjesztését egy virtuális gépről.

### [Remove-AzVMCustomScriptExtension](Remove-AzVMCustomScriptExtension.md)
Eltávolít egy egyéni parancsfájl-kiterjesztést egy virtuális gépről.

### [Remove-AzVMDataDisk](Remove-AzVMDataDisk.md)
Az adatlemez eltávolítása egy virtuális gépről.

### [Remove-AzVMDiagnosticsExtension](Remove-AzVMDiagnosticsExtension.md)
Eltávolítja a diagnosztikai bővítményt egy virtuális gépről.

### [Remove-AzVMDiskEncryptionExtension](Remove-AzVMDiskEncryptionExtension.md)
Eltávolítja a titkosítási bővítményt egy virtuális gépről.

### [Remove-AzVMDscExtension](Remove-AzVMDscExtension.md)
DSC-bővítmény eltávolítása egy erőforráscsoport virtuális gépén

### [Remove-AzVMExtension](Remove-AzVMExtension.md)
Eltávolít egy virtuális gépről a bővítményt.

### [Remove-AzVMNetworkInterface](Remove-AzVMNetworkInterface.md)
Egy virtuális gépről eltávolítja a hálózati felületet.

### [Remove-AzVMSecret](Remove-AzVMSecret.md)
Titkos (a) titok eltávolítása egy virtuális gép objektumból

### [Remove-AzVMSqlServerExtension](Remove-AzVMSqlServerExtension.md)
SQL Server-bővítmény eltávolítása virtuális gépről.

### [Remove-AzVmss](Remove-AzVmss.md)
Eltávolítja a VMSS belüli VMSS vagy virtuális gépet.

### [Remove-AzVmssDataDisk](Remove-AzVmssDataDisk.md)
Adatlemez eltávolítása a VMSS.

### [Remove-AzVmssDiagnosticsExtension](Remove-AzVmssDiagnosticsExtension.md)
Diagnosztikai bővítmény eltávolítása a VMSS.

### [Remove-AzVmssExtension](Remove-AzVmssExtension.md)
Eltávolít egy kiterjesztést a VMSS.

### [Remove-AzVmssNetworkInterfaceConfiguration](Remove-AzVmssNetworkInterfaceConfiguration.md)
Hálózati illesztőfelület-konfiguráció eltávolítása egy VMSS.

### [Remove-AzVmssVMDataDisk](Remove-AzVmssVMDataDisk.md)
Az adatlemez eltávolítása a virtuálisgép-készlet virtuális gépén

### [Javítás – AzVmssServiceFabricUpdateDomain](Repair-AzVmssServiceFabricUpdateDomain.md)
Kézi platform Update domain Walk to Update Virtual Machines a Service Fabric Virtual Machine Scale set.

### [Újraindítás – AzVM](Restart-AzVM.md)
Azure virtuális gép újraindítása.

### [Újraindítás – AzVmss](Restart-AzVmss.md)
A VMSS vagy egy virtuális gép újraindítása a VMSS belül.

### [Visszavonás-AzDiskAccess](Revoke-AzDiskAccess.md)
Hozzáférés visszavonása a lemezhez

### [Visszavonás-AzSnapshotAccess](Revoke-AzSnapshotAccess.md)
A pillanatképhez való hozzáférés visszavonása

### [Mentés-AzVhd](Save-AzVhd.md)
A letöltött. vhd képek helyi mentése

### [Mentés-AzVMImage](Save-AzVMImage.md)
Virtuális gép mentése VMImage.

### [Set-AzDiskDiskEncryptionKey](Set-AzDiskDiskEncryptionKey.md)
A Disk Encryption Key tulajdonságát állítja be a Disk objektumban.

### [Set-AzDiskImageReference](Set-AzDiskImageReference.md)
A kép hivatkozási tulajdonságait állítja be a Disk objektumban.

### [Set-AzDiskKeyEncryptionKey](Set-AzDiskKeyEncryptionKey.md)
A kulcs titkosítási kulcs tulajdonságait állítja be a Disk objektumban.

### [Set-AzDiskUpdateDiskEncryptionKey](Set-AzDiskUpdateDiskEncryptionKey.md)
A lemezvezérlő-titkosítási kulcs tulajdonságait állítja be a Disk Update objektumban.

### [Set-AzDiskUpdateKeyEncryptionKey](Set-AzDiskUpdateKeyEncryptionKey.md)
A kulcs titkosítási kulcs tulajdonságait állítja be a lemez Update objektumban.

### [Set-AzImageOsDisk](Set-AzImageOsDisk.md)
Az operációs rendszer lemezének tulajdonságait állítja be egy kép objektumon.

### [Set-AzSnapshotDiskEncryptionKey](Set-AzSnapshotDiskEncryptionKey.md)
A lemezvezérlő-titkosítási kulcs tulajdonságainak megadása egy pillanatkép-objektumon.

### [Set-AzSnapshotImageReference](Set-AzSnapshotImageReference.md)
Egy pillanatkép-objektum képhivatkozási tulajdonságainak megadása

### [Set-AzSnapshotKeyEncryptionKey](Set-AzSnapshotKeyEncryptionKey.md)
A kulcs titkosítási kulcs tulajdonságait állítja be egy pillanatkép-objektumon.

### [Set-AzSnapshotUpdateDiskEncryptionKey](Set-AzSnapshotUpdateDiskEncryptionKey.md)
A titkosító kulcs tulajdonságait állítja be egy Snapshot Update objektumban.

### [Set-AzSnapshotUpdateKeyEncryptionKey](Set-AzSnapshotUpdateKeyEncryptionKey.md)
A kulcs titkosítási kulcs tulajdonságait állítja be egy Snapshot Update objektumban.

### [Set-AzVM](Set-AzVM.md)
A virtuális gépet általánosként jelöli meg.

### [Set-AzVMAccessExtension](Set-AzVMAccessExtension.md)
A VMAccess bővítmény felvétele egy virtuális gépre.

### [Set-AzVMADDomainExtension](Set-AzVMADDomainExtension.md)
AD tartomány-kiterjesztést ad egy virtuális géphez.

### [Set-AzVMAEMExtension](Set-AzVMAEMExtension.md)
Engedélyezi a SAP-rendszerek figyelését.

### [Set-AzVMBackupExtension](Set-AzVMBackupExtension.md)
A biztonságimásolat-bővítmény tulajdonságainak megadása virtuális gépen.

### [Set-AzVMBginfoExtension](Set-AzVMBginfoExtension.md)
A BGInfo bővítmény felvétele egy virtuális gépre.

### [Set-AzVMBootDiagnostic](Set-AzVMBootDiagnostic.md)
A virtuális gép rendszerindítási diagnosztikai tulajdonságait módosítja.

### [Set-AzVMChefExtension](Set-AzVMChefExtension.md)
Kibővíti a séf kiterjesztését egy virtuális géphez.

### [Set-AzVMCustomScriptExtension](Set-AzVMCustomScriptExtension.md)
Egyéni parancsfájl-kiterjesztést ad hozzá egy virtuális géphez.

### [Set-AzVMDataDisk](Set-AzVMDataDisk.md)
Egy virtuális gép adatlemezének tulajdonságait módosítja.

### [Set-AzVMDiagnosticsExtension](Set-AzVMDiagnosticsExtension.md)
Az Azure Diagnostics bővítmény beállítása virtuális gépen.

### [Set-AzVMDiskEncryptionExtension](Set-AzVMDiskEncryptionExtension.md)
Engedélyezi a titkosítást egy futó IaaS virtuális gépen az Azure-ban.

### [Set-AzVMDscExtension](Set-AzVMDscExtension.md)
A DSC bővítmény beállítása virtuális gépen.

### [Set-AzVMExtension](Set-AzVMExtension.md)
Frissíti a bővítmény tulajdonságait, vagy bővítményt ad hozzá egy virtuális géphez.

### [Set-AzVMOperatingSystem](Set-AzVMOperatingSystem.md)
A virtuális gépek operációs rendszerének tulajdonságait állítja be.

### [Set-AzVMOSDisk](Set-AzVMOSDisk.md)
Beállítja az operációs rendszer lemezének tulajdonságait egy virtuális gépen.

### [Set-AzVMPlan](Set-AzVMPlan.md)
A piactér-terv adatait egy virtuális gépen állítja be.

### [Set-AzVMSourceImage](Set-AzVMSourceImage.md)
A virtuális gép képét adja meg.

### [Set-AzVMSqlServerExtension](Set-AzVMSqlServerExtension.md)
Egy virtuális gépen az Azure SQL Server-bővítményt állítja be.

### [Set-AzVmss](Set-AzVmss.md)
Meghatározott műveleteket határoz meg a megadott VMSS.

### [Set-AzVmssBootDiagnostic](Set-AzVmssBootDiagnostic.md)
A virtuálisgép-készlet rendszerindítási diagnosztikai profiljának beállítása.

### [Set-AzVmssDiskEncryptionExtension](Set-AzVmssDiskEncryptionExtension.md)
Lehetővé teszi a lemezek titkosítását a VM-léptékű készleteken.

### [Set-AzVmssOrchestrationServiceState](Set-AzVmssOrchestrationServiceState.md)
Beállítja a VMSS a hangszerelési szolgáltatás állapotát.

### [Set-AzVmssOsProfile](Set-AzVmssOsProfile.md)
Beállítja a VMSS operációs rendszer profiljának tulajdonságait.

### [Set-AzVmssRollingUpgradePolicy](Set-AzVmssRollingUpgradePolicy.md)
A VMSS-es folyamatos frissítési házirend tulajdonságainak megadása

### [Set-AzVmssStorageProfile](Set-AzVmssStorageProfile.md)
Beállítja a VMSS tárolási profiljának tulajdonságait.

### [Set-AzVmssVM](Set-AzVmssVM.md)
Módosítja egy VMSS-példány állapotát.

### [Start-AzVM](Start-AzVM.md)
Azure virtuális gépet indít el.

### [Start-AzVmss](Start-AzVmss.md)
Elindítja a VMSS vagy a virtuális gépeket a VMSS belül.

### [Start-AzVmssRollingOSUpgrade](Start-AzVmssRollingOSUpgrade.md)
Elindul a folyamatos frissítés, hogy az összes virtuálisgép-készlet példányát a legújabb elérhető platform Image OS verzióba helyezze át.

### [Stop-AzVM](Stop-AzVM.md)
Azure virtuális gép leállítása

### [Stop-AzVmss](Stop-AzVmss.md)
Leállítja a VMSS vagy a virtuális gépeket a VMSS belül.

### [Stop-AzVmssRollingUpgrade](Stop-AzVmssRollingUpgrade.md)
A jelenlegi virtuálisgép-méretarány beállítása a folyamatos frissítéshez.

### [Teszt-AzVMAEMExtension](Test-AzVMAEMExtension.md)
Ellenőrzi az agrár-AEM bővítmény konfigurációját.

### [Update-AzAvailabilitySet](Update-AzAvailabilitySet.md)
Elérhetőségi készlet frissítése

### [Update-AzContainerService](Update-AzContainerService.md)
Frissíti a tároló szolgáltatás állapotát.

### [Update-AzDisk](Update-AzDisk.md)
Frissít egy lemezt.

### [Update-AzDiskEncryptionSet](Update-AzDiskEncryptionSet.md)
Megadhatja a lemez-titkosítási készlet frissítését.

### [Update-AzGallery](Update-AzGallery.md)
Gyűjtemény frissítése

### [Update-AzGalleryImageDefinition](Update-AzGalleryImageDefinition.md)
Fényképgyűjtemény képdefiníciójának frissítése

### [Update-AzGalleryImageVersion](Update-AzGalleryImageVersion.md)
Gyűjtemény képének frissítése

### [Update-AzImage](Update-AzImage.md)
Frissít egy képet.

### [Update-AzSnapshot](Update-AzSnapshot.md)
Pillanatképek frissítése

### [Update-AzVM](Update-AzVM.md)
Frissíti az Azure Virtual Machine állapotát.

### [Update-AzVmss](Update-AzVmss.md)
Frissíti egy VMSS állapotát.

### [Update-AzVmssInstance](Update-AzVmssInstance.md)
Elindítja a VMSS példány manuális frissítését.

### [Update-AzVmssVM](Update-AzVmssVM.md)
Frissíti a Vmss VM állapotát.

