...
### Példa 1: a Function app beszerzése név szerint és leállítása.
```powershell
PS C:\> Get-AzFunctionApp -Name MyAppName -ResourceGroupName MyResourceGroupName | Stop-AzFunctionApp -Force
```

Ez a parancs a függvény alkalmazását név szerint kapja, és leállítja.

### 2. példa: a függvény alkalmazásának leállítása név szerint.
```powershell
PS C:\> Stop-AzFunctionApp -Name MyAppName -ResourceGroupName MyResourceGroupName -Force
```

Ez a parancs a függvény alkalmazását név szerint leállítja.
...
### -Force
Arra kényszeríti a parancsmagot, hogy állítsa le a függvény alkalmazást a megerősítés kérése nélkül.
