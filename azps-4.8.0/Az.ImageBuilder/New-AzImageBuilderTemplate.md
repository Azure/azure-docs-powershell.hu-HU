...
### Példa 1: virtuális gép Képsablon létrehozása
```powershell
...
Ezzel a paranccsal létrehozhatja a virtuális gép képsablonját.
...
A Képsablon létrehozásakor megvárni kívánt maximális időtartam
Az alapértelmezett (4 óra) beállításnál hagyja ki vagy adja meg a 0 értéket.
...
### -Testreszabás
A kép testreszabási lépéseinek (például a képforrásnak stb.) leírására szolgáló tulajdonságokat adja meg. A kivitelezéshez lásd: megjegyzések szakasz a tulajdonságok TESTRESZABÁSához és a kivonatoló táblázat létrehozásához.
...
### -DefaultProfile
terület: az Azuretal való kommunikációhoz használt hitelesítő adatok, fiók, bérlői fiók és előfizetések HideParameter.
...
Azok a terjesztési célok, amelyekben a kép kimenetének el kell térnie.
A kivitelezéshez lásd: megjegyzések szakasz a tulajdonságok ELOSZTÁSához és a kivonatoló táblázat létrehozásához.
...
Az utolsó futás befejezési időpontja (UTC)
...
Részletes információk az utolsó futtatási állapotról.
...
Az utolsó Futtatás alszintű állapota
...
Az utolsó Futtatás kezdési időpontja (UTC)
...
Részletes hibaüzenet a kiépítési hibáról.
...
A kivitelezéshez lásd: megjegyzések szakasz a forrás tulajdonságaihoz, és hozzon létre egy ujjlenyomat-táblázatot.
...
A felhasználó által kiosztott identitás azonosítója.
...
Az operációs rendszer merevlemezének mérete GB-ban
Az Azure alapértelmezett operációsrendszer-méretének használatához hagyja meg vagy adja meg a 0 értéket.
...
### -VMProfileVmSize
A képek létrehozásához, testreszabásához és rögzítéséhez használt virtuális gép mérete.
Hagyja üresen vagy adjon meg egy üres karakterláncot az alapértelmezett (Standard_D1_v2) használatához.
...
### -VnetConfigSubnetId
Egy korábban létező alhálózat erőforrás-azonosítója.
...
### Microsoft. Azure. PowerShell. parancsmagok. ImageBuilder. modellek. Api20200214. IImageTemplate
...
TESTRE <IImageTemplateCustomizer [] >: Itt adhatja meg a kép testreszabási lépéseit (például a képforrást stb.) leíró tulajdonságokat.
  - `Type <String>`: A képre használni kívánt testreszabási eszköz típusa. Például a "Shell" lehet Shell-testre szabva
  - `[Name <String>]`: Felhasználóbarát név a testreszabási lépésekről a környezet megadásához

DISTRIBUTion <IImageTemplateDistributor [] >: a terjesztési célok, amelyekben a kép kimenetének el kell jutnia.
  - `RunOutputName <String>`: A kapcsolódó RunOutput használandó név.
  - `Type <String>`: A terjesztési típus.
  - `[ArtifactTag <IImageTemplateDistributorArtifactTags>]`: A címkét, amelyet a program a forgalmazó által létrehozott/frissített, a tárgyhoz fog alkalmazni.
    - `[(Any) <String>]`: Ez azt jelzi, hogy az objektumhoz bármely tulajdonság hozzá lehet adni.

FORRÁS <IImageTemplateSource> : leírja a virtuális gép képforrását az építéshez, a testreszabáshoz és a terjesztéshez.
  - `Type <String>`: Itt adhatja meg, hogy milyen típusú képet szeretne kezdeni.
