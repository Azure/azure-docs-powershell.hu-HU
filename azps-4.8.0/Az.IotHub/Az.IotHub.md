---
Module Name: Az.IotHub
Module Guid: 51a3a993-fe39-4f8f-a437-2cb751584af5
Download Help Link: https://docs.microsoft.com/en-us/powershell/module/az.iothub
Help Version: 3.1.2.0
Locale: en-US
content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/IotHub/IotHub/help/Az.IotHub.md
original_content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/IotHub/IotHub/help/Az.IotHub.md
ms.openlocfilehash: ba7e171dfa925460d9b8aa7eadcd018cc388c219
ms.sourcegitcommit: 1de2b6c3c99197958fa2101bc37680e7507f91ac
ms.translationtype: MT
ms.contentlocale: hu-HU
ms.lasthandoff: 10/13/2020
ms.locfileid: "94018080"
---
# Az. IotHub modul
## Leírás
Az Azure IotHubs kezelésére szolgáló parancsok

## Az. IotHub parancsmagok
### [Add-AzIotHubCertificate](Add-AzIotHubCertificate.md)
Azure IoT hub tanúsítvány létrehozása/frissítése

### [Add-AzIotHubConfiguration](Add-AzIotHubConfiguration.md)
IoT automatikus eszközkezelés-konfiguráció felvétele a TARGET IoT-központban.

### [Add-AzIotHubDeployment](Add-AzIotHubDeployment.md)
IoT elhelyezése a TARGET IoT-központban

### [Add-AzIotHubDevice](Add-AzIotHubDevice.md)
Hozzon létre egy eszközt egy IoT-központban.

### [Add-AzIotHubDeviceChildren](Add-AzIotHubDeviceChildren.md)
A nem szélezett eszközök felvétele gyermekként az Edge-eszközre.

### [Add-AzIotHubEventHubConsumerGroup](Add-AzIotHubEventHubConsumerGroup.md)
Egy eventhub-fogyasztói csoportot hoz létre.

### [Add-AzIotHubKey](Add-AzIotHubKey.md)
Létrehoz egy IotHub kulcsot.

### [Add-AzIotHubMessageEnrichment](Add-AzIotHubMessageEnrichment.md)
Az IoT-központban kiválasztott végpontokra mutató üzenet dúsítását hozza létre.

### [Add-AzIotHubModule](Add-AzIotHubModule.md)
Hozzon létre egy modult egy cél IoT-eszközön egy IoT-központban.

### [Add-AzIotHubRoute](Add-AzIotHubRoute.md)
Útvonal létrehozása az IoT-központban

### [Add-AzIotHubRoutingEndpoint](Add-AzIotHubRoutingEndpoint.md)
Végpont felvétele a IoT-hub-ba

### [Get-AzIotHub](Get-AzIotHub.md)
Információt kap az előfizetésben szereplő IotHubs.

### [Get-AzIotHubCertificate](Get-AzIotHubCertificate.md)
Az Azure IoT-központban található összes tanúsítvány vagy tanúsítvány felsorolása. 

### [Get-AzIotHubCertificateVerificationCode](Get-AzIotHubCertificateVerificationCode.md)
Hitelesítő kódot hoz létre az Azure IoT hub tanúsítványához. 

### [Get-AzIotHubConfiguration](Get-AzIotHubConfiguration.md)
Felsorolja az automatikus eszközkezelés teljes konfigurációját vagy egy bizonyos IoT.

### [Get-AzIotHubConnectionString](Get-AzIotHubConnectionString.md)
Megkapja a IotHub connectionStrings.

### [Get-AzIotHubDeployment](Get-AzIotHubDeployment.md)
Felsorolja az egész vagy egy adott IoT-alapú központi telepítőt.

### [Get-AzIotHubDevice](Get-AzIotHubDevice.md)
Az Azure IoT-központban található összes eszköz vagy egy adott eszköz felsorolása. 

### [Get-AzIotHubDeviceChildren](Get-AzIotHubDeviceChildren.md)
A hozzárendelt alárendelt eszközök pontosvesszővel tagolt listájának nyomtatása.

### [Get-AzIotHubDeviceConnectionString](Get-AzIotHubDeviceConnectionString.md)
A IOT-hub IoT eszközének kapcsolati karakterláncának beszerzése

### [Get-AzIotHubDeviceParent](Get-AzIotHubDeviceParent.md)
A megadott eszköz fölérendelt eszközének beolvasása

### [Get-AzIotHubDeviceTwin](Get-AzIotHubDeviceTwin.md)
Különálló eszközt kap.

### [Get-AzIotHubDistributedTracing](Get-AzIotHubDistributedTracing.md)
Az adott eszközhöz tartozó elosztott nyomkövetési beállítások beszerzése

### [Get-AzIotHubEventHubConsumerGroup](Get-AzIotHubEventHubConsumerGroup.md)
Megkapja az összes eventhub-consumergroups.

### [Get-AzIotHubJob](Get-AzIotHubJob.md)
Információt kap egy IotHub-feladatról.

### [Get-AzIotHubKey](Get-AzIotHubKey.md)
Kap egy IotHub kulcsot.

### [Get-AzIotHubMessageEnrichment](Get-AzIotHubMessageEnrichment.md)
Felsorolja az összes üzenet dúsítását, vagy az IoT-hub adott üzenet dúsítását.

### [Get-AzIotHubModule](Get-AzIotHubModule.md)
Megtudhatja, hogy miként találhatja meg egy IoT-eszköz vagy egy IoT-eszközön található lista-modul adatait egy IoT-központban.

### [Get-AzIotHubModuleConnectionString](Get-AzIotHubModuleConnectionString.md)
Megtudhatja, hogy milyen kapcsolati karakterláncot hoz létre a TARGET IoT Device Module egy IOT-központban.

### [Get-AzIotHubModuleTwin](Get-AzIotHubModuleTwin.md)
IoT-os Device Module-modult kap.

### [Get-AzIotHubQuotaMetric](Get-AzIotHubQuotaMetric.md)
A kvóta metrikáját kapja egy IotHub.

### [Get-AzIotHubRegistryStatistic](Get-AzIotHubRegistryStatistic.md)
Megkapja a RegistryStatistics egy IotHub.

### [Get-AzIotHubRoute](Get-AzIotHubRoute.md)
Információk kérése az útvonalról az IoT-központban

### [Get-AzIotHubRoutingEndpoint](Get-AzIotHubRoutingEndpoint.md)
Információk beszerzése a IoT-hub összes végpontján

### [Get-AzIotHubValidSku](Get-AzIotHubValidSku.md)
Beilleszt minden érvényes SKU-t, amelyre a IotHub át tud térni.

### [Meghívó-AzIotHubConfigurationMetricsQuery](Invoke-AzIotHubConfigurationMetricsQuery.md)
Hivatkozhat egy IoT-eszköz konfigurációs metrikájának lekérdezésére.

### [Meghívó-AzIotHubDeploymentMetricsQuery](Invoke-AzIotHubDeploymentMetricsQuery.md)
Hívja fel a IoT Edge telepítő metrikus lekérdezését.

### [Meghívó-AzIotHubDeviceMethod](Invoke-AzIotHubDeviceMethod.md)
Közvetlen módszer meghívása egy eszközön.

### [Meghívó-AzIotHubManualFailover](Invoke-AzIotHubManualFailover.md)
A IoT-hub feladatátvételi folyamatának meghívása a Geo-párosított katasztrófa-helyreállítási területre.

### [Meghívó-AzIotHubModuleMethod](Invoke-AzIotHubModuleMethod.md)
Az Edge modul metódusának meghívása

### [Meghívó-AzIotHubQuery](Invoke-AzIotHubQuery.md)
IoT-hub lekérdezése hatékony SQL-szerű nyelv használatával.

### [Új – AzIotHub](New-AzIotHub.md)
Új IotHub hoz létre.

### [Új – AzIotHubExportDevice](New-AzIotHubExportDevice.md)
Új exportálási eszközök feladatot hoz létre.

### [Új – AzIotHubImportDevice](New-AzIotHubImportDevice.md)
Új importálási eszközök feladatot hoz létre.

### [Új – AzIotHubKey](New-AzIotHubKey.md)
Azure IoT hub-kulcs létrehozása

### [Új – AzIotHubSasToken](New-AzIotHubSasToken.md)
Hozzon létre egy SAS-tokent a TARGET IoT hub, eszköz vagy modul számára.

### [Remove-AzIotHub](Remove-AzIotHub.md)
Töröl egy IotHub.

### [Remove-AzIotHubCertificate](Remove-AzIotHubCertificate.md)
Azure IoT hub tanúsítvány törlése

### [Remove-AzIotHubConfiguration](Remove-AzIotHubConfiguration.md)
IoT-eszköz konfigurációjának törlése

### [Remove-AzIotHubDeployment](Remove-AzIotHubDeployment.md)
IoT-alapú központi példány törlése

### [Remove-AzIotHubDevice](Remove-AzIotHubDevice.md)
IoT-hub eszköz törlése

### [Remove-AzIotHubDeviceChildren](Remove-AzIotHubDeviceChildren.md)
A nem szélezett eszközök eltávolítása gyermekként a megadott Edge-eszközön.

### [Remove-AzIotHubEventHubConsumerGroup](Remove-AzIotHubEventHubConsumerGroup.md)
Töröl egy eventhub-consumergroup.

### [Remove-AzIotHubKey](Remove-AzIotHubKey.md)
Eltávolít egy IotHub kulcsot.

### [Remove-AzIotHubMessageEnrichment](Remove-AzIotHubMessageEnrichment.md)
Üzenet dúsításának törlése a IoT-központban.

### [Remove-AzIotHubModule](Remove-AzIotHubModule.md)
Modul (ok) törlése egy IoT-hub IoT-eszközén.

### [Remove-AzIotHubRoute](Remove-AzIotHubRoute.md)
Útvonal törlése az IoT hub-ban

### [Remove-AzIotHubRoutingEndpoint](Remove-AzIotHubRoutingEndpoint.md)
A IoT-hub végpontjának törlése

### [Küldés-AzIotHubDevice2CloudMessage](Send-AzIotHubDevice2CloudMessage.md)
Eszköz – Felhőbeli üzenet küldése

### [Set-AzIotHub](Set-AzIotHub.md)
Frissíti egy IotHub tulajdonságait.

### [Set-AzIotHubConfiguration](Set-AzIotHubConfiguration.md)
Frissítse a konfigurációs regisztráció változékony mezőit.

### [Set-AzIotHubDeployment](Set-AzIotHubDeployment.md)
Frissítheti a IoT Edge-telepítések változékony mezőit.

### [Set-AzIotHubDevice](Set-AzIotHubDevice.md)
IoT-hub eszköz frissítése

### [Set-AzIotHubDeviceParent](Set-AzIotHubDeviceParent.md)
A megadott eszköz fölérendelt eszközének beállítása.

### [Set-AzIotHubDistributedTracing](Set-AzIotHubDistributedTracing.md)
Az eszköz terjesztett nyomkövetési beállításainak frissítése

### [Set-AzIotHubEdgeModule](Set-AzIotHubEdgeModule.md)
Az Edge-modulokat egyetlen Edge-eszközön állítsa be.

### [Set-AzIotHubMessageEnrichment](Set-AzIotHubMessageEnrichment.md)
Frissítse az üzenet dúsítását a IoT-központban.

### [Set-AzIotHubModule](Set-AzIotHubModule.md)
Modul frissítése egy IoT-eszközön egy IoT-központban.

### [Set-AzIotHubRoute](Set-AzIotHubRoute.md)
Útvonal frissítése az IoT hub-ban

### [Set-AzIotHubVerifiedCertificate](Set-AzIotHubVerifiedCertificate.md)
Ellenőrzi az Azure IoT hub tanúsítványát. 

### [Teszt-AzIotHubRoute](Test-AzIotHubRoute.md)
Tesztelési útvonalak az IoT-központban

### [Update-AzIotHub](Update-AzIotHub.md)
Az Azure IoT-hub frissítése

### [Update-AzIotHubDeviceTwin](Update-AzIotHubDeviceTwin.md)
A Twin eszközök címkéit és a kívánt tulajdonságait frissíti.

### [Update-AzIotHubModuleTwin](Update-AzIotHubModuleTwin.md)
A IoT-eszközök moduljainak címkéi és a kívánt tulajdonságai frissítik.

