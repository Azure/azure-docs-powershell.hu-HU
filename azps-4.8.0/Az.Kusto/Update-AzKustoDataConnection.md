...
### UpdateExpandedEventHub (alapértelmezett)
```
...
### Példa 1: meglévő EventHub-adatkapcsolat frissítése
```powershell
...
A fenti parancs frissíti a "myeventhubdc" nevű "mykustodatabase" nevű adatbázis "" nevű "" nevű adatbázis "testnewkustocluster" nevű "EventHub" nevű adatkapcsolatát.

### 2. példa: meglévő EventGrid-adatkapcsolat frissítése
```powershell
...
A fenti parancs frissíti a "myeventgriddc" nevű "mykustodatabase" nevű adatbázis "" nevű "" nevű adatbázis "testnewkustocluster" nevű "EventGrid" nevű adatkapcsolatát.

### 3. példa: meglévő IotHub-adatkapcsolat frissítése
```powershell
...
A fenti parancs frissíti a "myiothubdc" nevű "mykustodatabase" nevű adatbázis "" nevű "" nevű adatbázis "testnewkustocluster" nevű "IotHub" nevű adatkapcsolatát.

### Példa 4: meglévő EventHub-adatkapcsolat frissítése identitás útján
```powershell
...
### Példa 5: meglévő EventGrid-adatkapcsolat frissítése identitás útján
```powershell
...
### Példa 6: meglévő IotHub-adatkapcsolat frissítése identitás útján
```powershell
...
A feldolgozni kívánt blob-tárolási esemény neve.
...
### -Tömörítés
Az esemény hub-üzenetek tömörítési típusa.
...
### -ConsumerGroup
Az esemény/IOT hub fogyasztói csoport.
...
Tetszés szerint minden üzenethez hozzáadhat adatformátumot.
...
### -EventHubResourceId
Az adatkapcsolat/esemény rácsának létrehozásához használt esemény hub erőforrás-azonosítója az események küldésére van konfigurálva.
...
### -IgnoreFirstRecord
Ha a True értékre van állítva, akkor az azt jelzi, hogy a lenyelési érték minden fájl első rekordját figyelmen kívül hagyja.
...
### -IotHubResourceId
Az IOT-központ erőforrás-azonosítója, amellyel adatkapcsolat hozható létre.
...
### -MappingRuleName
Az adatvesztéshez használandó megfeleltetési szabály.
Tetszés szerint a megfeleltetési adatok mindegyik üzenethez hozzáadhatók.
...
A megosztási hozzáférés házirendjének neve.
...
Annak a tárolási fióknak az erőforrás-azonosítója, amelyben az adatforgalom található.
...
### -Táblanév
Az a táblázat, ahol az adatot el kell fogyasztani.
Tetszés szerint a táblázat adatai hozzáadhatók az egyes üzenetekhez.
...
### Microsoft. Azure. PowerShell. parancsmagok. Kusto. modellek. Api20200614. IDataConnection
