...
## Leírás
A **Get-AzManagementGroupDeploymentWhatIfResult** parancsmag beilleszti a kar sablonját, What-If a megadott felügyeleti csoport hatókörének egy sablonba való bevezetésének eredményét adja eredményül. A módosítások listáját jeleníti meg, amely azt jelzi, hogy milyen erőforrások fognak frissülni, ha a telepítés a valós erőforrások módosítása nélkül lép érvénybe. Ha meg szeretné adni a visszatérési eredmény formátumát, használja a *ResultFormat* paramétert.

## Példák

### Példa 1: What-If eredményének beszerzése a felügyeleti csoport hatókörében
```powershell
...
Ez a parancs egy egyéni sablonfájl és egy, a lemezen lévő paraméteres fájl segítségével What-If eredményét kapja a felügyeleti csoport hatókörében.
A parancs a hely paramétert használva adja meg a központi telepítő adatainak tárolásának *helyét* .
A parancs a *ManagementGroupId* paraméterrel adja meg azt a kezelési csoportot, amelyben a sablont telepítik.
...
### 2. példa: What-If eredményének beszerzése a felügyeleti csoport hatókörében a ResourceIdOnly használatával
```powershell
PS C:\> Get-AzManagementGroupDeploymentWhatIfResult `
    -DeploymentName "deploy-01" `
    -Location "West US" `
...
Ha nem adja meg, akkor az alapértelmezett érték a sablonfájl nevében, amikor a sablonfájl meg van adva. Alapértelmezés szerint az aktuális időpontot kapja, ha a Template-objektum meg van határozva, például "20131223140835".
