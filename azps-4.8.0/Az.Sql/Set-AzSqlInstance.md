...
## Leírás
A **set-AzSqlInstance** parancsmag egy Azure SQL-adatbázis felügyelt példányának tulajdonságait módosítja.

## Példák

### 1. példa: a meglévő példány beállítása új AdministratorPassword,-LicenseType,-StorageSizeInGB,-VCore és-kiadásban.
```powershell
...
### 2. példa: a meglévő példány hardver-generálásának módosítása új ComputeGeneration-érték használatával
```powershell
...
### 3. példa: a meglévő példány beállítása új értékekkel – AdministratorPassword,-LicenseType,-StorageSizeInGB és-VCore egy példányban egy példányon belül
```powershell
PS C:\>$InstancePassword = "Newpassword1234"
PS C:\> $SecureString = ConvertTo-SecureString $InstancePassword -AsPlainText -Force
...
Ez a parancs a meglévő példányokat a-AdministratorPassword, a-LicenseType, a-StorageSizeInGB és-VCore új értékekkel állítja be egy adott példányon belül.
