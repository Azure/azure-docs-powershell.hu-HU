...
## Leírás
A **Restore-AzStorageBlobRange** parancsmag bizonyos blob-tartományokra visszaállítja a blobokat egy tárterület-fiókban.
A program a kezdőpontot is tartalmazza, és a végpontot kihagyja a blob-visszaállításban.

## Példák

### 1. példa: indítsa újra a blobokat egy bizonyos blob-tartományokat tartalmazó tárterület-fiókban.
```powershell
...
Ez a parancs először 2 blob-tartományt hoz létre, majd a blobokat az 1 napja A felhasználó a Get-AzStorageAccount segítségével később nyomon követheti a visszaállítás állapotát.

### 2. példa: az összes blob tárolása a backend-fiókban
```powershell
...
A parancs a tárterület-fiók minden blobját visszaállítja 30 perccel ezelőtt, és megvárja a visszaállítás befejeződését. Mivel a módosítások visszaállítása hosszú időt vesz igénybe, a backend with-Asjob paraméterrel futtatja azt, majd megvárja a feladat befejezését és a találatok megjelenítését.

### 3. példa: a Blobs-adatok közvetlen beírásával visszaállítja a blob-tartományokat, és várja meg, amíg kész
```powershell
...
A parancs visszaállítja a blobokat egy tárolási fiókból 1 nappal korábban, a 2 blob-tartomány közvetlen beírásával az Restore-AzStorageBlobRange parancsmagba. Ez a parancs a visszaállítás befejeződésére fog várni.
...
### -BlobRestoreRange
A visszaállítandó blob-tartománnyal.
Ha nem adja meg ezt a paramétert, a program visszaállítja az összes blobot.
...
### -TimeToRestore
A blob visszaállításának ideje.
...
### -WaitForComplete
Várakozás a tevékenység visszaállítása befejezésre
...
### Microsoft. Azure. Command. Management. Storage. models. PSBlobRestoreStatus
