...
## Leírás
A **Start-AzStorageBlobCopy** parancsmag a blob másolását indítja el.
...
Ez a parancs a ContosoUploads nevű tárolót a **Get-AzStorageContainer** parancsmaggal kapja meg, majd a tárolót átadja az aktuális parancsmagnak a pipeline operátor használatával.
...
### 3. példa: az összes blobot beolvashatja egy tárolóba, és a vágólapra másolhatja őket
```
C:\PS>Get-AzStorageBlob -Container "ContosoUploads" | Start-AzStorageBlobCopy -DestContainer "ContosoArchives"
```

Ez a parancs a ContosoUploads nevű tárolóban lévő blobokat a **Get-AzStorageBlob** parancsmag használatával kapja meg, majd a csővezeték-kezelő segítségével átadja az eredményeket az aktuális parancsmagnak.
...
A parancs elindítja a másolási műveletet a $Contextban tárolt cél környezetbe.
A forrás-objektumnak nincs tárolási környezete, ezért a forrás URI-nak hozzáféréssel kell rendelkeznie a forrás objektumhoz. Például: Ha a forrás egy nincs nyilvános Azure Blob, az URI-nek olyan SAS-tokent kell tartalmaznia, amelynek olvasási hozzáférése van a blobhoz.

### Példa 6: a blokk blob másolása a cél tárolóba egy új blob nevű névvel, a cél blob StandardBlobTier beállítása Forróként, RehydratePriority
```
...
Ez a parancs elindítja a blokk blob másolási műveletét a cél tárolóba egy új blob nevű névvel, és beállítja a cél blob-StandardBlobTier Forróként, RehydratePriority
...
### -RehydratePriority
BLOB-RehydratePriority letiltása Annak a prioritásnak a meghatározása, amellyel az archivált blobot újrahidratálja. Az érvényes értékek: magas/normál.
...
### -SrcFile
Egy **CloudFile** -objektumot ad meg az Azure Storage Client Library webhelyről.
...
### -StandardBlobTier
BLOB-szint letiltása: az érvényes értékek a meleg/hűvös/Archívek.
Részletes tudnivalók https://docs.microsoft.com/en-us/azure/storage/blobs/storage-blob-storage-tiers
