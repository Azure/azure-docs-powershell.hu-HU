...
## Leírás
Ezzel a parancsmaggal támogatási jegyet hozhat létre a számlázáshoz, az előfizetés-kezeléshez, a kvóta-vagy műszaki problémákhoz. A Get-AzSupportService és az Get-AzSupportProblemClassification parancsmagok segítségével keresse meg az Azure-szolgáltatást, és a megfelelő probléma a probléma besorolása, amelyben támogatást szeretne kérni. A következő paramétereket kell megadnia: 
...
A New-AzSupportContactProfileObject-segéd parancsmag segítségével CustomerContactDetail-objektumokat hozhat létre.

A felhőalapú megoldások szolgáltatói az ügyfél bérlői fiókjába bejelentkezve támogatási jegyet hozhatnak létre az ügyfél előfizetéséhez, és a *CSPHomeTenantId* paraméterrel megadhatják az otthoni bérlői azonosítót.

__Műszaki jegyek esetén:__

Az erőforrás nevének megadásához adja meg az erőforrás ARM erőforrás-AZONOSÍTÓját a *TechnicalTicketResourceId* paraméter használatával. Lásd az alábbi példát. 

__A kvóták esetében:__

Ha a **VM-magok** , a **köteg** , az SQL- **adatbázis** és az **SQL-adattárház** kvótájának növelését szeretné kérni, további részleteket adhat meg a *QuotaTicketDetail* objektum csoportban. Az QuotaTicketDetail objektum 3 tulajdonságból áll, az alább leírt módon. Részletes dokumentációért [kattintson ide.](https://aka.ms/supportrpquotarequestpayload)
...
Ha részletesen szeretné tudni, hogy miként építhet fel hasznos tartalmakat a különféle kontingensek típusaira, [kattintson ide](https://aka.ms/supportrpquotarequestpayload)

## Példák

### Példa 1: számlázási vagy előfizetés-kezelési támogatási jegy létrehozása. A Get-AzSupportService és a Get-AzSupportProblemClassification segítségével megkeresheti a számlázási vagy az előfizetés-kezelési problémák helyes AZONOSÍTÓját, amelyre támogatást szeretne kérni 
```powershell
...
### 2. példa: terméktámogatási jegy létrehozása a virtuális gép Windows-erőforráshoz. A Get-AzSupportService és a Get-AzSupportProblemClassification segítségével megkeresheti a Windows-alapú virtuális gép megfelelő GUID-azonosítóit, amelyekre támogatást szeretne kérni 
```powershell
...
### 3. példa: kvóta-támogatási jegy létrehozása egy adott VM-családhoz tartozó virtuális gép magok kvótájának növeléséhez. A Get-AzSupportService és a Get-AzSupportProblemClassification segítségével a kvóta kiszámításához a VM magok probléma besorolása című fejezet helyes GUID-elemeit olvashatja be.
```powershell
...
### Példa 4: kvóta-támogatási jegy létrehozása egy köteg-fiókban a kis prioritású magok kvótájának növeléséhez. A Get-AzSupportService és a Get-AzSupportProblemClassification segítségével megkeresheti a kvóta-használati problémák besorolásának megfelelő GUID-elemeit.
```powershell
...
### Példa 5: kvóta-támogatási jegy létrehozása egy adott VM-családhoz tartozó VM magok kvótájának növelése céljából egy köteg fiók esetén. A Get-AzSupportService és a Get-AzSupportProblemClassification segítségével megkeresheti a kvóta-használati problémák besorolásának megfelelő GUID-elemeit.
```powershell
...
### 6. példa: kvóta-támogatási jegy létrehozása egy köteghez tartozó készlet kvótájának növeléséhez. A Get-AzSupportService és a Get-AzSupportProblemClassification segítségével megkeresheti a kvóta-használati problémák besorolásának megfelelő GUID-elemeit.
```powershell
...
### 7. példa: kvóta-támogatási jegy létrehozása a kötegelt fiók aktív feladatainak és munkaütemezések kvótájának növeléséhez. A Get-AzSupportService és a Get-AzSupportProblemClassification segítségével megkeresheti a kvóta-használati problémák besorolásának megfelelő GUID-elemeit.
```powershell
...
### 8. példa: kvóta-támogatási jegy létrehozása az előfizetéshez tartozó kötegelt fiókok számának növeléséhez. A Get-AzSupportService és a Get-AzSupportProblemClassification segítségével megkeresheti a kvóta-használati problémák besorolásának megfelelő GUID-elemeit.
```powershell
...
### 9. példa: kvóta-támogatási jegy létrehozása az SQL-adatbázis DTU kvótájának növeléséhez. Az Get-AzSupportService és a Get-AzSupportProblemClassification segítségével beolvashatja a kvóta SQL-adatbázis problémáinak besorolása című fejezet helyes GUID azonosítóit.
```powershell
...
### 10. példa: kvóta-támogatási jegy létrehozása az SQL-adatbázis kiszolgálói kvótájának növeléséhez. Az Get-AzSupportService és a Get-AzSupportProblemClassification segítségével beolvashatja a kvóta SQL-adatbázis problémáinak besorolása című fejezet helyes GUID azonosítóit.
```powershell
...
### 11-es példa: kvóta-támogatási jegy létrehozása az SQL-adattárház DTU kvótájának növeléséhez. Az Get-AzSupportService és a Get-AzSupportProblemClassification segítségével megkeresheti a kvóta SQL Date Warehouse-probléma besorolásának megfelelő GUID-azonosítókat.
```powershell
...
### 12 példa: kvóta-támogatási jegy létrehozása az SQL-adattárházban lévő kiszolgálók kvótájának növeléséhez. A kvóta SQL-adatraktár problémáinak osztályozása az Get-AzSupportService és a Get-AzSupportProblemClassification használatával.
```powershell
PS C:\> New-AzSupportTicket -Name "test1" -Title "Test" -Description "Test" -Severity "minimal" -ProblemClassificationId "/providers/Microsoft.Support/services/{quota_service_guid}/problemClassifications/{sql_datawarehouse_problemClassification_guid}" -QuotaTicketDetail @{QuotaChangeRequestVersion = "1.0" ; QuotaChangeRequestSubType = "Servers" ; QuotaChangeRequests = (@{Region = "westus"; Payload = "{`"NewLimit`":200}"})} -CustomerContactDetail @{FirstName = "first" ; LastName = "last" ; PreferredTimeZone = "pacific standard time" ; PreferredSupportLanguage = "en-us" ; Country = "USA" ; PreferredContactMethod = "Email" ; PrimaryEmailAddress = "user@contoso.com"}
...
### 13. példa: kvóta-támogatási jegy létrehozása a kis prioritású magokra vonatkozó kvóta növeléséhez a gépi tanulási szolgáltatáshoz. A Get-AzSupportService és a Get-AzSupportProblemClassification segítségével megkeresheti a kvóta-automaták tanulási szolgáltatási problémájának megfelelő GUID-azonosítókat.
```powershell
...
### 14. példa: kvóta-támogatási jegy létrehozása a VM magok kvótájának növelése egy adott VM-családban a gépi tanulási szolgáltatáshoz. A Get-AzSupportService és a Get-AzSupportProblemClassification segítségével megkeresheti a kvóta-automaták tanulási szolgáltatási problémájának megfelelő GUID-azonosítókat.
```powershell
...
### 15. példa: kvóta-támogatási jegy létrehozása az Azure SQL-alapú felügyelt példány kvótájának növeléséhez. A Get-AzSupportService és a Get-AzSupportProblemClassification segítségével megkeresheti a kvóta SQL-alapú felügyelt példány-kezelési problémájának megfelelő GUID-azonosítókat.
```powershell
...
### 16 példa: támogatási jegy létrehozása a CustomerContactDetail objektum helyett az egyes ügyfél-kapcsolatfelvételi paraméterek megadásával. 
```powershell
...
### 17. példa: támogatási jegy létrehozása az Azure 24 x 7 válaszára vonatkozó kéréssel 
```powershell
...
### 18. példa: a támogatási jegy létrehozása az ügyfél nevében a felhőalapú megoldás-szolgáltató (CSP) esetén. A CSP-nek először be kell jelentkeznie a bérlői webhelyre, majd be kell jelentkeznie az ügyfél bérlői webhelyére az alábbi példa szerint. A támogatási jegy létrehozásakor a-CSPHomeTenantId paraméterrel meg kell adnia az otthoni bérlői azonosítót.  
```powershell
...
Az itt felsorolt e-mail-címeket a támogatási jegyre vonatkozó minden levelezésre másolhatja.
...
### -CSPHomeTenantId
Ez a felhőalapú megoldás-szolgáltató felhasználó otthoni bérlői azonosítója, amellyel támogatási jegyet hozhat létre az ügyfél számára.
...
Az SupportTicket-erőforráshoz társított ügyfél-elérhetőségi adatok.
...
### -CustomerCountry
Ügyfél országa.
A kódnak érvényes ISO Alpha-3 országkód (ISO 3166) kell lennie.
...
### -CustomerPhoneNumber
Ügyfél telefonszáma.
Erre akkor van szükség, ha az elsődleges névjegykártya-módszer telefon.
...
Az egyéni Peferred nyelve.
Ennek az itt felsorolt támogatott nyelvek egyikéhez érvényes célnyelv-kódnak kell lennie https://azure.microsoft.com/en-us/support/faq/ .
...
Ügyfél által preferált időzóna
Ennek érvényes System.TimeZoneInfo.Id-értéknek kell lennie.
...
### -ProblemClassificationId
Minden Azure-szolgáltatáshoz tartozik egy saját, a problémára vonatkozó besorolási kategória, amely megfelel az észlelt probléma típusának.
Ez a paraméter a ProblemClassification-erőforrás ARM erőforrás-azonosítója.
...
### -ProblemStartTime
A probléma kezdetének dátuma és időpontja.
...
### -QuotaTicketDetail
További részletek a kvóta-támogatási jegyekről.
...
### -Require24X7Response
Jelzi, hogy ez a támogatási jegy az Azuretól kapott 24x7 választ igényli-e.
...
Ez jelzi az eset sürgősségét, ami viszont az Azure-szal kötött technikai támogatási csomag szolgáltatási szintről szóló szerződése szerint a válaszadás időpontját határozza meg.
...
Ez az Azure szolgáltatási erőforrás erőforrás-azonosítója (például: egy virtuális gép vagy egy HDInsight-erőforrás), amelyhez a támogatási jegyet létrehozta.
