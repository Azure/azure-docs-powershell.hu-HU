---
Module Name: Az.Synapse
Module Guid: 89eceb4f-9916-4ec5-8499-d5cca97a9cae
Download Help Link: https://docs.microsoft.com/en-us/powershell/module/az.synapse
Help Version: 0.0.1.0
Locale: en-US
content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Synapse/Synapse/help/Az.Synapse.md
original_content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Synapse/Synapse/help/Az.Synapse.md
ms.openlocfilehash: 4f5325e7dc0451941361eacd956c4445d7f8c5e0
ms.sourcegitcommit: 1de2b6c3c99197958fa2101bc37680e7507f91ac
ms.translationtype: MT
ms.contentlocale: hu-HU
ms.lasthandoff: 10/13/2020
ms.locfileid: "94181236"
---
# Az. szinapszis modul
## Leírás
Az ebben a szakaszban található témakörök az Azure-szinapszisok Analytics rendszerére vonatkozó Azure PowerShell-parancsmagokat dokumentálják.

## Az. szinapszis parancsmagok
### [Add-AzSynapseTriggerSubscription](Add-AzSynapseTriggerSubscription.md)
Iratkozzon fel az eseményindítót külső szolgáltatási eseményekre.

### [Exportálás – AzSynapseNotebook](Export-AzSynapseNotebook.md)
Exportálja a notbooks.

### [Get-AzSynapseActivityRun](Get-AzSynapseActivityRun.md)
A tevékenységekről szóló információkat a csővezeték-futás során futtatja.

### [Get-AzSynapseDataFlow](Get-AzSynapseDataFlow.md)
Információt kap a munkaterületen lévő adatforgalomról.

### [Get-AzSynapseDataset](Get-AzSynapseDataset.md)
Információt kap az adatkészletekről a munkaterületen.

### [Get-AzSynapseFirewallRule](Get-AzSynapseFirewallRule.md)
Megkapja a szinapszis-elemzési tűzfalszabályok szabályát.

### [Get-AzSynapseIntegrationRuntime](Get-AzSynapseIntegrationRuntime.md)
Információt kap az integrációs futtatókörnyezet erőforrásairól.

### [Get-AzSynapseIntegrationRuntimeKey](Get-AzSynapseIntegrationRuntimeKey.md)
Egy önállóan működtetett integrációs futásidejű kulcsokat kap.

### [Get-AzSynapseIntegrationRuntimeMetric](Get-AzSynapseIntegrationRuntimeMetric.md)
Az integrációs futtatókörnyezet metrikus adatait kapja. 

### [Get-AzSynapseIntegrationRuntimeNode](Get-AzSynapseIntegrationRuntimeNode.md)
Beolvassa az integrációs futtatókörnyezet csomópontjának adatait.

### [Get-AzSynapseLinkedService](Get-AzSynapseLinkedService.md)
Információt kap a munkaterületen lévő kapcsolt szolgáltatásokról.

### [Get-AzSynapseNotebook](Get-AzSynapseNotebook.md)
Információt kap a munkaterületen lévő jegyzetfüzetekről.

### [Get-AzSynapsePipeline](Get-AzSynapsePipeline.md)
Információt kap a munkaterületen lévő csővezetékekről.

### [Get-AzSynapsePipelineRun](Get-AzSynapsePipelineRun.md)
Információt kap a csővezetékről.

### [Get-AzSynapseRoleAssignment](Get-AzSynapseRoleAssignment.md)
A szinapszis-Analytics szerepkör-hozzárendelést kapja.

### [Get-AzSynapseRoleDefinition](Get-AzSynapseRoleDefinition.md)
Egy szinapszis-Analytics szerepkör-definíciót kap.

### [Get-AzSynapseSparkJob](Get-AzSynapseSparkJob.md)
Egy szinapszis-elemző szikra munkát kap.

### [Get-AzSynapseSparkPool](Get-AzSynapseSparkPool.md)
A szinapszis-elemző Spark-medencét kapja.

### [Get-AzSynapseSparkSession](Get-AzSynapseSparkSession.md)
A szinapszis-elemző Spark-munkamenetet kapja.

### [Get-AzSynapseSparkStatement](Get-AzSynapseSparkStatement.md)
Beolvassa a szinapszis-elemző szikra nyilatkozatát.

### [Get-AzSynapseSqlDatabase](Get-AzSynapseSqlDatabase.md)
Ez a funkció egy korlátozott előzetes verzió, amely kezdetben csak bizonyos előfizetésekhez érhető el. A szinapszis Analytics SQL-adatbázis beolvasása

### [Get-AzSynapseSqlPool](Get-AzSynapseSqlPool.md)
A szinapszis Analytics SQL-készletet kapja.

### [Get-AzSynapseTrigger](Get-AzSynapseTrigger.md)
Információt kap a munkaterületen lévő eseményindítókkal kapcsolatban.

### [Get-AzSynapseTriggerRun](Get-AzSynapseTriggerRun.md)
Információt ad az indításról.

### [Get-AzSynapseTriggerSubscriptionStatus](Get-AzSynapseTriggerSubscriptionStatus.md)
Az esemény eseményindítójának állapotát a megadott külső szolgáltatási eseményekre vonatkozóan szerezheti be.

### [Get-AzSynapseWorkspace](Get-AzSynapseWorkspace.md)
A szinapszis Analytics-munkaterületet kapja.

### [Meghívó-AzSynapseIntegrationRuntimeUpgrade](Invoke-AzSynapseIntegrationRuntimeUpgrade.md)
Önkiszolgáló integrációs futtatókörnyezet frissítése

### [Meghívó-AzSynapsePipeline](Invoke-AzSynapsePipeline.md)
Arra hívja fel a futószalagot, hogy indítsa el a futtatását.

### [Meghívó-AzSynapseSparkStatement](Invoke-AzSynapseSparkStatement.md)
A szinapszis-elemző szikra utasítását hívja fel.

### [Új – AzSynapseFirewallRule](New-AzSynapseFirewallRule.md)
Létrehozza a szinapszis-elemzési tűzfalszabályok szabályát.

### [Új – AzSynapseIntegrationRuntimeKey](New-AzSynapseIntegrationRuntimeKey.md)
Önkiszolgáló integrációs futásidejű kulcs újbóli létrehozása

### [Új – AzSynapseRoleAssignment](New-AzSynapseRoleAssignment.md)
Egy szinapszis-Analytics szerepkör-hozzárendelést hoz létre.

### [Új – AzSynapseSparkPool](New-AzSynapseSparkPool.md)
Létrehoz egy szinapszis-elemzési szikra medencét.

### [Új – AzSynapseSqlDatabase](New-AzSynapseSqlDatabase.md)
Ez a funkció egy korlátozott előzetes verzió, amely kezdetben csak bizonyos előfizetésekhez érhető el. A szinapszis Analytics SQL-adatbázis létrehozása.

### [Új – AzSynapseSqlPool](New-AzSynapseSqlPool.md)
A szinapszis Analytics SQL-készlet létrehozása.

### [Új – AzSynapseWorkspace](New-AzSynapseWorkspace.md)
A szinapszis Analytics-munkaterületet hozza létre.

### [Remove-AzSynapseDataFlow](Remove-AzSynapseDataFlow.md)
Adatforgalom eltávolítása a munkaterületről.

### [Remove-AzSynapseDataset](Remove-AzSynapseDataset.md)
Adatkészlet eltávolítása a munkaterületről.

### [Remove-AzSynapseFirewallRule](Remove-AzSynapseFirewallRule.md)
Törli a szinapszis-elemzési tűzfalszabályok szabályát.

### [Remove-AzSynapseIntegrationRuntime](Remove-AzSynapseIntegrationRuntime.md)
Az integrációs futtatókörnyezet eltávolítása.

### [Remove-AzSynapseIntegrationRuntimeNode](Remove-AzSynapseIntegrationRuntimeNode.md)
A megadott nevű csomópont eltávolítása az integrációs futtatókörnyezetben.

### [Remove-AzSynapseLinkedService](Remove-AzSynapseLinkedService.md)
Egy kapcsolt szolgáltatás eltávolítása a munkaterületről.

### [Remove-AzSynapseNotebook](Remove-AzSynapseNotebook.md)
Jegyzetfüzet eltávolítása munkaterületről.

### [Remove-AzSynapsePipeline](Remove-AzSynapsePipeline.md)
Csővezeték eltávolítása a munkaterületről.

### [Remove-AzSynapseRoleAssignment](Remove-AzSynapseRoleAssignment.md)
Egy szinapszis-Analytics szerepkör-hozzárendelés törlése

### [Remove-AzSynapseSparkPool](Remove-AzSynapseSparkPool.md)
Törli a szinapszis-elemző Spark-medencét.

### [Remove-AzSynapseSqlDatabase](Remove-AzSynapseSqlDatabase.md)
Ez a funkció egy korlátozott előzetes verzió, amely kezdetben csak bizonyos előfizetésekhez érhető el. Egy szinapszis-Analytics SQL-adatbázis törlése

### [Remove-AzSynapseSqlPool](Remove-AzSynapseSqlPool.md)
Egy szinapszis-Analytics SQL-készlet törlése

### [Remove-AzSynapseTrigger](Remove-AzSynapseTrigger.md)
Eltávolítja az indítót egy munkaterületről.

### [Remove-AzSynapseTriggerSubscription](Remove-AzSynapseTriggerSubscription.md)
Lemondhatja az eseményindítót külső szolgáltatási eseményekre.

### [Remove-AzSynapseWorkspace](Remove-AzSynapseWorkspace.md)
A szinapszis Analytics-munkaterületének törlése.

### [Reset-AzSynapseSparkSessionTimeout](Reset-AzSynapseSparkSessionTimeout.md)
A szinapszis-elemző Spark-munkamenet időtúllépésének visszaállítása

### [Set-AzSynapseIntegrationRuntime](Set-AzSynapseIntegrationRuntime.md)
Az integrációs futtatókörnyezet frissítése

### [Set-AzSynapseDataFlow](Set-AzSynapseDataFlow.md)
Adatfolyamot hoz létre vagy frissít a munkaterületen.

### [Set-AzSynapseDataset](Set-AzSynapseDataset.md)
Adatkészletet hoz létre vagy frissít a munkaterületen.

### [Set-AzSynapseLinkedService](Set-AzSynapseLinkedService.md)
Az adattárolók vagy a Felhőbeli szolgáltatások összekapcsolása a munkaterületre

### [Set-AzSynapseNotebook](Set-AzSynapseNotebook.md)
Jegyzetfüzetet hoz létre vagy frissít a munkaterületen.

### [Set-AzSynapsePipeline](Set-AzSynapsePipeline.md)
Futószalagot hoz létre a munkaterületen.

### [Set-AzSynapseTrigger](Set-AzSynapseTrigger.md)
Létrehoz egy triggert egy munkaterületen.

### [Start-AzSynapseSparkSession](Start-AzSynapseSparkSession.md)
Elindítja a szinapszis-elemző szikra-munkamenetet.

### [Start-AzSynapseTrigger](Start-AzSynapseTrigger.md)
Elindítja az indítót egy munkaterületen.

### [Stop-AzSynapsePipelineRun](Stop-AzSynapsePipelineRun.md)
Egy munkaterületen futó csővezeték leállítása

### [Stop-AzSynapseSparkJob](Stop-AzSynapseSparkJob.md)
A szinapszis-elemző szikra munkahelyének lemondása

### [Stop-AzSynapseSparkSession](Stop-AzSynapseSparkSession.md)
A szinapszis-elemző szikra munkamenet leállítása.

### [Stop-AzSynapseSparkStatement](Stop-AzSynapseSparkStatement.md)
A szinapszis-elemző szikra nyilatkozatának lemondása

### [Stop-AzSynapseTrigger](Stop-AzSynapseTrigger.md)
A munkaterületen leállít egy triggert.

### [Submit-AzSynapseSparkJob](Submit-AzSynapseSparkJob.md)
Elküldhet egy szinapszis-elemző szikra munkát.

### [Szinkronizálás – AzSynapseIntegrationRuntimeCredential](Sync-AzSynapseIntegrationRuntimeCredential.md)
Szinkronizálja a hitelesítő adatokat az integrációs futásidejű csomópontok között.

### [Teszt-AzSynapseSparkPool](Test-AzSynapseSparkPool.md)
Ellenőrzi, hogy létezik-e egy szinapszis-elemző szikra medence.

### [Teszt-AzSynapseSqlDatabase](Test-AzSynapseSqlDatabase.md)
Ez a funkció egy korlátozott előzetes verzió, amely kezdetben csak bizonyos előfizetésekhez érhető el. A szinapszis-Analytics SQL-adatbázis meglétének ellenőrzése.

### [Teszt-AzSynapseSqlPool](Test-AzSynapseSqlPool.md)
A szinapszis-Analytics SQL-készlet létezésének ellenőrzése.

### [Teszt-AzSynapseWorkspace](Test-AzSynapseWorkspace.md)
A szinapszis Analytics-munkaterület létezésének ellenőrzése.

### [Update-AzSynapseFirewallRule](Update-AzSynapseFirewallRule.md)
A szinapszis Analytics-tűzfal szabályának frissítése

### [Update-AzSynapseIntegrationRuntime](Update-AzSynapseIntegrationRuntime.md)
Az integrációs futtatókörnyezet frissítése

### [Update-AzSynapseIntegrationRuntimeNode](Update-AzSynapseIntegrationRuntimeNode.md)
Frissítheti az önkiszolgáló integrációs futásidejű csomópontot.

### [Update-AzSynapseSparkPool](Update-AzSynapseSparkPool.md)
Frissíti a szinapszis-elemző Spark-medencét.

### [Update-AzSynapseSqlDatabase](Update-AzSynapseSqlDatabase.md)
Ez a funkció egy korlátozott előzetes verzió, amely kezdetben csak bizonyos előfizetésekhez érhető el. A szinapszis Analytics SQL-adatbázis frissítése

### [Update-AzSynapseSqlPool](Update-AzSynapseSqlPool.md)
A szinapszis Analytics SQL-készlet frissítése

### [Update-AzSynapseWorkspace](Update-AzSynapseWorkspace.md)
A szinapszis Analytics-munkaterületének frissítése

### [Várjon-AzSynapseSparkJob](Wait-AzSynapseSparkJob.md)
Várakozás a szinapszis-statisztika kitöltésére.

