...
### 1. példa: az App conifguration-tárolójának adattitkosításának engedélyezése rendszerszintű felügyelt identitással
```powershell
...
Ez a parancs az Azure Key Vault-ban tárolt kulccsal titkosítja az adattitkosítást egy rendszer által hozzárendelt felügyelt identitással.
A boltozatnak engedélyezve kell lennie a Soft-delete és a Purge-védelem beállításnak, és a felügyelt identitásnak meg kell felelnie a következő fontos engedélyeknek: Get, wrapKey, unwrapKey.

### 2. példa: az App conifguration-tárolójának adattitkosításának engedélyezése felhasználó által hozzárendelt felügyelt identitással
```powershell
PS C:\> $key = Add-AzKeyVaultKey -VaultName kv-Name -Name key-Name -Destination 'Software'
...
Ezzel a paranccsal az Azure Key Vault-ban tárolt kulccsal titkosíthatja az adattitkosítást egy felhasználó által hozzárendelt felügyelt identitás használatával.
A felhasználó által hozzárendelt identitást be kell állítani `-UserAssignedIdentity` .
A boltozatnak engedélyezve kell lennie a Soft-delete és a Purge-védelem beállításnak, és a felügyelt identitásnak meg kell felelnie a következő fontos engedélyeknek: Get, wrapKey, unwrapKey.

### 3. példa: az App conifguration-tárolójának titkosításának letiltása.
```powershell
...
Ez a parancs letiltja az App conifguration-tárolójának titkosítását.

### 3. példa: az conifguration-tároló (SKU) és a címkéje a csővezeték alapján.
```powershell
...
Ez a parancs a conifguration-tárolóban lévő app-tárolók SKU-és címkéjét frissíti.
...
Az adattitkosításhoz használt kulcskezelő kulcs URI-azonosítója.
...
Annak az identitásnak az ügyfél azonosítója, amelyet a kulcsfájl eléréséhez használni fog.
