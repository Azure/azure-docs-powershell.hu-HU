...
### Példa 1: a Databricks-munkaterület címkéit frissíti
```powershell
...
Ez a parancs frissíti a Databricks-munkaterület címkéit.

### 2. példa: a titkosítás engedélyezése Databricks-munkaterületeken
```powershell
...
A titkosítás engedélyezésének Databricks-munkaterülete három lépésből áll:
1.
Frissítse a munkaterületet `-PrepareEncryption` (ha még nem hozta létre).
1.
Keresse meg az `StorageAccountIdentityPrincipalId` utolsó lépés kimenetét.
Adjon meg kulcs-engedélyeket a megbízónak.
1.
A titkosítási kulccsal kapcsolatos információk kitöltéséhez frissítse újra a munkaterületet:
    - `-EncryptionKeySource`
    - `-EncryptionKeyVaultUri`
    - `-EncryptionKeyName`
    - `-EncryptionKeyVersion`

### 3. példa: a titkosítás letiltása Databricks-munkaterületen
```powershell
...
A titkosítás letiltásához egyszerűen állítsa be `-EncryptionKeySource` `'Default'` .
...
### -EncryptionKeySource
A titkosítási forrás (szolgáltató).
Lehetséges értékek (kis-és nagybetűk megkülönböztetése): alapértelmezett, Microsoft.-boltozat
...
### -PrepareEncryption
A munkaterület előkészítése titkosításhoz.
Engedélyezi a felügyelt tárterület-fiók felügyelt identitását.
...
### Microsoft. Azure. PowerShell. parancsmagok. Databricks. models. IDatabricksIdentity

## KIMENETEK

### Microsoft. Azure. PowerShell. parancsmagok. Databricks. modellek. Api20180401. IWorkspace
...
INPUTOBJECT <IDatabricksIdentity> : Identity paraméter
  ...
  - `[PeeringName <String>]`: A munkaterület vNete.
