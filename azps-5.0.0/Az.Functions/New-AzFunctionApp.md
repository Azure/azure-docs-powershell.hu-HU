...
### 1. példa: a használati PowerShell-függvény létrehozása a közép-Amerikai Egyesült Államokban.
```powershell
...
Ez a parancs a közép-amerikai fogyasztási PowerShell-függvény alkalmazást hoz létre.

### 2. példa: hozzon létre egy PowerShell-függvényt, amelyet egy szolgáltatási tervben fog tárolni.
```powershell
PS C:\> New-AzFunctionApp -Name MyUniqueFunctionAppName `
                          -ResourceGroupName MyResourceGroupName `
                          -PlanName MyPlanName `
...
Ez a parancs létrehoz egy PowerShell-függvény alkalmazást, amely egy szolgáltatási csomag keretében fog működni.

### 3. példa: a Function app létrehozása privát ACR-kép használatával.
```powershell
PS C:\> New-AzFunctionApp -Name MyUniqueFunctionAppName `
                          -ResourceGroupName MyResourceGroupName `
                          -PlanName MyPlanName `
...
Ez a parancs létrehoz egy függvény alkalmazást egy privát ACR-kép használatával.

## PARAMÉTEREK

### -ApplicationInsightsKey
A felvenni kívánt alkalmazás Instrumentation-kulcsa.
...
A függvény alkalmazásba felvenni kívánt meglévő alkalmazás-betekintési projekt neve.
...
Tiltsa le az alkalmazás-elemzések erőforrás-készítését a függvény app létrehozása során.
Nem lesz elérhető naplózás.
...
### -DockerImageName
Csak Linux.
Tároló képnév a dokkolóegység adatbázisából (például Publisher/Image-Name: címke).
...
A tároló beállításjegyzék-felhasználónevét és jelszavát.
A privát kibocsátásiegység-forgalmi jegyzékekhez szükséges.
...
### -IdentityID
A függvény alkalmazáshoz társított felhasználói azonosítók listáját adja meg.
...
### -IdentityType
A függvény alkalmazáshoz használt identitás típusát adja meg.
A paraméter elfogadható értékei a következők:-SystemAssigned-UserAssigned
...
### -Várva
Elindítja a műveletet, és a művelet befejezése előtt azonnal visszaküldi a műveletet.
Ha meg szeretné állapítani, hogy sikerült-e végrehajtani a műveletet, használjon más mechanizmust.
...
### -OSType
Az operációs rendszer a függvény app üzemeltetéséhez.
