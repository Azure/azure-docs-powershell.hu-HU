...
### Példa 1: a Function app beszerzése név szerint, majd újraindítása.
```powershell
PS C:\> Get-AzFunctionApp -Name MyAppName -ResourceGroupName MyResourceGroupName | Restart-AzFunctionApp -Force
```

Ez a parancs név szerint beilleszti a függvény alkalmazást, és újra elindítja.

### 2. példa: a függvény alkalmazás újraindítása név szerint.
```powershell
PS C:\> Restart-AzFunctionApp -Name MyAppName -ResourceGroupName MyResourceGroupName -Force
```

Ez a parancs a függvény alkalmazását egy névvel indítja el.
