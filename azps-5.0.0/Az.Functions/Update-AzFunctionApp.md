...
### Példa 1: Update függvény app-üzemeltetési terv.
```powershell
PS C:\> Update-AzFunctionApp -Name MyUniqueFunctionAppName -ResourceGroupName MyResourceGroupName -PlanName NewPlanName 
```

Ez a parancs frissíti a Function app üzemeltetési csomagját.

### 2. példa: SystemAssigned felügyelt identitás beállítása egy függvény alkalmazásban.
```powershell
PS C:\> Update-AzFunctionApp -Name MyUniqueFunctionAppName -ResourceGroupName MyResourceGroupName -IdentityType SystemAssigned 
```

Ez a parancs a SystemAssigned felügyelt identitását állítja be.

### 3. példa: a Update függvény alkalmazás-betekintési funkciói.
```powershell
...
Ez a parancs frissíti a Function App alkalmazásban betekintést.

### 3. példa: a felügyelt identitás eltávolítása egy függvény alkalmazásból.
```powershell
PS C:\> Update-AzFunctionApp -Name MyUniqueFunctionAppName -ResourceGroupName MyResourceGroupName -IdentityType None 
```

Ez a parancs egy függvény alkalmazásból távolítja el a felügyelt identitást.
...
A "nincs" típus esetén a függvény alkalmazásban minden identitás törlődik.
A paraméter elfogadható értékei a következők:-SystemAssigned-UserAssigned-nincs
