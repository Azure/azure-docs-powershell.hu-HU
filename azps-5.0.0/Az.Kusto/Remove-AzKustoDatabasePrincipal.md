...
### RemoveExpanded (alapértelmezett)
```
...
### 1. példa: az adatbázis-résztvevők engedélyeinek eltávolítása
```powershell
...
A fenti parancs eltávolítja az adatbázis-résztvevők engedélyeit
...
### -Value (érték)
A Kusto-adatbázis-résztvevők listája.
A kivitelezéshez lásd: az érték tulajdonságainak FELJEGYZÉSek szakasza, és hozzon létre egy ujjlenyomat-táblázatot.
...
VALUE <IDatabasePrincipal [] >: a Kusto adatbázis-résztvevők listája.
  - `Name <String>`: Adatbázis-egyszerű név.
  - `Role <DatabasePrincipalRole>`: Adatbázis-résztvevők szerepköre
  - `Type <DatabasePrincipalType>`: Adatbázis-résztvevő típusa.
  - `[AppId <String>]`: Alkalmazásspecifikus azonosító – csak az alkalmazásobjektum-típus esetén érvényes.
  - `[Email <String>]`: Ha létezik, az adatbázis-résztvevők e-mail-címe
  - `[Fqn <String>]`: Adatbázis-résztvevő: teljes mértékben minősített név.
