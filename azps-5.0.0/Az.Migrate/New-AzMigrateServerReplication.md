...
## Áttekintés
A megadott kiszolgáló replikációjának indítása.
...
### Példa 1: Ha csak az operációs rendszer lemeze van
```powershell
...
Ez a helyzet akkor jelenik meg, ha csak egyetlen, védett lemezről van-e szükség.
...
Ez a helyzet akkor jelenik meg, ha több olyan lemez van, amelyet védeni kell.
...
### -DiskToInclude
Itt adhatja meg, hogy a forráskiszolgálón mely lemezek kerüljenek a replikációba.
A kivitelezéshez tekintse meg a DISKTOINCLUDE tulajdonságainak megjegyzések szakaszát, és hozzon létre egy ujjlenyomat-táblázatot.
...
### -InputObject
Annak a felismert kiszolgálónak a meghatározása, amelyet át szeretne telepíteni.
A kiszolgálói objektum az Get-AzMigrateServer parancsmag használatával is beolvasható.
...
### -LicenseType
Annak megadása, hogy az Azure hibrid előny a forráskiszolgáló áttelepítéséhez szükséges-e.
...
### -MachineId
Annak a felismert kiszolgálónak a gépi AZONOSÍTÓját adja meg, amelyet át szeretne telepíteni.
...
### -OSDiskID
Megadja az áttelepítendő forráskiszolgáló operációs rendszerének lemezét.
...
### -PerformAutoResync
Azt adja meg, hogy az automatikus javítás történik-e, ha a változtatások nyomon követése elvész a forráskiszolgálón a replikáció csoportban.
...
Megadja a VM-creationSpecifies használandó elérhetőségi beállításokat a VM-készítéshez használt elérhetőségi érték esetén.
...
Itt adhatja meg a boot Diagnostics rendszerhez használandó tárolási fiókot.
...
### -TargetNetworkId
A cél Azure-előfizetéshez tartozó virtuális hálózati azonosítót adja meg, amelyre a kiszolgálót át kell telepíteni.
...
A cél Azure-előfizetéshez tartozó erőforráscsoport azonosítóját adja meg, amelyre a kiszolgálót át kell telepíteni.
...
### -TargetSubnetName
Annak a virtuális netowk a neve, amelyhez a kiszolgálót át kell telepíteni.
...
### -TargetVMName
A létrehozandó Azure VM-VM nevet adja meg.
...
### -TargetVMSize
A létrehozandó Azure VM-SKU-t adja meg.
...
DISKTOINCLUDE <IVMwareCbtDiskInput [] >: Itt adhatja meg, hogy a forráskiszolgálón milyen lemezek szerepeljenek a replikációban.
  - `DiskId <String>`: A lemezvezérlő-azonosító.
  - `IsOSDisk <String>`: Érték, amely azt jelzi, hogy a lemez az operációs rendszer lemeze-e.
  - `LogStorageAccountId <String>`: A log Storage Account ARM azonosítója.
  - `LogStorageAccountSasSecretName <String>`: A napló tárterületének titkos neve.
  - `[DiskEncryptionSetId <String>]`: A DiskEncryptionSet-kar azonosítója.
  - `[DiskType <DiskAccountType?>]`: A lemez típusa.

INPUTOBJECT <IVMwareMachine> : az áttelepítendő kiszolgálót adja meg. A kiszolgálói objektum az Get-AzMigrateServer parancsmag használatával is beolvasható.
