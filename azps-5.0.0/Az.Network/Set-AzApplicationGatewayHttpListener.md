...
## Leírás
A **set-AzApplicationGatewayHttpListener** parancsmag módosította az Azure Application ÁTJÁRÓk http-figyelőjét.

## Példák

### Példa 1: HTTP-figyelő beállítása
```
...
A második parancs az átjáróhoz tartozó HTTP-figyelőt állítja be a 80 $FIP-on tárolt, az előtér-konfigurációban lévő HTTP-protokollon keresztül.

### 2. példa: HTTPS-figyelő felvétele SSL-és állomásnevek használatával
```
PS C:\>$AppGw = Get-AzApplicationGateway -Name "ApplicationGateway01" -ResourceGroupName "ResourceGroup01"
...
A második parancs hozzáadja a figyelőt, amely a HTTPS protokollt használja az SSL-tanúsítványokkal és-állomásnevek használatával az Application Gateway-nek.

## PARAMÉTEREK

### -ApplicationGateway
Annak az alkalmazásnak az átjáróját adja meg, amelyhez ez a parancsmag társítja a HTTP-figyelőt.
...
Az alkalmazás-átjáró előtér IP-címét adja meg.
...
Az alkalmazás-átjáró előtér-IP-címének AZONOSÍTÓját adja meg.
...
### -FrontendPort
Az Application Gateway előtér-portot adja meg.
...
### -HostName (állomásnév)
Azt az állomásnevet adja meg, amelyre a parancsmag a HTTP-figyelőt elküldi.
...
Azt adja meg, hogy a parancsmagnak meg kell-e adni a kiszolgálónév jelzését.
A paraméter elfogadható értékei a következők: igaz vagy hamis.
...
### -SslCertificateId
Megadja a HTTP-figyelő Secure Socket Layer (SSL) tanúsítvány-AZONOSÍTÓját.
