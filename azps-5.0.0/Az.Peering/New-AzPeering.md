...
Új, közvetlen társközi kapcsolat létrehozása a Seattle-i létesítményben a PeerAsn 65000 használatával

### Új Exchange-alapú peer létrehozása
```powershell
#Gets the ASN
PS C:> $asn = Get-AzPeerAsn -PeerName Contoso
...
### Korábbi társközi nézet konvertálása az ARM-partnerek számára
```powershell
#Gets the ASN
PS C:> $asn = Get-AzPeerAsn -PeerName Contoso
...
### -DirectConnection
Új közvetlen kapcsolatok létrehozása a New-AzExchangePeeringConnection és a pipe parancs segítségével.
...
### -ExchangeConnection
Új Exchange-kapcsolatok létrehozása a New-AzExchangePeeringConnection és a pipe segítségével a parancs segítségével.
...
### -InputObject
Az objektum beolvasásához használja a Get-AzLegacyPeering.
...
### -MicrosoftNetwork
Jelölje ki a használni kívánt Microsoft-hálózatot.
...
### -PeerAsnResourceId
A társközi ASN-erőforrás azonosítója. az azonosító lekéréséhez használja a Get-AzPeerAsn.
...
### -PeeringLocation
Az Azure-területtől eltérő fizikai hely.
Használja a Get-AzPeeringLocation-Kind ( \<kind\> város neve) nevet kulcsként.
...
### -SKU
Válassza a Basic_Direct_Free vagy a Premium_Direct_Free lehetőséget, ha kifejezetten azt szeretné, hogy válasszon másik lehetőséget.
...
### -Címke
A Microsoft társközi szolgáltatásához társítani kívánt címkék.
