...
## Leírás
Új sablont hoz létre a megadott ARM-tartalommal. A tartalom nyers JSON-karakterláncból ( **FromJsonStringParameterSet** -set) vagy egy megadott JSON-fájlból (a **FromJsonFileParameterSet** paraméterrel van beállítva) származhat.  

Ha a root sablon specifikációja még nem létezik, akkor az a sablon spec verzióval együtt jön létre. Ha egy olyan sablon létezik, amely már létezik a megadott névvel, akkor a megadott verzió frissül (a többi meglévő verzió is megmarad).
...
A "v 1.0" nevű új sablon létrehozása egy "myTemplateSpec" nevű sablonban. A megadott verzió a verzió ARM-sablonjának tartalmát fogja $templateJson.

 **Megjegyzés:** A példában a kar sablon nem-op, mivel nem tartalmaz tényleges erőforrásokat.
...
Egy "v 2.0" nevű új sablont hoz létre a "myTemplateSpec" nevű sablonban. A megadott verzió a verzió ARM-sablonjának tartalmát tartalmazó helyi myTemplateContent.jsfájlból származó tartalmat fogja tartalmazni.
...
### -Force
Ha egy meglévő verziót felülír, ne kérjen megerősítést.
...
### -Hely
A sablon specifikációjának helye. Csak akkor kötelező, ha a specifikációban még nem szerepel a sablon.
...
### -TemplateFile
A helyi Azure Resource Manager template JSON fájl elérési útja.
...
### -TemplateJson
Az Azure Resource Manager sablon JSON-je.
