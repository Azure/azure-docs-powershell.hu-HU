---
Module Name: Az.Storage
Module Guid: da67eaa7-4cb1-4bfa-a194-8bf3faae8ac5
Download Help Link: https://docs.microsoft.com/en-us/powershell/module/az.storage
Help Version: 4.2.3.0
Locale: en-US
content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Storage/Storage.Management/help/Az.Storage.md
original_content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Storage/Storage.Management/help/Az.Storage.md
ms.openlocfilehash: 1808aa26760f962affe3b1cc45647c33aa10c15e
ms.sourcegitcommit: b4a38bcb0501a9016a4998efd377aa75d3ef9ce8
ms.translationtype: MT
ms.contentlocale: hu-HU
ms.lasthandoff: 10/27/2020
ms.locfileid: "94194179"
---
# Az. Storage modul
## Leírás
Ez a témakör az Azure Storage-parancsmagok súgótémaköröket jeleníti meg.

## Az. Storage parancsmagok
### [Add-AzRmStorageContainerLegalHold](Add-AzRmStorageContainerLegalHold.md)
Jogi visszatartási címke hozzáadása a tároló blob-tárolóhoz

### [Add-AzStorageAccountManagementPolicyAction](Add-AzStorageAccountManagementPolicyAction.md)
Felvesz egy tevékenységet a bemeneti ManagementPolicy, vagy létrehoz egy ManagementPolicy-objektumot a művelettel. Az objektum használható a New-AzStorageAccountManagementPolicyRule.

### [Add-AzStorageAccountNetworkRule](Add-AzStorageAccountNetworkRule.md)
 IpRules vagy VirtualNetworkRules hozzáadása a NetworkRule tulajdonságához

### [Close-AzStorageFileHandle](Close-AzStorageFileHandle.md)
Bezárja a fájlmegosztás, a fájl vagy a fájl fájljait.

### [Disable-AzStorageBlobDeleteRetentionPolicy](Disable-AzStorageBlobDeleteRetentionPolicy.md)
Tiltsa le a törlés adatmegőrzési házirendet az Azure Storage blob szolgáltatásban.

### [Disable-AzStorageBlobRestorePolicy](Disable-AzStorageBlobRestorePolicy.md)
A blob-visszaállítási házirend letiltása a tárterület-fiókban.

### [Disable-AzStorageDeleteRetentionPolicy](Disable-AzStorageDeleteRetentionPolicy.md)
Tiltsa le a törlés adatmegőrzési házirendet az Azure Storage blob szolgáltatásban.

### [Disable-AzStorageStaticWebsite](Disable-AzStorageStaticWebsite.md)
Tiltsa le az Azure Storage-fiók statikus webhelyét.

### [Enable-AzStorageBlobDeleteRetentionPolicy](Enable-AzStorageBlobDeleteRetentionPolicy.md)
Engedélyezze az Azure Storage blob szolgáltatás adatmegőrzési házirendjének törlését.

### [Enable-AzStorageBlobRestorePolicy](Enable-AzStorageBlobRestorePolicy.md)
Lehetővé teszi a blob-visszaállítási házirendet egy tárterület-fiókban.

### [Enable-AzStorageDeleteRetentionPolicy](Enable-AzStorageDeleteRetentionPolicy.md)
Engedélyezze az Azure Storage blob szolgáltatás adatmegőrzési házirendjének törlését.

### [Enable-AzStorageStaticWebsite](Enable-AzStorageStaticWebsite.md)
Engedélyezze az Azure Storage-fiók statikus webhelyét.

### [Get-AzDataLakeGen2ChildItem](Get-AzDataLakeGen2ChildItem.md)
A címtárból vagy a fájlrendszer gyökeréből származó alkönyvtárak és fájlok listázása.

### [Get-AzDataLakeGen2Item](Get-AzDataLakeGen2Item.md)
Egy fájlrendszerben lévő fájl vagy könyvtár részleteit kapja meg.

### [Get-AzDataLakeGen2ItemContent](Get-AzDataLakeGen2ItemContent.md)
Fájl letöltése

### [Get-AzRmStorageContainer](Get-AzRmStorageContainer.md)
Tároló blob-tárolók beolvasása vagy listázása

### [Get-AzRmStorageContainerImmutabilityPolicy](Get-AzRmStorageContainerImmutabilityPolicy.md)
ImmutabilityPolicy kapja a tároló blob-tárolókban

### [Get-AzRmStorageShare](Get-AzRmStorageShare.md)
Tárterület-megosztást kap vagy listáz.

### [Get-AzStorageAccount](Get-AzStorageAccount.md)
Megkapja a tárolási fiókot.

### [Get-AzStorageAccountKey](Get-AzStorageAccountKey.md)
Egy Azure-tárterülethez tartozó hozzáférési kulcsokat kapja meg.

### [Get-AzStorageAccountManagementPolicy](Get-AzStorageAccountManagementPolicy.md)
Az Azure Storage-fiók kezelési házirendjét kapja meg.

### [Get-AzStorageAccountNameAvailability](Get-AzStorageAccountNameAvailability.md)
Ellenőrzi a tárolási fiók nevének elérhetőségét.

### [Get-AzStorageAccountNetworkRuleSet](Get-AzStorageAccountNetworkRuleSet.md)
Tárolási fiók NetWorkRule tulajdonságának beszerzése

### [Get-AzStorageBlob](Get-AzStorageBlob.md)
A tárolóban lévő Blobs listák.

### [Get-AzStorageBlobContent](Get-AzStorageBlobContent.md)
Tárhely blobjának letöltése.

### [Get-AzStorageBlobCopyState](Get-AzStorageBlobCopyState.md)
Az Azure Storage blob másolati állapotát kapja meg.

### [Get-AzStorageBlobQueryResult](Get-AzStorageBlobQueryResult.md)
Egy egyszerű strukturált lekérdezési nyelv (SQL) utasítását alkalmazza egy blob-tartalomra, és csak az adatok lekérdezett részhalmazát menti egy helyi fájlba.

### [Get-AzStorageBlobServiceProperty](Get-AzStorageBlobServiceProperty.md)
Az Azure Storage blob Services szolgáltatás tulajdonságainak beolvasása.

### [Get-AzStorageContainer](Get-AzStorageContainer.md)
Felsorolja a tároló tárolókat.

### [Get-AzStorageContainerStoredAccessPolicy](Get-AzStorageContainerStoredAccessPolicy.md)
Beolvassa a tárolt hozzáférés házirendjének vagy házirendjeinek egy Azure Storage tárolót.

### [Get-AzStorageCORSRule](Get-AzStorageCORSRule.md)
Beolvassa a tárolási CORS vonatkozó szabályokat.

### [Get-AzStorageFile](Get-AzStorageFile.md)
Az elérési utakhoz tartozó könyvtárak és fájlok felsorolása.

### [Get-AzStorageFileContent](Get-AzStorageFileContent.md)
Letölti a fájl tartalmát.

### [Get-AzStorageFileCopyState](Get-AzStorageFileCopyState.md)
A másolási művelet állapotát kapja meg.

### [Get-AzStorageFileHandle](Get-AzStorageFileHandle.md)
A fájlmegosztás, a fájl vagy a fájl fájljainak a fájljait jeleníti meg.

### [Get-AzStorageFileServiceProperty](Get-AzStorageFileServiceProperty.md)
Az Azure Storage file Services szolgáltatás tulajdonságainak beolvasása

### [Get-AzStorageObjectReplicationPolicy](Get-AzStorageObjectReplicationPolicy.md)
A tárolási fiók objektum-replikációs házirendjének beolvasása vagy listázása.

### [Get-AzStorageQueue](Get-AzStorageQueue.md)
A tárolási várólisták listája.

### [Get-AzStorageQueueStoredAccessPolicy](Get-AzStorageQueueStoredAccessPolicy.md)
Beolvassa a tárolt hozzáférési házirendet vagy házirendet egy Azure-tárterület várólistájához.

### [Get-AzStorageServiceLoggingProperty](Get-AzStorageServiceLoggingProperty.md)
Az Azure Storage Services naplózási tulajdonságait kapja.

### [Get-AzStorageServiceMetricsProperty](Get-AzStorageServiceMetricsProperty.md)
Az Azure Storage szolgáltatás metrikájának tulajdonságai

### [Get-AzStorageServiceProperty](Get-AzStorageServiceProperty.md)
Az Azure Storage Services tulajdonságainak beolvasása

### [Get-AzStorageShare](Get-AzStorageShare.md)
Megkapja a fájlmegosztás listáját.

### [Get-AzStorageShareStoredAccessPolicy](Get-AzStorageShareStoredAccessPolicy.md)
Tárolhatja a tárolt elérési házirendeket egy tárterület-megosztáshoz.

### [Get-AzStorageTable](Get-AzStorageTable.md)
A tárolási táblák listája.

### [Get-AzStorageTableStoredAccessPolicy](Get-AzStorageTableStoredAccessPolicy.md)
Beolvassa a tárolt hozzáférési házirendet vagy házirendet egy Azure Storage table.

### [Get-AzStorageUsage](Get-AzStorageUsage.md)
A jelenlegi előfizetés tárolási erőforrás-felhasználását kapja meg.

### [Meghívó-AzStorageAccountFailover](Invoke-AzStorageAccountFailover.md)
Meghívja a tárolási fiók feladatátvételét.

### [Lock-AzRmStorageContainerImmutabilityPolicy](Lock-AzRmStorageContainerImmutabilityPolicy.md)
Tároló blob-tárolók ImmutabilityPolicy zárolása

### [Move-AzDataLakeGen2Item](Move-AzDataLakeGen2Item.md)
Fájlok vagy könyvtárak áthelyezése egy másik fájlba vagy könyvtárba ugyanabba a tárterület-fiókba.

### [Új – AzDataLakeGen2Item](New-AzDataLakeGen2Item.md)
Fájl vagy könyvtár létrehozása a fájlrendszerben.

### [Új – AzRmStorageContainer](New-AzRmStorageContainer.md)
Tároló blob-tároló létrehozása

### [Új – AzRmStorageShare](New-AzRmStorageShare.md)
Létrehoz egy tárterület-megosztást.

### [Új – AzStorageAccount](New-AzStorageAccount.md)
Tároló fiókot hoz létre.

### [Új – AzStorageAccountKey](New-AzStorageAccountKey.md)
Egy Azure-tárterülethez tartozó tárterület kulcsának újragenerálása.

### [Új – AzStorageAccountManagementPolicyFilter](New-AzStorageAccountManagementPolicyFilter.md)
Létrehoz egy ManagementPolicy-szabályt, amely a New-AzStorageAccountManagementPolicyRule is használható.

### [Új – AzStorageAccountManagementPolicyRule](New-AzStorageAccountManagementPolicyRule.md)
Létrehoz egy ManagementPolicy-szabályt, amelyet a set-AzStorageAccountManagementPolicy használhat.

### [Új – AzStorageAccountSASToken](New-AzStorageAccountSASToken.md)
Ügyfél szintű SAS-tokent hoz létre.

### [Új – AzStorageBlobQueryConfig](New-AzStorageBlobQueryConfig.md)
BLOB-lekérdezési konfigurációs objektumot hoz létre, amelyet a Get-AzStorageBlobQueryResult használhat.

### [Új – AzStorageBlobRangeToRestore](New-AzStorageBlobRangeToRestore.md)
BLOB-tartománnyal létrehozott objektumot hoz létre a tárolási fiók visszaállításához.

### [Új – AzStorageBlobSASToken](New-AzStorageBlobSASToken.md)
Egy, az Azure Storage blob-hoz generált SAS-tokent hoz létre.

### [Új – AzStorageContainer](New-AzStorageContainer.md)
Azure-tároló tárolót hoz létre.

### [Új – AzStorageContainerSASToken](New-AzStorageContainerSASToken.md)
Létrehoz egy SAS-jogkivonatot az Azure Storage tárolóhoz.

### [Új – AzStorageContainerStoredAccessPolicy](New-AzStorageContainerStoredAccessPolicy.md)
Tárol egy tárolt hozzáférési házirendet egy Azure tároló tárolóhoz.

### [Új – AzStorageContext](New-AzStorageContext.md)
Azure-tárterületet hoz létre.

### [Új – AzStorageDirectory](New-AzStorageDirectory.md)
Könyvtárat hoz létre.

### [Új – AzStorageFileSASToken](New-AzStorageFileSASToken.md)
Megosztott hozzáférés-aláírási tokent hoz létre egy tárterülethez.

### [Új – AzStorageObjectReplicationPolicyRule](New-AzStorageObjectReplicationPolicyRule.md)
Objektum-replikációs házirend-szabályt hoz létre.

### [Új – AzStorageQueue](New-AzStorageQueue.md)
Tárolási várólistát hoz létre.

### [Új – AzStorageQueueSASToken](New-AzStorageQueueSASToken.md)
Egy Access-aláírási tokent hoz létre egy Azure-tárterület várólistájához.

### [Új – AzStorageQueueStoredAccessPolicy](New-AzStorageQueueStoredAccessPolicy.md)
Tárolt hozzáférés-házirendet hoz létre az Azure tárterület-várólistához.

### [Új – AzStorageShare](New-AzStorageShare.md)
Létrehozza a fájl megosztását.

### [Új – AzStorageShareSASToken](New-AzStorageShareSASToken.md)
Megosztott hozzáférés-aláírási jogkivonat létrehozása az Azure tárhely-megosztásához.

### [Új – AzStorageShareStoredAccessPolicy](New-AzStorageShareStoredAccessPolicy.md)
Tárolt hozzáférési házirendet hoz létre egy tárterület-megosztásban.

### [Új – AzStorageTable](New-AzStorageTable.md)
Tároló táblát hoz létre.

### [Új – AzStorageTableSASToken](New-AzStorageTableSASToken.md)
Létrehoz egy SAS-jogkivonatot az Azure Storage-táblájához.

### [Új – AzStorageTableStoredAccessPolicy](New-AzStorageTableStoredAccessPolicy.md)
Egy, az Azure Storage-táblájához tárolt hozzáférési házirendet hoz létre.

### [Remove-AzDataLakeGen2AclRecursive](Remove-AzDataLakeGen2AclRecursive.md)
A megadott elérési úton rekurzív módon távolítsa el az ACL-t. 

### [Remove-AzDataLakeGen2Item](Remove-AzDataLakeGen2Item.md)
Fájlok vagy könyvtárak eltávolítása.

### [Remove-AzRmStorageContainer](Remove-AzRmStorageContainer.md)
Tároló blob-tároló eltávolítása

### [Remove-AzRmStorageContainerImmutabilityPolicy](Remove-AzRmStorageContainerImmutabilityPolicy.md)
ImmutabilityPolicy-tároló blob-tárolójának eltávolítása a zárolt házirenddel

### [Remove-AzRmStorageContainerLegalHold](Remove-AzRmStorageContainerLegalHold.md)
A jogi mentességgel rendelkező címkék eltávolítása a tárterület-tárolóból

### [Remove-AzRmStorageShare](Remove-AzRmStorageShare.md)
Eltávolít egy tárterület-megosztást.

### [Remove-AzStorageAccount](Remove-AzStorageAccount.md)
Eltávolít egy tárterület-fiókot az Azure-ról.

### [Remove-AzStorageAccountManagementPolicy](Remove-AzStorageAccountManagementPolicy.md)
Egy Azure-tárterületet tároló fiók kezelési házirendjét távolítja el.

### [Remove-AzStorageAccountNetworkRule](Remove-AzStorageAccountNetworkRule.md)
IpRules vagy VirtualNetworkRules eltávolítása a NetWorkRule tulajdonságból

### [Remove-AzStorageBlob](Remove-AzStorageBlob.md)
Eltávolítja a megadott tárterület-blobot.

### [Remove-AzStorageContainer](Remove-AzStorageContainer.md)
Eltávolítja a megadott tárterület-tárolót.

### [Remove-AzStorageContainerStoredAccessPolicy](Remove-AzStorageContainerStoredAccessPolicy.md)
Egy tárolt hozzáférés-házirend eltávolítása az Azure Storage tárolóból.

### [Remove-AzStorageCORSRule](Remove-AzStorageCORSRule.md)
Eltávolítja a CORS a tárterület-szolgáltatáshoz.

### [Remove-AzStorageDirectory](Remove-AzStorageDirectory.md)
Címtár törlése

### [Remove-AzStorageFile](Remove-AzStorageFile.md)
Töröl egy fájlt.

### [Remove-AzStorageObjectReplicationPolicy](Remove-AzStorageObjectReplicationPolicy.md)
Eltávolítja a meghatározott objektum replikációs házirendjét egy tároló fiókból.

### [Remove-AzStorageQueue](Remove-AzStorageQueue.md)
Tároló-várólista eltávolítása.

### [Remove-AzStorageQueueStoredAccessPolicy](Remove-AzStorageQueueStoredAccessPolicy.md)
Egy tárolt Access-házirend eltávolítása egy Azure-tárterület-várólistából.

### [Remove-AzStorageShare](Remove-AzStorageShare.md)
Fájl megosztásának törlése

### [Remove-AzStorageShareStoredAccessPolicy](Remove-AzStorageShareStoredAccessPolicy.md)
Egy tárolt Access-házirend eltávolítása a tárterület-megosztásból.

### [Remove-AzStorageTable](Remove-AzStorageTable.md)
Tároló tábla eltávolítása.

### [Remove-AzStorageTableStoredAccessPolicy](Remove-AzStorageTableStoredAccessPolicy.md)
A tárolt Access-házirendek eltávolítása az Azure Storage-táblájából.

### [Restore-AzRmStorageShare](Restore-AzRmStorageShare.md)
Visszaállítja a törölt fájlok megosztását.

### [Restore-AzStorageBlobRange](Restore-AzStorageBlobRange.md)
Bizonyos blob-tartományokhoz tartozó tárolási fiók visszaállítása.

### [Visszavonás-AzStorageAccountUserDelegationKeys](Revoke-AzStorageAccountUserDelegationKeys.md)
Visszavonhatja a tárterület-fiók összes felhasználói meghatalmazási kulcsát.

### [Set-AzCurrentStorageAccount](Set-AzCurrentStorageAccount.md)
Módosítja a megadott előfizetés jelenlegi tárterület-fiókját.

### [Set-AzDataLakeGen2AclRecursive](Set-AzDataLakeGen2AclRecursive.md)
A megadott elérési úton állítsa be az ACL-t rekurzív módon. 

### [Set-AzDataLakeGen2ItemAclObject](Set-AzDataLakeGen2ItemAclObject.md)
Létrehozza/frissíti a DataLake Gen2-objektum ACL-objektumát, amelyet az Update-AzDataLakeGen2Item parancsmagban használhat.

### [Set-AzRmStorageContainerImmutabilityPolicy](Set-AzRmStorageContainerImmutabilityPolicy.md)
ImmutabilityPolicy-tárolók létrehozása vagy frissítése

### [Set-AzStorageAccount](Set-AzStorageAccount.md)
Módosítja a tárolási fiókot.

### [Set-AzStorageAccountManagementPolicy](Set-AzStorageAccountManagementPolicy.md)
Azure-tárterületet tároló fiók kezelési házirendjének létrehozása vagy módosítása.

### [Set-AzStorageBlobContent](Set-AzStorageBlobContent.md)
Egy helyi fájlt tölt fel egy Azure Storage blob-webhelyre.

### [Set-AzStorageContainerAcl](Set-AzStorageContainerAcl.md)
A nyilvános hozzáférés engedélyének megadása tároló tárolóhoz.

### [Set-AzStorageContainerStoredAccessPolicy](Set-AzStorageContainerStoredAccessPolicy.md)
Egy, az Azure-tároló tárolójának tárolt hozzáférési házirendjét állítja be.

### [Set-AzStorageCORSRule](Set-AzStorageCORSRule.md)
A CORS szabályait állítja be.

### [Set-AzStorageFileContent](Set-AzStorageFileContent.md)
Feltölti a fájl tartalmát.

### [Set-AzStorageObjectReplicationPolicy](Set-AzStorageObjectReplicationPolicy.md)
A megadott objektum replikációs házirendjének létrehozása vagy frissítése egy tárterület-fiókban.

### [Set-AzStorageQueueStoredAccessPolicy](Set-AzStorageQueueStoredAccessPolicy.md)
Tárol egy tárolt hozzáférés-házirendet egy Azure Storage Queue-hoz.

### [Set-AzStorageServiceLoggingProperty](Set-AzStorageServiceLoggingProperty.md)
Az Azure Storage Services naplózásának módosítása.

### [Set-AzStorageServiceMetricsProperty](Set-AzStorageServiceMetricsProperty.md)
Módosítja az Azure Storage szolgáltatás metrikák tulajdonságát.

### [Set-AzStorageShareQuota](Set-AzStorageShareQuota.md)
A megosztás tárterület-kapacitását adja meg.

### [Set-AzStorageShareStoredAccessPolicy](Set-AzStorageShareStoredAccessPolicy.md)
A tárolt hozzáférési házirendet frissíti egy tárterület-megosztáson.

### [Set-AzStorageTableStoredAccessPolicy](Set-AzStorageTableStoredAccessPolicy.md)
Egy Azure Storage Table tárolt hozzáférés házirendjének beállítása.

### [Start-AzStorageBlobCopy](Start-AzStorageBlobCopy.md)
Elindítja a blob másolását.

### [Start-AzStorageBlobIncrementalCopy](Start-AzStorageBlobIncrementalCopy.md)
Egy növekményes másolási művelet elindítása egy blob-pillanatképből a megadott rendeltetési lapra blob-ra.

### [Start-AzStorageFileCopy](Start-AzStorageFileCopy.md)
Elindítja a forrásfájl másolását.

### [Stop-AzStorageBlobCopy](Stop-AzStorageBlobCopy.md)
Leállít egy másolási műveletet.

### [Stop-AzStorageFileCopy](Stop-AzStorageFileCopy.md)
Leállít egy másolási műveletet a megadott célfájlba.

### [Update-AzDataLakeGen2AclRecursive](Update-AzDataLakeGen2AclRecursive.md)
A megadott elérési úton rekurzív módon frissítse az ACL-t. 

### [Update-AzDataLakeGen2Item](Update-AzDataLakeGen2Item.md)
Frissítsen egy fájlt vagy könyvtárat a tulajdonságok, a metaadatok, az engedély, az ACL és a tulajdonos lapon.

### [Update-AzRmStorageContainer](Update-AzRmStorageContainer.md)
Tárterület-tároló blob-tárolójának módosítása

### [Update-AzRmStorageShare](Update-AzRmStorageShare.md)
A tárterület-fájlmegosztás módosítása.

### [Update-AzStorageAccountNetworkRuleSet](Update-AzStorageAccountNetworkRuleSet.md)
Tárolási fiók NetworkRule tulajdonságának frissítése

### [Update-AzStorageBlobServiceProperty](Update-AzStorageBlobServiceProperty.md)
Módosítja az Azure Storage blob szolgáltatás szolgáltatás tulajdonságát.

### [Update-AzStorageFileServiceProperty](Update-AzStorageFileServiceProperty.md)
Módosítja az Azure Storage file szolgáltatás tulajdonságát.

### [Update-AzStorageServiceProperty](Update-AzStorageServiceProperty.md)
Módosítja az Azure Storage szolgáltatás tulajdonságait.

