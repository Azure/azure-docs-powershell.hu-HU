...
A **New-AzApiManagementApiRevision** parancsmag létrehoz egy API-változatot egy meglévő API-hoz az API-kezelés környezetében.

## PÉLDÁK

### 1. példa: Üres API-változat létrehozása egy API-hoz
```powershell
...
Ez a parancs létrehozza az API `2` `echo-api` API-változatát.

### 2. példa: API-változat létrehozása egy meglévő Api-ból, és az összes művelet, címke és házirend másolása
```powershell
...
Ez a parancs létrehozza az API `5` `echo-api` API-változatát.

## PARAMETERS

### -ApiId
Annak az API-nak az azonosítója, amelynek a korrektúraszámát létre kell hozatni.
...
### -ApiRevisionDescription
Az API változatának leírása. Ez a paraméter nem kötelező.
...
### -ServiceUrl
A webszolgáltatás URL-címe, amely felfedi az API-t a Háttérszolgáltatásban. Ezt az URL-címet csak az Azure API Management fogja használni, és nem teszi nyilvánosra. 1–2000 karakter hosszúnak kell lennie. Ezt a paramétert kötelező megadni.
...
### -SourceApiRevision
A forrás API Api-korrektúraazonosítója. Ez a paraméter nem kötelező.
