...
## LEÍRÁS
A **Remove-AzApiManagementPolicy** parancsmag eltávolítja az API-kezelési házirendet a megadott hatókörből.
...
Ez a parancs eltávolítja a bérlői szintű házirendet az API-kezelésből.

### 2. példa: A termékkörre vonatkozó házirend eltávolítása
```
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
PS C:\>Remove-AzApiManagementPolicy -Context $apimContext -ProductId "0123456789"
```

Ez a parancs eltávolítja a termékhatókör-házirendet az API-kezelésből.

### 3. példa: Az API-hatókör házirend eltávolítása
```
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
PS C:\>Remove-AzApiManagementPolicy -Context $apimContext -ApiId "9876543210"
```

Ez a parancs eltávolítja az API-hatókör házirendet az API-kezelésből.

### 4. példa: A művelet hatókör-házirend eltávolítása
```
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
PS C:\>Remove-AzApiManagementPolicy -Context $apimContext -ApiId "9876543210" -OperationId "777"
```

Ez a parancs eltávolítja a művelet-hatókör házirendet az API-kezelésből.

## PARAMETERS

### -ApiId
Egy meglévő API azonosítóját adja meg.
Ha ezt a paramétert adja meg, a parancsmag eltávolítja az API-hatókör házirendet.
...
Ha ezt a paramétert az *ApiId paraméterrel* adja meg, ez a parancsmag eltávolítja a művelet-hatókör házirendet.
...
Ha ezt a paramétert adja meg, a parancsmag eltávolítja a termék-hatókör házirendet.
