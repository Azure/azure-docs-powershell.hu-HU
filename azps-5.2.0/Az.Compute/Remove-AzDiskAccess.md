...
## LEÍRÁS
A **Remove-AzDiskAccess** parancsmag eltávolítja a lemezelérési erőforrást.

## PÉLDÁK

### 1. példa: Lemezelérés eltávolítása az alapértelmezett paraméterkészlettel
```
PS C:\> Remove-AzDiskAccess -ResourceGroupName "ResourceGroup01" -Name "DiskAccess01"
```

Ez a parancs eltávolítja a "DiskAccess01" lemezelérést az "ResourceGroup01" erőforráscsoportban

### 2. példa: Lemezelérés eltávolítása erőforrás-azonosító használatával
```
...
Ez a parancs eltávolítja a lemezelérést az Erőforrás-azonosító szerint

### 3. példa: Lemezelérés eltávolítása bemeneti objektum használatával
```
PS C:\> $myDiskAccess = Get-AzDiskAccess -ResourceGroupName "ResourceGroup01" -Name "DiskAccess01"
PS C:\> Remove-AzDiskAccess -InputObject $myDiskAccess
```

Ezzel a paranccsal az InputObject eltávolítja a lemezelérést

### 4. példa: Lemezelérés eltávolítása a bemeneti objektummal
```
PS C:\> Get-AzDiskAccess -ResourceGroupName "ResourceGroup01" -Name "DiskAccess01" | Remove-AzDiskAccess 
```

Ez a parancs eltávolítja a lemezelérést az InputObject
...
### -InputObject
PowerShell Lemezelérési objektum
