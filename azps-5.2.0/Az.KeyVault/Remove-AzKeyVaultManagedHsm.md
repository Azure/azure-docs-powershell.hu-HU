...
### RemoveManagedHsmByName (alapértelmezett)
```
...
## LEÍRÁS
Az **Remove-AzKeyVaultManagedHsm** parancsmag törli a megadott felügyelt HSM-et.
Ezenkívül törli az ebben a példányban található összes billentyűt is.
Vegye figyelembe, hogy bár az erőforráscsoport megadása nem kötelező ennél a parancsmagnál, a jobb teljesítmény érdekében érdemes így lennie.

## PÉLDÁK

### 1. példa: Felügyelt HSM eltávolítása
```powershell
...
Ez a parancs eltávolítja a myhsm nevű felügyelt hsm-et az aktuális előfizetéséből.

### 2. példa: Felügyelt hsm eltávolítása egy adott erőforráscsoportból
```powershell
...
Ez a parancs eltávolítja a myhsm nevű felügyelt hsm-et a myrg1 nevű erőforráscsoportból.
Ha nem adja meg az erőforráscsoport nevét, a parancsmag megkeresi az aktuális előfizetésben törölni kívánt felügyelt HSM-et.
...
Ez a parancsmag alapértelmezés szerint kérni fogja, hogy erősítse meg a felügyelt HSM törlését.
...
### -Name
Az eltávolítható felügyelt HSM neve.
...
### -ResourceGroupName
Az Azure által felügyelt HSM-hez eltávolítható erőforráscsoport nevét adja meg.
