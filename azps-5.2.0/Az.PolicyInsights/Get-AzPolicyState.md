...
## LEÍRÁS
Házirend-megfelelőségi államokat kap az erőforrásokhoz. A házirendállapot-rekordok lekérdezhetőek különböző hatókörökben. A megadott időintervallum alapján (alapértelmezés szerint az utolsó nap) lekérdezhető a legújabb házirendállapotok vagy az összes házirendállapot-áttűnés. Az eredmények szűrhetők, csoportosíthatóak és csoportos összesítések is kiszámíthatók.

## PÉLDÁK

### 1. példa: A legújabb házirend-állapotok lekérte a jelenlegi előfizetési hatókört
```powershell
...
### 13. példa: Házirend-hozzárendelés legutóbbi házirend-kiosztásának lekérte a megadott előfizetésben
```powershell
...
### 15. példa: A legújabb házirend-állapotok lekérte az aktuális előfizetés hatókörét az OrderBy, a Top és a Select lekérdezési beállításokkal
```powershell
...
Az aktuális munkamenet kontextusában az előfizetés összes erőforrására vonatkozóan az utolsó napon létrehozott legújabb házirendállapot-rekordokat kapja le. A parancs korlátozza a megfelelőségi állapot alapján való szűrés által visszaadott eredményeket (csak a nem megfelelő állapotot tartalmazza).
Az eredményeket az erőforrás-azonosító alapján csoportosulja. Ez az előfizetés azon erőforrásainak listáját generálja, amelyek legalább egy házirendnek nem megfelelőek.
