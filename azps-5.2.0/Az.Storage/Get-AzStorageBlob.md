...
## LEÍRÁS
A **Get-AzStorageBlob** parancsmag egy Azure-tárfiók adott tárolóján lévő blobokat sorolja fel.

## PÉLDÁK

### 1. példa: Blob lekérte blobnév szerint
```
PS C:\>Get-AzStorageBlob -Container "ContainerName" -Blob blob*
```

Ez a parancs blobnevet és helyettesítő karaktereket használ a blob lekérthez.

### 2. példa: Blobok lekérte egy tárolóban a folyamat használatával
```
...
Ez a parancs a folyamat segítségével behoz minden blobot (a törölt állapot blobját is beleértve) egy tárolóba.

### 3. példa: Blobok lekért névelőtagja
```
PS C:\>Get-AzStorageBlob -Container "ContainerName" -Prefix "blob"
```

Ez a parancs egy névelőtagot használ a blobok leküldésekor.

### 4. példa: Több köteg blobos listájának létrehozása
```
...
Ebben a példában *a MaxCount* és *a ContinuationToken paramétert* használva több kötegben listába soroljuk az Azure Storage blobokat.
Az első négy parancs értékeket rendel a példában használt változókhoz.
Az ötödik parancs egy **Olyan Do-While utasítást** ad meg, amely a **Get-AzStorageBlob** parancsmagot használja a blobok be szereznie.
Az utasítás tartalmazza a folytatási jogkivonatot, amely a $Token változóban található.
$Token a hurok futásakor megváltozik az érték.
További információért írja be a `Get-Help About_Do` .
Az utolsó parancs a **Visszhang paranccsal** jeleníti meg az összeget.

### 5. példa: Egy tároló összes blobját le kell szereznie, amely tartalmazza a blobverziót
```
...
Ez a parancs a tárolók összes blobját tartalmazza, a blobverziót is.

### 6. példa: Egyetlen blobverzió lekérte
```
...
Ez a parancs egyetlen blob veriont kap a VersionId segítségével.

### 7. példa: Egyetlen blob-pillanatfelvétel készítése
```
...
Ez a parancs egyetlen blob-pillanatképet kap a SnapshotTime-val.

## PARAMETERS

### -Blob
Egy nevet vagy egy névmintát ad meg, amely használható helyettesítő karakteres kereséshez.
Ha nincs megadva blobnév, a parancsmag felsorolja a megadott tároló összes blobját.
Ha a paraméterhez meg van adva egy érték, a parancsmag felsorolja az összes olyan blobot, amely megfelel ennek a paraméternek. Ez a paraméter a karakterlánc bármely helyében támogatja a helyettesítő karaktereket.
...
### -Környezet
Megadja azt az Azure-tárfiókot, amelyből le szeretné kapni a blobok listáját.
A New-AzStorageContext környezet létrehozásához használhatja a parancsmagot.
...
Ezt a paramétert és *a MaxCount paramétert* használva több kötegben is listába sorolhatja a blobokat.
...
### -IncludeDeleted
A Törölt blobok alapértelmezés szerint nem tartalmaznak törölt blobot.
...
### -IncludeVersion
A blobverziók csak akkor lesznek felsorolva, ha ez a paraméter jelen van, alapértelmezés szerint a blob lekért verziója nem fog blobverziókat tartalmazni.
...
### -Előtag
A lekért blobnevek előtagját adja meg.
Ez a paraméter nem támogatja reguláris kifejezések vagy helyettesítő karakterek keresését.
Ez azt jelenti, hogy ha a tárolóban csak a "Saját", a "MyBlob1" és a "MyBlob2" nevű blobok vannak, és a "-Előtag my*" értéket adja meg, a parancsmag nem ad vissza blobokat.
Ha azonban a "-Prefix My" értéket adja meg, a parancsmag a "Saját", a "MyBlob1" és a "MyBlob2" értéket adja vissza.
