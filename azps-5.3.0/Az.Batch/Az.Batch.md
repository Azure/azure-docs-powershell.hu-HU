---
Module Name: Az.Batch
Module Guid: a8f00f40-1c1a-49b5-9db3-24076b75c3cf
Download Help Link: https://docs.microsoft.com/en-us/powershell/module/az.batch
Help Version: 4.0.0.0
Locale: en-US
content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Batch/Batch/help/Az.Batch.md
original_content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Batch/Batch/help/Az.Batch.md
ms.openlocfilehash: f6bd55502c5c800bff245b905bc5334b1bf35bdb
ms.sourcegitcommit: 68451baa389791703e666d95469602c5652609ee
ms.translationtype: MT
ms.contentlocale: hu-HU
ms.lasthandoff: 01/05/2021
ms.locfileid: "98478429"
---
# Az.Batmodul
## Leírás
Az Azure-modul Azure Batch-parancsmagja lehetővé teszi a Microsoft Azure Batch-szolgáltatások kezelését az Azure PowerShellben.

## Az.Batch parancsmagok
### [Disable-AzBatchAutoScale](Disable-AzBatchAutoScale.md)
Letiltja a készlet automatikus méretezését.

### [Disable-AzBatchComputeNodeScheduling](Disable-AzBatchComputeNodeScheduling.md)
Letiltja a tevékenységütemezést a megadott számítási csomóponton.

### [Disable-AzBatchUpp](Disable-AzBatchJob.md)
Letilt egy köteg-feladatot.

### [Disable-AzBatchSchedule](Disable-AzBatchJobSchedule.md)
Letiltja a köteg feladatütemtervét.

### [Enable-AzBatchAutoScale](Enable-AzBatchAutoScale.md)
Engedélyezi a készlet automatikus méretezését.

### [Enable-AzBatchComputeNodeScheduling](Enable-AzBatchComputeNodeScheduling.md)
Engedélyezi a tevékenységek ütemezését a megadott számítási csomóponton.

### [Enable-AzBatchUpp](Enable-AzBatchJob.md)
Engedélyezi a köteget.

### [Enable-AzBatchSchedule](Enable-AzBatchJobSchedule.md)
Engedélyezi a köteg feladatütemtervét.

### [Enable-AzBatchTask](Enable-AzBatchTask.md)
Tevékenység újraaktiválása.

### [Get-AzBatchAccount](Get-AzBatchAccount.md)
Kötegfiókot kap az aktuális előfizetésben.

### [Get-AzBatchAccountKey](Get-AzBatchAccountKey.md)
Egy Batch-fiók kulcsait kapja meg.

### [Get-AzBatchApplication](Get-AzBatchApplication.md)
Információt kap a megadott alkalmazásról.

### [Get-AzBatchApplicationPackage](Get-AzBatchApplicationPackage.md)
Információkat kap egy Batch-fiókban található alkalmazáscsomagról.

### [Get-AzBatchCertificate](Get-AzBatchCertificate.md)
Egy Batch-fiók tanúsítványait kapja meg.

### [Get-AzBatchComputeNode](Get-AzBatchComputeNode.md)
Köteg számítási csomópontokat kap egy készletből.

### [Get-AzBatchUpp](Get-AzBatchJob.md)
Batch-fiókhoz vagy feladatütemtervhez kap kötegelt feladatokat.

### [Get-AzBatchPreparationAndReleaseTaskStatus](Get-AzBatchJobPreparationAndReleaseTaskStatus.md)
Kötegfeladat-előkészítést és feladatállapotot ad vissza.

### [Get-AzBatchSchedule](Get-AzBatchJobSchedule.md)
Kötegelt feladatütemterveket kap.

### [Get-AzBatchStatistic](Get-AzBatchJobStatistic.md)
Egy Batch-fiókhoz kap egy feladat-összefoglaló statisztikát.

### [Get-AzBatchLocationQuota](Get-AzBatchLocationQuota.md)
Az adott helyen megkapja az előfizetés kötegszolgáltatásra vonatkozó kvótáját.

### [Get-AzBatchNodeFile](Get-AzBatchNodeFile.md)
A Batch-csomópontfájlok tulajdonságait kapja meg.

### [Get-AzBatchNodeFileContent](Get-AzBatchNodeFileContent.md)
Egy Batch-csomópontfájlt kap.

### [Get-AzBatchPool](Get-AzBatchPool.md)
Kötegkészleteket kap a megadott Batch-fiók alatt.

### [Get-AzBatchPoolNodeCount](Get-AzBatchPoolNodeCount.md)
A Köteg csomópontok száma csomópontonként, készletazonosító szerint csoportosítva.

### [Get-AzBatchPoolStatistic](Get-AzBatchPoolStatistic.md)
Készlet-összegző statisztikákat kap egy Batch-fiókhoz.

### [Get-AzBatchPoolUsageMetric](Get-AzBatchPoolUsageMetric.md)
Készlethasználati metrikákat kap egy Batch-fiókhoz.

### [Get-AzBatchRemoteDesktopProtocolFile](Get-AzBatchRemoteDesktopProtocolFile.md)
RdP-fájlt kap egy számítási csomópontból.

### [Get-AzBatchRemoteLoginSetting](Get-AzBatchRemoteLoginSetting.md)
Távoli bejelentkezési beállításokat kap egy számítási csomóponthoz.

### [Get-AzBatchSubtask](Get-AzBatchSubtask.md)
A megadott tevékenység altevékenység-információinak lekérte.

### [Get-AzBatchSupportedImage](Get-AzBatchSupportedImage.md)
Batch-fiók támogatott képeit kapja.

### [Get-AzBatchTask](Get-AzBatchTask.md)
Egy feladat kötegfeladatát kapja meg.

### [Get-AzBatchTaskCount](Get-AzBatchTaskCount.md)
A megadott feladathoz a tevékenységek számát adja meg.

### [New-AzBatchAccount](New-AzBatchAccount.md)
Létrehoz egy Batch-fiókot.

### [New-AzBatchAccountKey](New-AzBatchAccountKey.md)
A Batch-fiók kulcsának újragenerálása.

### [New-AzBatchApplication](New-AzBatchApplication.md)
Hozzáad egy alkalmazást a megadott Batch-fiókhoz.

### [New-AzBatchApplicationPackage](New-AzBatchApplicationPackage.md)
Alkalmazáscsomagot hoz létre egy Batch-fiókban.

### [New-AzBatchCertificate](New-AzBatchCertificate.md)
Hozzáad egy tanúsítványt a megadott Batch-fiókhoz.

### [New-AzBatchComputeNodeUser](New-AzBatchComputeNodeUser.md)
Létrehoz egy felhasználói fiókot egy batch számítási csomóponton.

### [New-AzBatchUpp](New-AzBatchJob.md)
Létrehozza a feladatot a Batch szolgáltatásban.

### [New-AzBatchSchedule](New-AzBatchJobSchedule.md)
Létrehozza a munkabeosztást a Batch szolgáltatásban.

### [New-AzBatchPool](New-AzBatchPool.md)
Létrehoz egy készletet a Batch szolgáltatásban.

### [New-AzBatchResourceFile](New-AzBatchResourceFile.md)
Létrehozza az erőforrásfájlt `New-AzBatchTask` használatra.

### [New-AzBatchTask](New-AzBatchTask.md)
Létrehoz egy Batch-feladatot egy feladat alatt.

### [Remove-AzBatchAccount](Remove-AzBatchAccount.md)
Eltávolít egy Batch-fiókot.

### [Remove-AzBatchApplication](Remove-AzBatchApplication.md)
Egy alkalmazást töröl egy Batch-fiókból.

### [Remove-AzBatchApplicationPackage](Remove-AzBatchApplicationPackage.md)
Egy alkalmazáscsomagrekord és a bináris fájl törlése.

### [Remove-AzBatchCertificate](Remove-AzBatchCertificate.md)
Törli a tanúsítványt egy fiókból.

### [Remove-AzBatchComputeNode](Remove-AzBatchComputeNode.md)
Eltávolítja a számítási csomópontokat egy készletből.

### [Remove-AzBatchComputeNodeUser](Remove-AzBatchComputeNodeUser.md)
Felhasználói fiókot töröl egy batch számítási csomópontból.

### [Remove-AzBatchJob](Remove-AzBatchJob.md)
Egy kötegfájl-feladatot töröl.

### [Remove-AzBatchSchedule](Remove-AzBatchJobSchedule.md)
Eltávolítja a köteg feladatütemtervét.

### [Remove-AzBatchNodeFile](Remove-AzBatchNodeFile.md)
Egy feladat vagy számítási csomópont csomópontfájljának törlése.

### [Remove-AzBatchPool](Remove-AzBatchPool.md)
Törli a megadott kötegkészletet.

### [Remove-AzBatchTask](Remove-AzBatchTask.md)
Egy kötegfeladat törlése.

### [Reset-AzBatchComputeNode](Reset-AzBatchComputeNode.md)
Újratelepíti az operációs rendszert a megadott számítási csomóponton.

### [Restart-AzBatchComputeNode](Restart-AzBatchComputeNode.md)
A megadott számítási csomópont újraindítása.

### [Set-AzBatchAccount](Set-AzBatchAccount.md)
Kötegfiók frissítése.

### [Set-AzBatchApplication](Set-AzBatchApplication.md)
A megadott alkalmazás beállításainak frissítése.

### [Set-AzBatchComputeNodeUser](Set-AzBatchComputeNodeUser.md)
Módosítja egy fiók tulajdonságait egy batch számítási csomóponton.

### [Set-AzBatchUpp](Set-AzBatchJob.md)
Egy köteg-feladat frissítése.

### [Set-AzBatchSchedule](Set-AzBatchJobSchedule.md)
Munkabeosztást állít be.

### [Set-AzBatchPool](Set-AzBatchPool.md)
Frissíti egy készlet tulajdonságait.

### [Set-AzBatchTask](Set-AzBatchTask.md)
Frissíti egy feladat tulajdonságait.

### [Start-AzBatchComputeNodeServiceLogUpload](Start-AzBatchComputeNodeServiceLogUpload.md)
Töltse fel a számítási csomópontokat tartalmazó naplófájlokat egy Azure Storage-tárolóba.

### [Start-AzBatchPoolResize](Start-AzBatchPoolResize.md)
Kezd átméretezni egy készletet.

### [Stop-AzBatchCertificateDeletion](Stop-AzBatchCertificateDeletion.md)
Visszavonja egy tanúsítvány sikertelen törlését.

### [Stop-AzBatchJob](Stop-AzBatchJob.md)
Egy köteg-feladat leállítja.

### [Stop-AzBatchSchedule](Stop-AzBatchJobSchedule.md)
Leállítja a köteg feladatütemtervét.

### [Stop-AzBatchPoolResize](Stop-AzBatchPoolResize.md)
A készlet átméretezésének leállítja a műveletet.

### [Stop-AzBatchTask](Stop-AzBatchTask.md)
Egy kötegfeladat leállítja.

### [Test-AzBatchAutoScale](Test-AzBatchAutoScale.md)
Egy automatikus méretezési képlet eredményét kapja egy készleten.

