...
## LEÍRÁS
A **Disable-AzFrontDoorCustomDomainHttps letiltja a HTTPS-t** egy egyéni tartományhoz.

## PÉLDÁK

### 1. példa: A HTTPS letiltása a FrontDoorName és a ResourceGroupName egyéni tartomány esetén.
```powershell
...
Tiltsa le a HTTPS-t a "frontendpointname1-custom-xyz" egyéni tartományhoz úgy, hogy a FrontDoorName "frontdoor1" és az ResourceGroupName "resourcegroup1" (erőforráscsoport1) nevet használja.

### 2. példa: A HTTPS letiltása egy PSFrontendEndpoint-objektummal létrehozott egyéni tartományhoz.
```powershell
...
A PSFrontendEndpoint-objektum egyéni tartomány esetén tiltsa le a HTTPS-t.

### 3. példa: A HTTPS letiltása egyéni tartomány esetén az ResourceId beállítással.
```powershell
...
Tiltsa le a HTTPS-t egy egyéni tartomány "frontendpointname1-custom-xyz" tartományában, amely a ResourceId mint $resourceId.
...
### -ResourceId
A https letiltására a Front Door végpont erőforrás-azonosítója
