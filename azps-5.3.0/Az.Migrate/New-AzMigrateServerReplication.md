...
## SYNOPSIS
Replikáció indítása a megadott kiszolgálóhoz.

## SZINTAXIS

### ByIdDefaultUser (alapértelmezett)
```
...
### 1. példa: Ha csak az operációs rendszer lemeze van
```powershell
...
Ez olyan esetben van így, amikor csak egyetlen lemez van védve.

### 2. példa: Több lemez esetén
```powershell
...
Ez akkor van így, ha több lemez van, amelyek védettek.
...
### -DiskToInclude
Megadja, hogy a forráskiszolgáló mely lemezeket tartalmazza a replikációhoz.
A felépítésről a DISKTOINCLUDE tulajdonságokat és a kivonattáblát a NOTES (MEGJEGYZÉSEK) című szakaszban láthatja.
...
### -InputObject
Az áttelepíteni talált kiszolgálót határozza meg.
A kiszolgálóobjektum a Get-AzMigrateServer le.
...
### -LicenseType
Azt adja meg, hogy az Azure Hibrid előny alkalmazható-e a forráskiszolgáló áttelepítésére.
...
### -MachineId
Az áttelepíteni talált kiszolgáló számítógépazonosítóját adja meg.
...
### -OSDiskID
Annak az operációs rendszernek a lemeze, amelybe a forráskiszolgálót át kell migrálni.
...
Megadja a VM-létrehozáshoz használható rendelkezésre állási halmazT, amely meghatározza a VM-létrehozáshoz használható rendelkezésre állási készletet.
...
### -TargetNetworkId
Megadja a virtuális hálózat azonosítóját azon a cél Azure-előfizetésen belül, amelybe a kiszolgálót át kell migrálni.
...
Annak az Erőforráscsoport-azonosítónak a megadása azon a cél Azure-előfizetésen belül, amelybe a kiszolgálót át kell migrálni.
...
### -TargetSubnetName
Megadja annak az alhálózatnak a nevét a virtual netowk célhelyen belül, amelybe a kiszolgálót át kell migrálni.
...
### -TargetVMName
A létrehozható Azure VM nevét adja meg.
...
### -TargetVMSize
Megadja a létrehozható Azure VM termékváltozatát.
...
DISKTOINCLUDE <IVMwareCbtDiskInput[]>: Megadja a forráskiszolgálón a replikációhoz szükséges lemezeket.
  - `DiskId <String>`: A lemezazonosító.
  - `IsOSDisk <String>`: Egy érték, amely azt jelzi, hogy a lemez az operációs rendszer lemeze-e.
  - `LogStorageAccountId <String>`: A naplótároló fiók ARM azonosító.
  - `LogStorageAccountSasSecretName <String>`: A naplótároló fiók kulcstárának titkos neve.
  - `[DiskEncryptionSetId <String>]`: The DiskEncryptionSet ARM Id.
  - `[DiskType <DiskAccountType?>]`: A lemez típusa.

INPUTOBJECT: Az áttelepíteni <IVMwareMachine> szükséges felderített kiszolgáló. A kiszolgálóobjektum a Get-AzMigrateServer le.
  - `[GuestOSDetailOstype <String>]`: Az operációs rendszer típusa.
