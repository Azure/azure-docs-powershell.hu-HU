...
## LEÍRÁS
Az a `Reservation` érték `Reservation` felosztása két s-re a megadott mennyiségeloszlással.
...
A megadott érték felosztása `Reservation` `Reservation` két s-re a megfelelő mennyiségekkel
...
### -Mennyiség
A két szám mennyiségmezője vesszővel elválasztott `Reservation` egész szám.
...
### -ReservationId
A `Reservation` felosztani következő azonosítója
...
### -ReservationOrderId
Annak az `ReservationOrder` azonosítónak az azonosítója, amely a `Reservation` felosztani kívánt felhasználót tartalmazza
