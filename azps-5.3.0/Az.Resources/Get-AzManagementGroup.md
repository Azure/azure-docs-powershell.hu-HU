...
## LEÍRÁS
A Get-AzManagementGroup parancsmag az összeset vagy egy adott felügyeleti csoportot kap.

## PÉLDÁK

### 1. példa: Az összes felügyeleti csoport lekérte
```
...
### 2. példa: Adott felügyeleti csoport lekérte
```
...
### 3. példa: Adott felügyeleti csoport és a hierarchia első szintje
```
...
A jelölővel navigálhat a tömbben, és `Expand` `Children` lekérte az egyes gyermekek adatait. Megadhatja például a megjelenítendő nevet tartalmazó `Children[0]` csoport `TestGroup1DisplayName` adatait.

### 4. példa: Adott felügyeleti csoport és a hierarchia minden szintje
```
...
### -Kibontás
Bontsa ki a kimenetet a felügyeleti csoport gyermekeinek listájához
