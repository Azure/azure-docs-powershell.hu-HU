---
Module Name: Az.Sql
Module Guid: 150d9544-6348-4373-806f-10cd0b4de4cb
Download Help Link: https://docs.microsoft.com/en-us/powershell/module/az.sql
Help Version: 4.4.0.0
Locale: en-US
content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Sql/Sql/help/Az.Sql.md
original_content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Sql/Sql/help/Az.Sql.md
ms.openlocfilehash: 8d4e270ea1be514581f596376f31c23244993341
ms.sourcegitcommit: 68451baa389791703e666d95469602c5652609ee
ms.translationtype: MT
ms.contentlocale: hu-HU
ms.lasthandoff: 01/05/2021
ms.locfileid: "98375533"
---
# Az.Sql Module
## Leírás
Ez a témakör az Azure SQL-adatbázis parancsmagokkal kapcsolatos súgótémaköreket mutatja be.

## Az.Sql-parancsmagok
### [Add-AzSqlDatabaseToFailoverGroup](Add-AzSqlDatabaseToFailoverGroup.md)
Hozzáad egy vagy több adatbázist egy Azure SQL-adatbázis feladatátvételi csoportjához.

### [Add-AzSqlElasticJobStep](Add-AzSqlElasticJobStep.md)
Feladatlépés hozzáadása egy feladathoz

### [Add-AzSqlElasticAsticAsticTarget](Add-AzSqlElasticJobTarget.md)
Cél hozzáadása egy célcsoporthoz

### [Add-AzSqlInstanceKeyVaultKey](Add-AzSqlInstanceKeyVaultKey.md)
Hozzáad egy kulcskulcsot a megadott felügyelt példányhoz. 

### [Add-AzSqlManagedInstanceTransparentDataEncryptionCertificate](Add-AzSqlManagedInstanceTransparentDataEncryptionCertificate.md)
Áttetsző adattitkosítási tanúsítvány hozzáadása a megadott felügyelt példányhoz

### [Add-AzSqlServerKeyVaultKey](Add-AzSqlServerKeyVaultKey.md)
Hozzáad egy kulcstárkulcsot egy SQL-kiszolgálóhoz.

### [Add-AzSqlServerTransparentDataEncryptionCertificate](Add-AzSqlServerTransparentDataEncryptionCertificate.md)
Áttetsző adattitkosítási tanúsítvány hozzáadása a megadott SQL Server-példányhoz

### [Clear-AzSqlDatabaseAdvancedThreatProtectionSetting](Clear-AzSqlDatabaseAdvancedThreatProtectionSetting.md)
Eltávolítja a komplex veszélyforrások elleni védelem speciális beállításait az adatbázisból.

### [Clear-AzSqlDatabaseVulnerabilityAssessmentRuleBaseline](Clear-AzSqlDatabaseVulnerabilityAssessmentRuleBaseline.md)
Törli a biztonsági rés felmérésére vonatkozó szabály alapértékét.

### [Clear-AzSqlDatabaseVulnerabilityAssessmentSetting](Clear-AzSqlDatabaseVulnerabilityAssessmentSetting.md)
Törli egy adatbázis biztonsági felmérési beállításait.

### [Clear-AzSqlInstanceDatabaseVulnerabilityAssessmentRuleBaseline](Clear-AzSqlInstanceDatabaseVulnerabilityAssessmentRuleBaseline.md)
Törli a biztonsági rés felmérésére vonatkozó szabály alapértékét.

### [Clear-AzSqlInstanceDatabaseVulnerabilityAssessmentSetting](Clear-AzSqlInstanceDatabaseVulnerabilityAssessmentSetting.md)
Törli a felügyelt adatbázisok biztonsági rés-felmérési beállításait.

### [Clear-AzSqlInstanceVulnerabilityAssessmentSetting](Clear-AzSqlInstanceVulnerabilityAssessmentSetting.md)
Törli egy felügyelt példány biztonsági rés-felmérési beállításait.

### [Clear-AzSqlServerAdvancedThreatProtectionSetting](Clear-AzSqlServerAdvancedThreatProtectionSetting.md)
Eltávolítja a komplex veszélyforrások elleni védelem speciális beállításait a kiszolgálóról.

### [Clear-AzSqlServerVulnerabilityAssessmentSetting](Clear-AzSqlServerVulnerabilityAssessmentSetting.md)
Törli a kiszolgáló biztonsági résértékelési beállításait.

### [Complete-AzSqlInstanceDatabaseLogReplay](Complete-AzSqlInstanceDatabaseLogReplay.md)
A megadott adatbázis napló-visszajátszási szolgáltatásának befejezése.

### [Convert-AzSqlDatabaseVulnerabilityAssessmentScan](Convert-AzSqlDatabaseVulnerabilityAssessmentScan.md)
A biztonsági rés felmérésének eredményét Excel-formátumra konvertálja.

### [Convert-AzSqlInstanceDatabaseVulnerabilityAssessmentScan](Convert-AzSqlInstanceDatabaseVulnerabilityAssessmentScan.md)
A biztonsági rés felmérésének eredményét Excel-formátumra konvertálja.

### [Disable-AzSqlDatabaseSensitivityRecommendation](Disable-AzSqlDatabaseSensitivityRecommendation.md)
Letiltja (mellőzi) a bizalmasságra vonatkozó javaslatokat az adatbázis oszlopainál.

### [Disable-AzSqlInstanceActiveDirectoryOnlyAuthentication](Disable-AzSqlInstanceActiveDirectoryOnlyAuthentication.md)
Letiltja az Azure AD-hitelesítést csak egy adott SQL Felügyelt példány esetén.

### [Disable-AzSqlInstanceAdvancedDataSecurity](Disable-AzSqlInstanceAdvancedDataSecurity.md)
Letiltja a speciális adatbiztonságot egy felügyelt példányon.

### [Disable-AzSqlInstanceDatabaseSensitivityRecommendation](Disable-AzSqlInstanceDatabaseSensitivityRecommendation.md)
Letiltja (mellőzi) a bizalmasságra vonatkozó javaslatokat az Azure SQL felügyeltpéldány-adatbázis oszlopainál.

### [Disable-AzSqlServerActiveDirectoryOnlyAuthentication](Disable-AzSqlServerActiveDirectoryOnlyAuthentication.md)
Letiltja az Azure AD-hitelesítést csak egy adott SQL Server esetén.

### [Disable-AzSqlServerAdvancedDataSecurity](Disable-AzSqlServerAdvancedDataSecurity.md)
Letiltja a speciális adatbiztonságot a kiszolgálón.

### [Disable-AzSqlServerAdvancedThreatProtection](Disable-AzSqlServerAdvancedThreatProtection.md)
Letiltja a komplex veszélyforrások elleni védelmet a kiszolgálón.

### [Enable-AzSqlDatabaseSensitivityRecommendation](Enable-AzSqlDatabaseSensitivityRecommendation.md)
Engedélyezi a bizalmasságra vonatkozó javaslatokat az oszlopokon (a javaslatok alapértelmezés szerint minden oszlopban engedélyezve vannak) az adatbázisban.

### [Enable-AzSqlInstanceActiveDirectoryOnlyAuthentication](Enable-AzSqlInstanceActiveDirectoryOnlyAuthentication.md)
Engedélyezi az Azure AD-hitelesítést csak egy adott SQL Felügyelt példány esetén.

### [Enable-AzSqlInstanceAdvancedDataSecurity](Enable-AzSqlInstanceAdvancedDataSecurity.md)
Speciális adatbiztonságot tesz lehetővé felügyelt példányon.

### [Enable-AzSqlInstanceDatabaseSensitivityRecommendation](Enable-AzSqlInstanceDatabaseSensitivityRecommendation.md)
Engedélyezi a bizalmasságra vonatkozó javaslatokat az oszlopokon (a javaslatok alapértelmezés szerint minden oszlopban engedélyezve vannak) az Azure SQL felügyeltpéldány-adatbázisban.

### [Enable-AzSqlServerActiveDirectoryOnlyAuthentication](Enable-AzSqlServerActiveDirectoryOnlyAuthentication.md)
Az Azure AD-hitelesítést csak egy adott SQL Serverhez engedélyezi.

### [Enable-AzSqlServerAdvancedDataSecurity](Enable-AzSqlServerAdvancedDataSecurity.md)
Engedélyezi a speciális adatbiztonságot a kiszolgálón.

### [Enable-AzSqlServerAdvancedThreatProtection](Enable-AzSqlServerAdvancedThreatProtection.md)
Engedélyezi a Komplex veszélyforrások elleni védelmet egy kiszolgálón.

### [Get-AzSqlCapability](Get-AzSqlCapability.md)
SQL-adatbázis-funkciókat kap az aktuális előfizetéshez.

### [Get-AzSqlDatabase](Get-AzSqlDatabase.md)
Egy vagy több adatbázist kap.

### [Get-AzSqlDatabaseActivity](Get-AzSqlDatabaseActivity.md)
Az adatbázis-műveletek állapotát kapja meg.

### [Get-AzSqlDatabaseAdvancedThreatProtectionSetting](Get-AzSqlDatabaseAdvancedThreatProtectionSetting.md)
A komplex veszélyforrások elleni védelem speciális beállításainak lekérdezése egy adatbázishoz.

### [Get-AzSqlDatabaseAdvisor](Get-AzSqlDatabaseAdvisor.md)
Egy vagy több tanácsadót kap egy Azure SQL-adatbázishoz.

### [Get-AzSqlDatabaseAudit](Get-AzSqlDatabaseAudit.md)
Egy Azure SQL-adatbázis naplózási beállításait kapja meg.

### [Get-AzSqlDatabaseBackupLongTermRetentionPolicy](Get-AzSqlDatabaseBackupLongTermRetentionPolicy.md)
Hosszú távú adatmegőrzési házirendet kap az adatbázishoz.

### [Get-AzSqlDatabaseBackupShortTermRetentionPolicy](Get-AzSqlDatabaseBackupShortTermRetentionPolicy.md)
Biztonsági másolatot kap rövid távú adatmegőrzési házirendről.

### [Get-AzSqlDatabaseDataMaskingPolicy](Get-AzSqlDatabaseDataMaskingPolicy.md)
Beveszi egy adatbázis adatmaszkolási házirendjeit.

### [Get-AzSqlDatabaseDataMaskingRule](Get-AzSqlDatabaseDataMaskingRule.md)
Beveszi az adatmaszkolási szabályokat egy adatbázisból.

### [Get-AzSqlDatabaseExpanded](Get-AzSqlDatabaseExpanded.md)
Adatbázist és kibontott tulajdonságértékeket kap.

### [Get-AzSqlDatabaseFailoverGroup](Get-AzSqlDatabaseFailoverGroup.md)
Az Azure SQL-adatbázis feladatátvételi csoportjainak levétele vagy listái.

### [Get-AzSqlDatabaseGeoBackup](Get-AzSqlDatabaseGeoBackup.md)
Geodundáns biztonsági másolatot készít egy adatbázisról.

### [Get-AzSqlDatabaseGeoBackupPolicy](Get-AzSqlDatabaseGeoBackupPolicy.md)
Geo backup házirendet kap az adatbázishoz.

### [Get-AzSqlDatabaseImportExportStatus](Get-AzSqlDatabaseImportExportStatus.md)
Az Azure SQL-adatbázis importálásának vagy exportálásának részleteit tartalmazza.

### [Get-AzSqlDatabaseIndexRecommendation](Get-AzSqlDatabaseIndexRecommendation.md)
A kiszolgálóhoz vagy adatbázishoz ajánlott indexműveleteket kapja meg.

### [Get-AzSqlDatabaseInstanceFailoverGroup](Get-AzSqlDatabaseInstanceFailoverGroup.md)
Példány feladatátvételi csoportjait kapja meg vagy sorolja fel.

### [Get-AzSqlDatabaseLongTermRetentionBackup](Get-AzSqlDatabaseLongTermRetentionBackup.md)
Egy vagy több hosszú távú adatmegőrzési biztonsági másolatot kap.

### [Get-AzSqlDatabaseRecomdataAction](Get-AzSqlDatabaseRecommendedAction.md)
Egy vagy több javasolt műveletet kap egy Azure SQL adatbázis-tanácsadóhoz.

### [Get-AzSqlDatabaseReplicationLink](Get-AzSqlDatabaseReplicationLink.md)
Lekérte az Azure SQL-adatbázis és egy erőforráscsoport vagy az SQL Server közötti földrajzi replikációs kapcsolatokat.

### [Get-AzSqlDatabaseRestorePoint](Get-AzSqlDatabaseRestorePoint.md)
Beolvassa a különálló visszaállítási pontokat, amelyekből az SQL-adattárhely visszaállítható.

### [Get-AzSqlDatabaseSensitivityClassification](Get-AzSqlDatabaseSensitivityClassification.md)
Begyűjti az adatbázis oszlopainak aktuális adattípusait és bizalmasság-címkéit.

### [Get-AzSqlDatabaseSensitivityRecommendation](Get-AzSqlDatabaseSensitivityRecommendation.md)
Az adatbázis oszlopainak ajánlott adattípusait és bizalmasság-címkéit kapja meg.

### [Get-AzSqlDatabaseTransparentDataEncryption](Get-AzSqlDatabaseTransparentDataEncryption.md)
Egy adatbázis TDE-állapotát kapja meg.

### [Get-AzSqlDatabaseTransparentDataEncryptionActivity](Get-AzSqlDatabaseTransparentDataEncryptionActivity.md)
Beolvassa egy adatbázis TDE-vizsgálatának előrehaladását.

### [Get-AzSqlDatabaseUpgradeHint](Get-AzSqlDatabaseUpgradeHint.md)
Az adatbázisokra vonatkozó árszint-tippeket kap.

### [Get-AzSqlDatabaseVulnerabilityAssessmentRuleBaseline](Get-AzSqlDatabaseVulnerabilityAssessmentRuleBaseline.md)
Beveszi a biztonsági rés felmérésére vonatkozó szabály alapértékét.

### [Get-AzSqlDatabaseVulnerabilityAssessmentScanRecord](Get-AzSqlDatabaseVulnerabilityAssessmentScanRecord.md)
Beolvassa az adott adatbázishoz tartozó biztonsági résértékelési vizsgálati rekordokat.

### [Get-AzSqlDatabaseVulnerabilityAssessmentSetting](Get-AzSqlDatabaseVulnerabilityAssessmentSetting.md)
Beveszi egy adatbázis biztonsági felmérési beállításait.

### [Get-AzSqlDeletedDatabaseBackup](Get-AzSqlDeletedDatabaseBackup.md)
Visszaállítható törölt adatbázist kap.

### [Get-AzSqlDeletedInstanceDatabaseBackup](Get-AzSqlDeletedInstanceDatabaseBackup.md)
Visszaállítható törölt adatbázist kap.

### [Get-AzSqlElasticJob](Get-AzSqlElasticJob.md)
Egy vagy több álláshoz jut

### [Get-AzSqlElasticAgent](Get-AzSqlElasticJobAgent.md)
Azure SQL-rugalmassági feladat ügynökhöz jut

### [Get-AzSqlElasticCredential](Get-AzSqlElasticJobCredential.md)
Egy vagy több hitelesítő adatokat kap

### [Get-AzSqlElasticAsticAsticExecution](Get-AzSqlElasticJobExecution.md)
Egy vagy több feladatvégrehajtást kap

### [Get-AzSqlElasticJobStep](Get-AzSqlElasticJobStep.md)
Egy vagy több feladat lépéseit kapja

### [Get-AzSqlElasticJobStepExecution](Get-AzSqlElasticJobStepExecution.md)
Egy vagy több feladatlépés végrehajtása

### [Get-AzSqlElasticAsticAsticTargetExecution](Get-AzSqlElasticJobTargetExecution.md)
Egy vagy több feladatcél végrehajtása

### [Get-AzSqlElasticAsticAsticTargetGroup](Get-AzSqlElasticJobTargetGroup.md)
Egy vagy több célcsoportot kap a feladathoz

### [Get-AzSqlElasticPool](Get-AzSqlElasticPool.md)
Rugalmas készleteket és azok tulajdonságértékeket kap egy Azure SQL-adatbázisban.

### [Get-AzSqlElasticPoolActivity](Get-AzSqlElasticPoolActivity.md)
Egy rugalmas készleten található műveletek állapotát kapja meg.

### [Get-AzSqlElasticPoolAdvisor](Get-AzSqlElasticPoolAdvisor.md)
Egy vagy több tanácsadót kap egy Azure SQL-rugalmassági készlethez.

### [Get-AzSqlElasticPoolDatabase](Get-AzSqlElasticPoolDatabase.md)
Rugalmas adatbázisokat és tulajdonságértékeket tartalmaz.

### [Get-AzSqlElasticPoolRecommendation](Get-AzSqlElasticPoolRecommendation.md)
Rugalmas készletjavaslatokat kap.

### [Get-AzSqlElasticPoolRecomasticAction](Get-AzSqlElasticPoolRecommendedAction.md)
Egy vagy több javasolt műveletet kap egy Azure SQL rugalmassági készlet tanácsadóhoz.

### [Get-AzSqlInstance](Get-AzSqlInstance.md)
Információkat ad vissza az Azure SQL Felügyelt adatbázis-példányról.

### [Get-AzSqlInstanceActiveDirectoryAdministrator](Get-AzSqlInstanceActiveDirectoryAdministrator.md)
Információkat kap az SQL Felügyelt példány Azure AD-rendszergazdájának adatairól.

### [Get-AzSqlInstanceActiveDirectoryOnlyAuthentication](Get-AzSqlInstanceActiveDirectoryOnlyAuthentication.md)
Az Azure AD-hitelesítést csak egy adott SQL Felügyelt példány esetén kapja meg.

### [Get-AzSqlInstanceAdvancedDataSecurityPolicy](Get-AzSqlInstanceAdvancedDataSecurityPolicy.md)
Felügyelt példány speciális adatbiztonsági házirendet kap.

### [Get-AzSqlInstanceDatabase](Get-AzSqlInstanceDatabase.md)
Információkat ad vissza az Azure SQL Felügyelt példány adatbázisról.

### [Get-AzSqlInstanceDatabaseBackupLongTermRetentionPolicy](Get-AzSqlInstanceDatabaseBackupLongTermRetentionPolicy.md)
Felügyelt adatbázis hosszú távú adatmegőrzési házirendje

### [Get-AzSqlInstanceDatabaseBackupShortTermRetentionPolicy](Get-AzSqlInstanceDatabaseBackupShortTermRetentionPolicy.md)
Biztonsági másolatot kap rövid távú adatmegőrzési házirendről.

### [Get-AzSqlInstanceDatabaseGeoBackup](Get-AzSqlInstanceDatabaseGeoBackup.md)
Az Azure SQL Felügyelt példány adatbázis redundáns biztonsági másolatának adatait adja eredményül.

### [Get-AzSqlInstanceDatabaseLogReplay](Get-AzSqlInstanceDatabaseLogReplay.md)
A Napló visszajátszása szolgáltatás állapotát kapja.

### [Get-AzSqlInstanceDatabaseLongTermRetentionBackup](Get-AzSqlInstanceDatabaseLongTermRetentionBackup.md)
Hosszú távú adatmegőrzési biztonsági mentést kap.

### [Get-AzSqlInstanceDatabaseSensitivityClassification](Get-AzSqlInstanceDatabaseSensitivityClassification.md)
Az Azure SQL felügyeltpéldány-adatbázisban lévő oszlopok aktuális adattípusainak és bizalmasság-címkéinek begyűjtése.

### [Get-AzSqlInstanceDatabaseSensitivityRecommendation](Get-AzSqlInstanceDatabaseSensitivityRecommendation.md)
Az Azure SQL felügyeltpéldány-adatbázisban található oszlopok ajánlott adattípusait és bizalmasság-címkéit kapja meg.

### [Get-AzSqlInstanceDatabaseVulnerabilityAssessmentRuleBaseline](Get-AzSqlInstanceDatabaseVulnerabilityAssessmentRuleBaseline.md)
Beveszi a biztonsági rés felmérésére vonatkozó szabály alapértékét.

### [Get-AzSqlInstanceDatabaseVulnerabilityAssessmentScanRecord](Get-AzSqlInstanceDatabaseVulnerabilityAssessmentScanRecord.md)
Beolvassa az adott felügyelt adatbázishoz tartozó biztonsági résértékelési vizsgálati rekordokat.

### [Get-AzSqlInstanceDatabaseVulnerabilityAssessmentSetting](Get-AzSqlInstanceDatabaseVulnerabilityAssessmentSetting.md)
Egy felügyelt adatbázis biztonsági résértékelési beállításait kapja meg.

### [Get-AzSqlInstanceKeyVaultKey](Get-AzSqlInstanceKeyVaultKey.md)
Egy SQL-példány kulcstárkulcsait kapja meg.

### [Get-AzSqlInstanceOperation](Get-AzSqlInstanceOperation.md)
Egy SQL által felügyelt példány műveleteit kapja meg.

### [Get-AzSqlInstancePool](Get-AzSqlInstancePool.md)
Az Azure SQL-példánykészlet adatait adja eredményül.

### [Get-AzSqlInstancePoolUsage](Get-AzSqlInstancePoolUsage.md)
Információkat ad vissza egy Azure SQL-példánykészlet használatáról.

### [Get-AzSqlInstanceTransparentDataEncryptionProtector](Get-AzSqlInstanceTransparentDataEncryptionProtector.md)
Az SQL által kezelt példány átlátszó adattitkosítási (TDE) védelemre van bevéve.

### [Get-AzSqlInstanceVulnerabilityAssessmentSetting](Get-AzSqlInstanceVulnerabilityAssessmentSetting.md)
Egy felügyelt példány biztonsági résértékelési beállításait kapja meg.

### [Get-AzSqlServer](Get-AzSqlServer.md)
Az SQL Database-kiszolgálókra vonatkozó információkat adja vissza.

### [Get-AzSqlServerActiveDirectoryAdministrator](Get-AzSqlServerActiveDirectoryAdministrator.md)
Információkat kap az SQL Serverhez szükséges Azure AD-rendszergazdáról.

### [Get-AzSqlServerActiveDirectoryOnlyAuthentication](Get-AzSqlServerActiveDirectoryOnlyAuthentication.md)
Az Azure AD-hitelesítést csak egy adott SQL Serverhez kapja meg.

### [Get-AzSqlServerAdvancedDataSecurityPolicy](Get-AzSqlServerAdvancedDataSecurityPolicy.md)
A kiszolgáló speciális adatbiztonsági házirendet kap.

### [Get-AzSqlServerAdvancedThreatProtectionSetting](Get-AzSqlServerAdvancedThreatProtectionSetting.md)
A kiszolgáló komplex veszélyforrások elleni védelemre vonatkozó speciális beállításait kapja meg.

### [Get-AzSqlServerAdvisor](Get-AzSqlServerAdvisor.md)
Egy vagy több tanácsadót kap egy Azure SQL Serverhez.

### [Get-AzSqlServerAudit](Get-AzSqlServerAudit.md)
Egy Azure SQL-kiszolgáló naplózási beállításait kapja meg.

### [Get-AzSqlServerCommunicationLink](Get-AzSqlServerCommunicationLink.md)
Kommunikációs hivatkozásokat kap az adatbázis-kiszolgálók közötti rugalmas adatbázis-tranzakciókhoz.

### [Get-AzSqlServerMSSupportAudit](Get-AzSqlServerMSSupportAudit.md)
A Microsoft támogatási műveleteinek naplózási beállításait kapja meg egy Azure SQL-kiszolgálóhoz.

### [Get-AzSqlServerDisasterRecoveryConfiguration](Get-AzSqlServerDisasterRecoveryConfiguration.md)
Adatbázis-kiszolgálói rendszer helyreállítási konfigurációját kapja meg.

### [Get-AzSqlServerDisasterRecoveryConfigurationActivity](Get-AzSqlServerDisasterRecoveryConfigurationActivity.md)
Egy adatbázis-kiszolgáló rendszer-helyreállítási konfigurációjának tevékenységét kapja meg.

### [Get-AzSqlServerDnsAlias](Get-AzSqlServerDnsAlias.md)
Lekérte vagy felsorolja az Azure SQL Server DNS-aliasát.

### [Get-AzSqlServerFirewallRule](Get-AzSqlServerFirewallRule.md)
Tűzfalszabályokat kap egy SQL-adatbázis-kiszolgálóhoz.

### [Get-AzSqlServerKeyVaultKey](Get-AzSqlServerKeyVaultKey.md)
Egy SQL-kiszolgáló kulcstárkulcsait kapja meg.

### [Get-AzSqlServerRecomsqlAction](Get-AzSqlServerRecommendedAction.md)
Egy vagy több javasolt műveletet kap egy Azure SQL Server-tanácsadóhoz.

### [Get-AzSqlServerServiceObjective](Get-AzSqlServerServiceObjective.md)
Szolgáltatás-célkitűzéseit kapja egy Azure SQL-adatbázis-kiszolgálóhoz.

### [Get-AzSqlServerTransparentDataEncryptionProtector](Get-AzSqlServerTransparentDataEncryptionProtector.md)
Az Átlátszó adattitkosítás (TDE) védelme

### [Get-AzSqlServerUpgradeHint](Get-AzSqlServerUpgradeHint.md)
Az Azure SQL-adatbázis-kiszolgáló frissítéséhez árszint-tippeket ad.

### [Get-AzSqlServerVirtualNetworkRule](Get-AzSqlServerVirtualNetworkRule.md)
Az Azure SQL Server Virtuális hálózati szabály lekérte vagy felsorolja azokat.

### [Get-AzSqlServerVulnerabilityAssessmentSetting](Get-AzSqlServerVulnerabilityAssessmentSetting.md)
A kiszolgáló biztonsági résértékelési beállításainak lekérte.

### [Get-AzSqlSyncAgent](Get-AzSqlSyncAgent.md)
Információkat ad vissza az Azure SQL Sync Agents szolgáltatásról.

### [Get-AzSqlSyncAgentLinkedDatabase](Get-AzSqlSyncAgentLinkedDatabase.md)
Szinkronizálási ügynök által csatolt SQL Server-adatbázisokkal kapcsolatos információkat ad eredményül.

### [Get-AzSqlSyncGroup](Get-AzSqlSyncGroup.md)
Információkat ad vissza az Azure SQL Database Sync Groups szolgáltatásról.

### [Get-AzSqlSyncGroupLog](Get-AzSqlSyncGroupLog.md)
Egy Azure SQL Database Sync-csoport naplóit adja eredményül.

### [Get-AzSqlSyncMember](Get-AzSqlSyncMember.md)
Információkat ad vissza az Azure SQL Database Sync tagjairól.

### [Get-AzSqlSyncSchema](Get-AzSqlSyncSchema.md)
Egy tag- vagy központi adatbázis szinkronizálási sémájáról ad információkat.

### [Get-AzSqlVirtualCluster](Get-AzSqlVirtualCluster.md)
Az Azure SQL Virtuális fürtről ad információkat.

### [Invoke-AzSqlDatabaseFailover](Invoke-AzSqlDatabaseFailover.md)
Feladatátvételt ad az adatbázisnak.

### [Invoke-AzSqlElasticPoolFailover](Invoke-AzSqlElasticPoolFailover.md)
Feladatátvétel egy rugalmas készletben.

### [Invoke-AzSqlInstanceFailover](Invoke-AzSqlInstanceFailover.md)
Feladatátvétel egy Azure SQL Felügyelt példányban.

### [New-AzSqlDatabase](New-AzSqlDatabase.md)
Létrehoz egy adatbázist vagy egy rugalmas adatbázist.

### [New-AzSqlDatabaseCopy](New-AzSqlDatabaseCopy.md)
Létrehozza a pillanatfelvételt használó SQL-adatbázis másolatát.

### [New-AzSqlDatabaseDataMaskingRule](New-AzSqlDatabaseDataMaskingRule.md)
Adatmaszkolási szabályt hoz létre egy adatbázishoz.

### [New-AzSqlDatabaseExport](New-AzSqlDatabaseExport.md)
Azure SQL-adatbázist exportál .bacpac fájlként egy tárfiókba.

### [New-AzSqlDatabaseFailoverGroup](New-AzSqlDatabaseFailoverGroup.md)
Ez a parancs létrehoz egy új Azure SQL-adatbázis feladatátvételi csoportot.

### [New-AzSqlDatabaseImport](New-AzSqlDatabaseImport.md)
.bacpac fájlt importál, és új adatbázist hoz létre a kiszolgálón.

### [New-AzSqlDatabaseInstanceFailoverGroup](New-AzSqlDatabaseInstanceFailoverGroup.md)
Ez a parancs létrehoz egy új Azure SQL-adatbázispéldány feladatátvételi csoportot.

### [New-AzSqlDatabaseRestorePoint](New-AzSqlDatabaseRestorePoint.md)
Létrehoz egy új visszaállítási pontot, amelyből az SQL-adatbázis visszaállítható.

### [New-AzSqlDatabaseSecondary](New-AzSqlDatabaseSecondary.md)
Létrehoz egy másodlagos adatbázist egy meglévő adatbázishoz, és megkezdi az adatreplikációt.

### [New-AzSqlElasticJob](New-AzSqlElasticJob.md)
Új feladat létrehozása

### [New-AzSqlElasticAgent](New-AzSqlElasticJobAgent.md)
Új rugalmas ügynök létrehozása

### [New-AzSqlElasticCredential](New-AzSqlElasticJobCredential.md)
Új feladat hitelesítő adatainak létrehozása

### [New-AzSqlElasticAsticAsticTargetGroup](New-AzSqlElasticJobTargetGroup.md)
Új célcsoport létrehozása

### [New-AzSqlElasticPool](New-AzSqlElasticPool.md)
Rugalmas adatbáziskészletet hoz létre egy SQL-adatbázishoz.

### [New-AzSqlInstance](New-AzSqlInstance.md)
Létrehoz egy Azure SQL-adatbázis felügyelt példányát.

### [New-AzSqlInstanceDatabase](New-AzSqlInstanceDatabase.md)
Létrehoz egy Azure SQL Felügyeltpéldány-adatbázist.

### [New-AzSqlInstancePool](New-AzSqlInstancePool.md)
Létrehoz egy Azure SQL-példánykészletet.

### [New-AzSqlServer](New-AzSqlServer.md)
SQL-adatbázis-kiszolgálót hoz létre.

### [New-AzSqlServerCommunicationLink](New-AzSqlServerCommunicationLink.md)
Kommunikációs hivatkozást hoz létre két SQL-adatbázis-kiszolgáló közötti rugalmas adatbázis-tranzakciókhoz.

### [New-AzSqlServerDisasterRecoveryConfiguration](New-AzSqlServerDisasterRecoveryConfiguration.md)
Adatbázis-kiszolgálói rendszer helyreállítási konfigurációját hozza létre.

### [New-AzSqlServerDnsAlias](New-AzSqlServerDnsAlias.md)
Ez a parancs új Azure SQL Server DNS-aliast hoz létre.

### [New-AzSqlServerFirewallRule](New-AzSqlServerFirewallRule.md)
Tűzfalszabályt hoz létre egy SQL-adatbázis-kiszolgálóhoz.

### [New-AzSqlServerVirtualNetworkRule](New-AzSqlServerVirtualNetworkRule.md)
Azure SQL Server virtuális hálózati szabályt hoz létre. 

### [New-AzSqlSyncAgent](New-AzSqlSyncAgent.md)
Létrehoz egy Azure SQL Sync-ügynököt.

### [New-AzSqlSyncAgentKey](New-AzSqlSyncAgentKey.md)
Létrehozza az Azure SQL Sync ügynökkulcsát.

### [New-AzSqlSyncGroup](New-AzSqlSyncGroup.md)
Létrehoz egy Azure SQL Database Sync-csoportot.

### [New-AzSqlSyncMember](New-AzSqlSyncMember.md)
Létrehoz egy Azure SQL Database Sync- tagot.

### [Remove-AzSqlDatabase](Remove-AzSqlDatabase.md)
Eltávolít egy Azure SQL-adatbázist.

### [Remove-AzSqlDatabaseAudit](Remove-AzSqlDatabaseAudit.md)
Eltávolítja egy Azure SQL-adatbázis naplózási beállításait.

### [Remove-AzSqlDatabaseDataMaskingRule](Remove-AzSqlDatabaseDataMaskingRule.md)
Eltávolítja az adatbázis adatmaszkolási szabályát.

### [Remove-AzSqlDatabaseFailoverGroup](Remove-AzSqlDatabaseFailoverGroup.md)
Eltávolít egy Azure SQL-adatbázis feladatátvételi csoportot.

### [Remove-AzSqlDatabaseFromFailoverGroup](Remove-AzSqlDatabaseFromFailoverGroup.md)
Eltávolít egy vagy több adatbázist egy Azure SQL-adatbázis feladatátvételi csoportjából.

### [Remove-AzSqlDatabaseInstanceFailoverGroup](Remove-AzSqlDatabaseInstanceFailoverGroup.md)
Egy példány feladatátvételi csoportjának eltávolítása.

### [Remove-AzSqlDatabaseLongTermRetentionBackup](Remove-AzSqlDatabaseLongTermRetentionBackup.md)
Törli a hosszú távú adatmegőrzési biztonsági mentést.

### [Remove-AzSqlDatabaseRestorePoint](Remove-AzSqlDatabaseRestorePoint.md)
A megadott visszaállítási pont eltávolítása az SQL-adatbázisból.

### [Remove-AzSqlDatabaseSecondary](Remove-AzSqlDatabaseSecondary.md)
Az SQL-adatbázis és a megadott másodlagos adatbázis közötti adatreplikáció megszüntetése.

### [Remove-AzSqlDatabaseSensitivityClassification](Remove-AzSqlDatabaseSensitivityClassification.md)
Eltávolítja az adatbázis oszlopainak adattípusait és bizalmasság-címkéit.

### [Remove-AzSqlElasticJob](Remove-AzSqlElasticJob.md)
Feladat eltávolítása

### [Remove-AzSqlElasticAgent](Remove-AzSqlElasticJobAgent.md)
A rugalmas munka ügynök eltávolítása

### [Remove-AzSqlElasticCredential](Remove-AzSqlElasticJobCredential.md)
A rugalmas feladat hitelesítő adatainak eltávolítása

### [Remove-AzSqlElasticJobStep](Remove-AzSqlElasticJobStep.md)
A feladat lépésének eltávolítása

### [Remove-AzSqlElasticAsticTarget](Remove-AzSqlElasticJobTarget.md)
A cél eltávolítása a célcsoportból

### [Remove-AzSqlElasticAsticTargetGroup](Remove-AzSqlElasticJobTargetGroup.md)
A célcsoport eltávolítása

### [Remove-AzSqlElasticPool](Remove-AzSqlElasticPool.md)
Egy rugalmas adatbáziskészlet törlése.

### [Remove-AzSqlInstance](Remove-AzSqlInstance.md)
Eltávolít egy Azure SQL Felügyelt adatbázis-példányt.

### [Remove-AzSqlInstanceActiveDirectoryAdministrator](Remove-AzSqlInstanceActiveDirectoryAdministrator.md)
Eltávolít egy Azure AD-rendszergazdát az SQL Felügyelt példányhoz.

### [Remove-AzSqlInstanceDatabase](Remove-AzSqlInstanceDatabase.md)
Eltávolít egy Azure SQL Managed Instance-adatbázist.

### [Remove-AzSqlInstanceDatabaseLongTermRetentionBackup](Remove-AzSqlInstanceDatabaseLongTermRetentionBackup.md)
Törli a hosszú távú adatmegőrzési biztonsági mentést.

### [Remove-AzSqlInstanceDatabaseSensitivityClassification](Remove-AzSqlInstanceDatabaseSensitivityClassification.md)
Eltávolítja az Azure SQL felügyeltpéldány-adatbázisban található oszlopok adattípusait és bizalmasság-címkéit.

### [Remove-AzSqlInstanceKeyVaultKey](Remove-AzSqlInstanceKeyVaultKey.md)
Kulcsbillentyű eltávolítása egy SQL-kezelt példányból

### [Remove-AzSqlInstancePool](Remove-AzSqlInstancePool.md)
Eltávolít egy Azure SQL-példánykészletet.

### [Remove-AzSqlServer](Remove-AzSqlServer.md)
Eltávolít egy Azure SQL-adatbázis-kiszolgálót.

### [Remove-AzSqlServerActiveDirectoryAdministrator](Remove-AzSqlServerActiveDirectoryAdministrator.md)
Eltávolít egy Azure AD-rendszergazdát az SQL Serverhez.

### [Remove-AzSqlServerAudit](Remove-AzSqlServerAudit.md)
Eltávolítja egy Azure SQL-kiszolgáló naplózási beállításait.

### [Remove-AzSqlServerCommunicationLink](Remove-AzSqlServerCommunicationLink.md)
Törli a két kiszolgáló közötti rugalmas adatbázis-tranzakciók kommunikációs hivatkozását.

### [Remove-AzSqlServerDisasterRecoveryConfiguration](Remove-AzSqlServerDisasterRecoveryConfiguration.md)
Eltávolítja az SQL-adatbázis-kiszolgáló rendszer-helyreállítási konfigurációját.

### [Remove-AzSqlServerDnsAlias](Remove-AzSqlServerDnsAlias.md)
Eltávolítja az Azure SQL Server DNS-aliasát.

### [Remove-AzSqlServerFirewallRule](Remove-AzSqlServerFirewallRule.md)
Tűzfalszabály törlése SQL-adatbázis-kiszolgálóról.

### [Remove-AzSqlServerKeyVaultKey](Remove-AzSqlServerKeyVaultKey.md)
Eltávolít egy kulcstárkulcsot egy SQL-kiszolgálóról.

### [Remove-AzSqlServerMSSupportAudit](Remove-AzSqlServerMSSupportAudit.md)
Eltávolítja a Microsoft támogatási műveleteinek naplózási beállításait egy Azure SQL-kiszolgálóról.

### [Remove-AzSqlServerVirtualNetworkRule](Remove-AzSqlServerVirtualNetworkRule.md)
Törli az Azure SQL Server virtuális hálózati szabályát.

### [Remove-AzSqlSyncAgent](Remove-AzSqlSyncAgent.md)
Eltávolít egy Azure SQL Sync-ügynököt.

### [Remove-AzSqlSyncGroup](Remove-AzSqlSyncGroup.md)
Eltávolít egy Azure SQL Database Sync-csoportot.

### [Remove-AzSqlSyncMember](Remove-AzSqlSyncMember.md)
Eltávolít egy Azure SQL Database Sync- tagot.

### [Remove-AzSqlVirtualCluster](Remove-AzSqlVirtualCluster.md)
Eltávolít egy Azure SQL virtuális fürtöt.

### [Restore-AzSqlDatabase](Restore-AzSqlDatabase.md)
Sql-adatbázis visszaállítása.

### [Restore-AzSqlInstanceDatabase](Restore-AzSqlInstanceDatabase.md)
Visszaállít egy Azure SQL Felügyelt példány adatbázist.

### [Resume-AzSqlDatabase](Resume-AzSqlDatabase.md)
Sql Data Warehouse-adatbázis folytatása.

### [Set-AzSqlDatabase](Set-AzSqlDatabase.md)
Beállítja egy adatbázis tulajdonságait, vagy egy meglévő adatbázist rugalmas készletbe áthelyez.

### [Set-AzSqlDatabaseAdvisorAutoExecuteStatus](Set-AzSqlDatabaseAdvisorAutoExecuteStatus.md)
Az Azure SQL-adatbázis tanácsadó automatikus végrehajtásának állapotát módosítja.

### [Set-AzSqlDatabaseAudit](Set-AzSqlDatabaseAudit.md)
Egy Azure SQL-adatbázis naplózási beállításait módosítja.

### [Set-AzSqlDatabaseBackupLongTermRetentionPolicy](Set-AzSqlDatabaseBackupLongTermRetentionPolicy.md)
Kiszolgálói hosszú távú adatmegőrzési házirendet állít be.

### [Set-AzSqlDatabaseBackupShortTermRetentionPolicy](Set-AzSqlDatabaseBackupShortTermRetentionPolicy.md)
Biztonsági másolatot készít rövid távú adatmegőrzési házirendről.

### [Set-AzSqlDatabaseDataMaskingPolicy](Set-AzSqlDatabaseDataMaskingPolicy.md)
Beállítja egy adatbázis adatmaszkolását.

### [Set-AzSqlDatabaseDataMaskingRule](Set-AzSqlDatabaseDataMaskingRule.md)
Beállítja egy adatbázis adatmaszkolási szabályának tulajdonságait.

### [Set-AzSqlDatabaseFailoverGroup](Set-AzSqlDatabaseFailoverGroup.md)
Módosítja egy Azure SQL-adatbázis feladatátvételi csoportjának konfigurációját.

### [Set-AzSqlDatabaseGeoBackupPolicy](Set-AzSqlDatabaseGeoBackupPolicy.md)
Az adatbázis földrajzi biztonsági mentésének házirendet állít be.

### [Set-AzSqlDatabaseInstanceFailoverGroup](Set-AzSqlDatabaseInstanceFailoverGroup.md)
Módosítja egy példány feladatátvételi csoportjának konfigurációját.

### [Set-AzSqlDatabaseRecomstateActionState](Set-AzSqlDatabaseRecommendedActionState.md)
Frissíti az Azure SQL-adatbázis ajánlott műveletének állapotát.

### [Set-AzSqlDatabaseSecondary](Set-AzSqlDatabaseSecondary.md)
A feladatátvétel kezdeményezése érdekében elsődlegesre vált egy másodlagos adatbázist.

### [Set-AzSqlDatabaseSensitivityClassification](Set-AzSqlDatabaseSensitivityClassification.md)
Beállítja az adatbázis oszlopainak információs típusait és bizalmasság-címkéit.

### [Set-AzSqlDatabaseTransparentDataEncryption](Set-AzSqlDatabaseTransparentDataEncryption.md)
Módosítja egy adatbázis TDE tulajdonságát.

### [Set-AzSqlDatabaseVulnerabilityAssessmentRuleBaseline](Set-AzSqlDatabaseVulnerabilityAssessmentRuleBaseline.md)
Beállítja a biztonsági rés felmérésére vonatkozó szabály alapértékét.

### [Set-AzSqlElasticJob](Set-AzSqlElasticJob.md)
Feladat frissítése

### [Set-AzSqlElasticAgent](Set-AzSqlElasticJobAgent.md)
Rugalmas ügynök frissítése

### [Set-AzSqlElasticCredential](Set-AzSqlElasticJobCredential.md)
A feladat hitelesítő adatainak frissítése

### [Set-AzSqlElasticJobStep](Set-AzSqlElasticJobStep.md)
Feladat lépésének frissítése

### [Set-AzSqlElasticPool](Set-AzSqlElasticPool.md)
Módosítja egy rugalmas adatbáziskészlet tulajdonságait az Azure SQL-adatbázisban.

### [Set-AzSqlElasticPoolAdvisorAutoExecuteStatus](Set-AzSqlElasticPoolAdvisorAutoExecuteStatus.md)
Frissíti az Azure SQL rugalmassági készlet tanácsadója automatikus végrehajtásának állapotát.

### [Set-AzSqlElasticPoolRecomstateActionState](Set-AzSqlElasticPoolRecommendedActionState.md)
Frissíti az Ajánlott Azure SQL-rugalmassági készlet állapotát.

### [Set-AzSqlInstance](Set-AzSqlInstance.md)
Beállítja egy Azure SQL-adatbázis felügyelt példányának tulajdonságait.

### [Set-AzSqlInstanceActiveDirectoryAdministrator](Set-AzSqlInstanceActiveDirectoryAdministrator.md)
Azure AD-rendszergazdát hoz létre az SQL Felügyelt példányhoz.

### [Set-AzSqlInstanceDatabaseBackupLongTermRetentionPolicy](Set-AzSqlInstanceDatabaseBackupLongTermRetentionPolicy.md)
A **Set-AzSqlInstanceDatabaseLongTermRetentionBackup** parancsmag beállítja a felügyelt adatbázis hosszú távú adatmegőrzési házirendet.

### [Set-AzSqlInstanceDatabaseBackupShortTermRetentionPolicy](Set-AzSqlInstanceDatabaseBackupShortTermRetentionPolicy.md)
Biztonsági másolatot készít rövid távú adatmegőrzési házirendről.

### [Set-AzSqlInstanceDatabaseSensitivityClassification](Set-AzSqlInstanceDatabaseSensitivityClassification.md)
Beállítja az Azure SQL felügyeltpéldány-adatbázisban az oszlopok adattípusát és bizalmasság-címkéit.

### [Set-AzSqlInstanceDatabaseVulnerabilityAssessmentRuleBaseline](Set-AzSqlInstanceDatabaseVulnerabilityAssessmentRuleBaseline.md)
Beállítja a biztonsági rés felmérésére vonatkozó szabály alapértékét.

### [Set-AzSqlInstancePool](Set-AzSqlInstancePool.md)
Beállítja egy Azure SQL-példánykészlet tulajdonságait.

### [Set-AzSqlInstanceTransparentDataEncryptionProtector](Set-AzSqlInstanceTransparentDataEncryptionProtector.md)
Az ÁTLÁTSZÓ adattitkosítás (TDE) védelmét állítja be egy SQL-kezelt példányhoz.

### [Set-AzSqlServer](Set-AzSqlServer.md)
Módosítja egy SQL-adatbázis-kiszolgáló tulajdonságait.

### [Set-AzSqlServerActiveDirectoryAdministrator](Set-AzSqlServerActiveDirectoryAdministrator.md)
Azure AD-rendszergazdát hoz létre az SQL Serverhez.

### [Set-AzSqlServerAdvisorAutoExecuteStatus](Set-AzSqlServerAdvisorAutoExecuteStatus.md)
Frissíti egy Azure SQL Server-tanácsadó automatikus végrehajtási állapotát.

### [Set-AzSqlServerAudit](Set-AzSqlServerAudit.md)
Egy Azure SQL-kiszolgáló naplózási beállításainak módosítása.

### [Set-AzSqlServerDisasterRecoveryConfiguration](Set-AzSqlServerDisasterRecoveryConfiguration.md)
Módosítja az adatbázis-kiszolgáló helyreállítási konfigurációját.

### [Set-AzSqlServerDnsAlias](Set-AzSqlServerDnsAlias.md)
Módosítja azt a kiszolgálót, amelyre az Azure SQL Server DNS-aliasa mutat

### [Set-AzSqlServerFirewallRule](Set-AzSqlServerFirewallRule.md)
Módosít egy tűzfalszabályt az Azure SQL-adatbázis-kiszolgálón.

### [Set-AzSqlServerMSSupportAudit](Set-AzSqlServerMSSupportAudit.md)
Egy Azure SQL-adatbázis Microsoft támogatási műveleteinek naplózási beállításait módosítja.

### [Set-AzSqlServerRecomstateActionState](Set-AzSqlServerRecommendedActionState.md)
Frissíti az Ajánlott Azure SQL Server-művelet állapotát.

### [Set-AzSqlServerTransparentDataEncryptionProtector](Set-AzSqlServerTransparentDataEncryptionProtector.md)
Az átlátszó adattitkosítás (TDE) védelmét állítja be egy SQL-kiszolgálóhoz.

### [Set-AzSqlServerVirtualNetworkRule](Set-AzSqlServerVirtualNetworkRule.md)
Módosítja egy Azure SQL Server virtuális hálózati szabály konfigurációját.

### [Start-AzSqlDatabaseExecuteIndexRecommendation](Start-AzSqlDatabaseExecuteIndexRecommendation.md)
Elindítja az ajánlott indexműveletet futtató munkafolyamatot.

### [Start-AzSqlDatabaseVulnerabilityAssessmentScan](Start-AzSqlDatabaseVulnerabilityAssessmentScan.md)
Biztonsági vizsgálat indítása.

### [Start-AzSqlElasticJob](Start-AzSqlElasticJob.md)
Elindít egy feladatot, és visszaad egy feladatvégrehajtási azonosítót, amely az állapotának megtekintéséhez lekérdezható

### [Start-AzSqlInstanceDatabaseLogReplay](Start-AzSqlInstanceDatabaseLogReplay.md)
Elindítja a napló visszajátszása szolgáltatást a megadott paraméterekkel.

### [Start-AzSqlInstanceDatabaseVulnerabilityAssessmentScan](Start-AzSqlInstanceDatabaseVulnerabilityAssessmentScan.md)
Biztonsági vizsgálat indítása.

### [Start-AzSqlSyncGroupSync](Start-AzSqlSyncGroupSync.md)
Szinkronizálási csoportszinkronizálás indítása.

### [Stop-AzSqlDatabaseActivity](Stop-AzSqlDatabaseActivity.md)
Megszakítja az adatbázis aszinkron frissítésének műveletét.

### [Stop-AzSqlDatabaseExecuteIndexRecommendation](Stop-AzSqlDatabaseExecuteIndexRecommendation.md)
Leállítja az ajánlott indexműveletet futtató munkafolyamatot.

### [Stop-AzSqlElasticJob](Stop-AzSqlElasticJob.md)
Leállít egy feladatot, mert az feladatvégrehajtási azonosítója

### [Stop-AzSqlElasticPoolActivity](Stop-AzSqlElasticPoolActivity.md)
Megszakítja a rugalmas készlet aszinkron frissítési műveletét.

### [Stop-AzSqlInstanceDatabaseLogReplay](Stop-AzSqlInstanceDatabaseLogReplay.md)
Megszakítja a Napló visszajátszása szolgáltatást az adatbázis leejtésében.

### [Stop-AzSqlInstanceOperation](Stop-AzSqlInstanceOperation.md)
Leállítja egy SQL által felügyelt példány műveleteit.

### [Stop-AzSqlSyncGroupSync](Stop-AzSqlSyncGroupSync.md)
Leállítja a szinkronizálási csoport szinkronizálását.

### [Suspend-AzSqlDatabase](Suspend-AzSqlDatabase.md)
Sql Data Warehouse-adatbázis felfüggesztése.

### [Switch-AzSqlDatabaseFailoverGroup](Switch-AzSqlDatabaseFailoverGroup.md)
Az Azure SQL-adatbázis feladatátvételi csoportjának feladatátvételét hajtja végre.

### [Switch-AzSqlDatabaseInstanceFailoverGroup](Switch-AzSqlDatabaseInstanceFailoverGroup.md)
Egy példány feladatátvételi csoportjának feladatátvételét hajtja végre.

### [Update-AzSqlDatabaseAdvancedThreatProtectionSetting](Update-AzSqlDatabaseAdvancedThreatProtectionSetting.md)
Speciális veszélyforrás-védelmi beállításokat ad meg egy adatbázishoz.

### [Update-AzSqlDatabaseVulnerabilityAssessmentSetting](Update-AzSqlDatabaseVulnerabilityAssessmentSetting.md)
Frissíti egy adatbázis biztonsági rés-felmérési beállításait.

### [Update-AzSqlInstanceDatabaseVulnerabilityAssessmentSetting](Update-AzSqlInstanceDatabaseVulnerabilityAssessmentSetting.md)
Frissíti a felügyelt adatbázisok biztonsági résértékelési beállításait.

### [Update-AzSqlInstanceVulnerabilityAssessmentSetting](Update-AzSqlInstanceVulnerabilityAssessmentSetting.md)
Frissíti a felügyelt példányok biztonsági résértékelési beállításait.

### [Update-AzSqlServerAdvancedThreatProtectionSetting](Update-AzSqlServerAdvancedThreatProtectionSetting.md)
Speciális veszélyforrás-védelmi beállításokat ad meg egy kiszolgálón.

### [Update-AzSqlServerVulnerabilityAssessmentSetting](Update-AzSqlServerVulnerabilityAssessmentSetting.md)
Frissíti a kiszolgáló biztonsági résértékelési beállításait.

### [Update-AzSqlSyncGroup](Update-AzSqlSyncGroup.md)
Frissíti az Azure SQL Database Sync-csoportot.

### [Update-AzSqlSyncMember](Update-AzSqlSyncMember.md)
Frissíti az Azure SQL Database Sync egyik tagját.

### [Update-AzSqlSyncSchema](Update-AzSqlSyncSchema.md)
Frissítse a szinkronizálási séma szinkronizálási tagadatbázis vagy szinkronizálási központ adatbázisát.
Be fogja szerezni a legújabb adatbázissémát a valós adatbázisból, majd azt használva frissíti a szinkronizálási metaadat-adatbázis által gyorsítótárazott sémát.
Ha a "SyncMemberName" érték meg van adva, az frissíti a tagadatbázis-sémát; ha nem, akkor frissíti a központi adatbázissémát.

