...
A társzolgáltatás hitelesítésének részleteit itt https://docs.microsoft.com/en-us/rest/api/storageservices/authorization-for-the-azure-storage-services olvashatja.
...
Ez a parancs a helyi fejlesztési tárfiók használatával létrehoz egy környezetet, majd a folyamat műveleti operátorával átadja az új környezetnek a **Get-AzStorageContainer** parancsmagnak.
...
Az utolsó parancs a $Context 01-ben és a $Context 02-ben a **Get-AzStorageContainer** használatával tárolt környezetek tárolóit kapja meg.
...
Az első parancs egy SAS-jogkivonatot generál a ContosoMain nevű tároló **New-AzStorageContainerSASToken** parancsmagját használva, majd a $SasToken tárolja.
Ez a jogkivonat olvasási, hozzáadási, frissítési és törlési engedélyekhez szükséges.
A második parancs létrehoz egy környezetet a ContosoGeneral nevű fiókhoz, amely a $SasToken-ban tárolt SAS-jogkivonatot használja, majd a környezet a $Context változóban tárolja.
A végleges parancs felsorolja a ContosoMain nevű tárolóhoz társított összes blobot a $Context.
...
Ez a parancsmag kontextust hoz létre a paraméter által megadott fiókhoz.
...
### -UseConnectedAccount
Azt jelzi, hogy ez a parancsmag Azure Storage környezetben hoz létre OAuth (Azure AD) hitelesítéssel.
