---
Module Name: Az.Websites
Module Guid: cc69c625-e961-43f4-8b50-0061eba6e4b6
Download Help Link: https://docs.microsoft.com/en-us/powershell/module/az.websites
Help Version: 1.0.0.0
Locale: en-US
content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Websites/Websites/help/Az.Websites.md
original_content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Websites/Websites/help/Az.Websites.md
ms.openlocfilehash: e21d0de8ee0d0fc407208e7d5ef899147670a0aa
ms.sourcegitcommit: 68451baa389791703e666d95469602c5652609ee
ms.translationtype: MT
ms.contentlocale: hu-HU
ms.lasthandoff: 01/05/2021
ms.locfileid: "98467039"
---
# Az.Websites modul
## Leírás
ARM (Azure Resource Manager) Web App és App Service Plan parancsai.

## Az.Websites parancsmagok
### [Add-AzWebAppAccessRestrictionRule](Add-AzWebAppAccessRestrictionRule.md)
Access-restiction szabályt ad hozzá egy Azure Web Apphoz.

### [Add-AzWebAppTrafficRouting](Add-AzWebAppTrafficRouting.md)
Adjon hozzá egy útválasztási szabályt a slothoz.

### [Edit-AzWebAppBackupConfiguration](Edit-AzWebAppBackupConfiguration.md)


### [Enter-AzWebAppContainerPSSession](Enter-AzWebAppContainerPSSession.md)
Távoli PowerShell-munkamenet megnyitása egy adott webhelyen vagy tárolóhelyen és erőforráscsoportban megadott windows-tárolóban

### [Get-AzAppServicePlan](Get-AzAppServicePlan.md)
Egy Azure App Service-tervet kap a megadott erőforráscsoportban.

### [Get-AzDeletedWebApp](Get-AzDeletedWebApp.md)
Törlődnek a webalkalmazások az előfizetésből.

### [Get-AzWebApp](Get-AzWebApp.md)
Az Azure Web Apps alkalmazást a megadott erőforráscsoportban kapja meg.

### [Get-AzWebAppAccessRestrictionConfig](Get-AzWebAppAccessRestrictionConfig.md)
Gets Access Restiction configuration for an Azure Web App.

### [Get-AzWebAppBackup](Get-AzWebAppBackup.md)


### [Get-AzWebAppBackupConfiguration](Get-AzWebAppBackupConfiguration.md)


### [Get-AzWebAppBackupList](Get-AzWebAppBackupList.md)


### [Get-AzWebAppCertificate](Get-AzWebAppCertificate.md)
Azure Web App-tanúsítványt kap.

### [Get-AzWebAppContainerContinuousDeploymentUrl](Get-AzWebAppContainerContinuousDeploymentUrl.md)
Get-AzWebAppContainerContinuousDeploymentUrl tároló folyamatos telepítési URL-címét adja vissza

### [Get-AzWebAppPublishingProfile](Get-AzWebAppPublishingProfile.md)
Azure Web App közzétételi profilt kap.

### [Get-AzWebAppszaj](Get-AzWebAppSlot.md)
Egy Azure Web App-slotot kap.

### [Get-AzWebAppSzajConfigName](Get-AzWebAppSlotConfigName.md)
A Web App Slot config-nevek listájának lekérte

### [Get-AzWebAppSzajPublishingProfile](Get-AzWebAppSlotPublishingProfile.md)
Egy Azure Web App-slot közzétételi profilt kap.

### [Get-AzWebAppSnapshot](Get-AzWebAppSnapshot.md)
Elérhetővé teszi a pillanatfelvételeket egy webalkalmazáshoz.

### [Get-AzWebAppSSLBinding](Get-AzWebAppSSLBinding.md)
Azure Web App-tanúsítvány SSL-kötést kap.

### [Get-AzWebAppTrafficRouting](Get-AzWebAppTrafficRouting.md)
Routing Rule for the given Slot name.Get a routing Rule for the given Slot name.

### [New-AzAppServicePlan](New-AzAppServicePlan.md)
Azure App Service-előfizetést hoz létre egy adott földrajzi helyen.

### [New-AzWebApp](New-AzWebApp.md)
Létrehoz egy Azure Web Appot.

### [New-AzWebAppAzureStoragePath](New-AzWebAppAzureStoragePath.md)
Olyan objektumot hoz létre, amely egy Web Appban felvehető Azure Storage elérési utat képvisel. Paraméterként (-AzureStoragePath) való használatra szolgál a Set-AzWebApp és Set-AzWebAppSlot

### [New-AzWebAppBackup](New-AzWebAppBackup.md)


### [New-AzWebAppContainerPSSession](New-AzWebAppContainerPSSession.md)
New-AzWebAppContainerPSSession új távoli PowerShell-munkamenetet hoz létre egy adott webhelyen vagy helyen, illetve erőforráscsoportban megadott windowsos tárolóban

### [New-AzWebAppDatabaseBackupSetting](New-AzWebAppDatabaseBackupSetting.md)


### [New-AzWebAppszaj](New-AzWebAppSlot.md)
Létrehoz egy Azure Web App-helyet.

### [New-AzWebAppSSLBinding](New-AzWebAppSSLBinding.md)
Ssl-tanúsítványkötést hoz létre egy Azure Web Apphoz.

### [Publish-AzWebApp](Publish-AzWebApp.md)
Zip, JAR vagy WAR fájlból telepíti az Azure Web Appot zipdeploy használatával. 

### [Remove-AzAppServicePlan](Remove-AzAppServicePlan.md)
Eltávolít egy Azure App Service-előfizetést.

### [Remove-AzWebApp](Remove-AzWebApp.md)
Eltávolít egy Azure Web Appot.

### [Remove-AzWebAppAccessRestrictionRule](Remove-AzWebAppAccessRestrictionRule.md)
Eltávolít egy hozzáférési korlátozási szabályt egy Azure Web Appból.

### [Remove-AzWebAppBackup](Remove-AzWebAppBackup.md)


### [Remove-AzWebAppszaj](Remove-AzWebAppSlot.md)


### [Remove-AzWebAppSSLBinding](Remove-AzWebAppSSLBinding.md)
Eltávolít egy SSL-kötést egy feltöltött tanúsítványból.

### [Remove-AzWebAppTrafficRouting](Remove-AzWebAppTrafficRouting.md)
Útválasztási szabály eltávolítása a tárolóhelyről.

### [Reset-AzWebAppPublishingProfile](Reset-AzWebAppPublishingProfile.md)


### [Reset-AzWebAppSzajPublishingProfile](Reset-AzWebAppSlotPublishingProfile.md)


### [Restart-AzWebApp](Restart-AzWebApp.md)
Egy Azure Web App újraindítása.

### [Restart-AzWebAppszaj](Restart-AzWebAppSlot.md)


### [Restore-AzDeletedWebApp](Restore-AzDeletedWebApp.md)
Törölt webalkalmazás visszaállítása új vagy meglévő webalkalmazásra.

### [Restore-AzWebAppBackup](Restore-AzWebAppBackup.md)


### [Restore-AzWebAppSnapshot](Restore-AzWebAppSnapshot.md)
Webalkalmazás pillanatképének visszaállítása.

### [Set-AzAppServicePlan](Set-AzAppServicePlan.md)
Beállítja az Azure App Service-előfizetést.

### [Set-AzWebApp](Set-AzWebApp.md)
Egy Azure Web Appot módosít.

### [Set-AzWebAppszaj](Set-AzWebAppSlot.md)
Módosít egy Azure Web App-helyet.

### [Set-AzWebAppSzajConfigName](Set-AzWebAppSlotConfigName.md)
Set Web App Slot Config names

### [Start-AzWebApp](Start-AzWebApp.md)
Elindít egy Azure Web Appot.

### [Start-AzWebAppszaj](Start-AzWebAppSlot.md)
Elindít egy Azure Web App-helyet.

### [Stop-AzWebApp](Stop-AzWebApp.md)
Leállítja az Azure Web Appot.

### [Stop-AzWebAppszaj](Stop-AzWebAppSlot.md)
Leállítja az Azure Web App-tárolóhelyet.

### [Switch-AzWebAppszaj](Switch-AzWebAppSlot.md)
Két bővítőhely felcserélve webalkalmazással

### [Update-AzWebAppAccessRestrictionConfig](Update-AzWebAppAccessRestrictionConfig.md)
Frissíti egy Azure Web App SCM-webhelyére vonatkozó Főwebhely Hozzáférés restiction konfigját.

### [Update-AzWebAppTrafficRouting](Update-AzWebAppTrafficRouting.md)
Továbbítási szabály frissítése a slotra.

