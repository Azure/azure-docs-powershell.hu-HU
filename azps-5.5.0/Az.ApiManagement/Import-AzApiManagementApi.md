...
## LEÍRÁS
Az **Import-AzApiManagementApi** parancsmag importálja az Azure API Felügyeleti API-t egy fájlból vagy egy URL-címből a webalkalmazás leírásának nyelve (WADL), a webszolgáltatások leírási nyelve (WSDL) vagy a Swagger formátumban.
...
Ez a parancs egy API-t importál a megadott WADL-fájlból.

### 2. példa: API importálása Swagger-fájlból
```powershell
PS C:\>$ApiMgmtContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
Ez a parancs egy API-t importál a megadott Swagger-fájlból.
...
Ez a parancs egy API-t importál a megadott WADL-csatolásból.

### 4. példa: API importálása Open Api-hivatkozásból
```powershell
...
Ez a parancs egy API-t importál a megadott Open 3.0 specifikációkapcsolatból.

### 5. példa: API importálása Open Api-hivatkozásból ApiVersion-készletbe
...
Ez a parancs importál egy API-t a megadott Open 3.0 specifikációs dokumentumból, és létrehoz egy új ApiVersiont.

## PARAMETERS

### -ApiId
Megadja az importálni megfelelő API azonosítóját.
...
### -ApiRevision
Az API-változat azonosítója. Ez a paraméter nem kötelező. Ha nincs megadva, az importálás az aktuálisan aktív változatra vagy egy új api-ra történik.
...
### -ApiType
Ez a paraméter nem kötelező a Http alapértelmezett értékkel. A Soap beállítás csak WSDL importáló esetén alkalmazható, és SOAP Átadó API-t hoz létre.
...
### -Path
Egy webes API-elérési utat ad meg az API nyilvános URL-címének utolsó részeként.
Ezt az URL-címet használják az API-t használó ügyfelek arra, hogy kéréseket küldenek a webszolgáltatásnak.
...
### -Protocol
Web API-protokollok (http, https). Protokollok, amelyeken keresztül az API elérhetővé válik. Ez a paraméter nem kötelező. Ha fel van adva, az felülírja a specifikációs dokumentumban megadott protokollokat.
...
### -ServiceUrl
A webszolgáltatás API-t kitűnő URL-címe. Ezt az URL-címet csak az Azure API Management fogja használni, és nem teszi nyilvánosra. Ez a paraméter nem kötelező. Ha fel van adva, az felülírja a Specifikációk dokumentumban megadott ServiceUrl szabványt.
...
psdx_paramvalues Wadl, Wsdl és Swagger.
...
### -SpecificationPath
A specifikáció fájl elérési útját adja meg.
...
### -WsdlEndpointName
Az importálni szánt WSDL-végpont (port) helyi neve. 1–400 karakter hosszúnak kell lennie. Ez a paraméter nem kötelező, és csak a Wsdl importálásához szükséges. Az alapértelmezett érték $null.
...
### -WsdlServiceName
Az importálni szánt WSDL-szolgáltatás helyi neve. 1–400 karakter hosszúnak kell lennie. Ez a paraméter nem kötelező, és csak a Wsdl importálásához szükséges. Az alapértelmezett érték $null.
