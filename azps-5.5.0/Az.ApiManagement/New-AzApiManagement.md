...
## LEÍRÁS
A **New-AzApiManagement** parancsmag api-kezelési telepítést hoz létre az Azure API Managementben.

## PÉLDÁK

### 1. példa: Fejlesztői szintű API-kezelő szolgáltatás létrehozása
```powershell
...
Ez a parancs fejlesztői szintű API-kezelő szolgáltatást hoz létre.
A parancs megadja a szervezetet és a rendszergazda címét.
A parancs nem adja meg az *SKU paramétert.*
Ezért a parancsmag a Fejlesztőeszközök alapértelmezett értékét használja.

### 2. példa: Standard szintű szolgáltatás létrehozása három egységből
```powershell
PS C:\>New-AzApiManagement -ResourceGroupName "ContosoGroup02" -Name "ContosoApi" -Location "Central US" -Organization "Contoso" -AdminEmail "admin@contoso.com" -Sku Standard -Capacity 3
```

Ez a parancs létrehoz egy standard szintű API-kezelő szolgáltatást, amely három egységből áll.

### 3. példa: Fogyasztási szint szolgáltatás létrehozása
```powershell
...
Ez a parancs létrehoz egy felhasználásszintű API-kezelési szolgáltatást, amelynél engedélyezve van az ügyfél tanúsítványa Nyugat-Európában.
...
Ez a parancs egy prémium szintű API-kezelési szolgáltatást hoz létre egy Azure virtuális hálózati alhálózatban, amelynek külső átjáró végpontja van egy fő régióval az Egyesült Államok nyugati részén.

### 5. példa: API-kezelési szolgáltatás létrehozása belső virtuális hálózathoz
```powershell
PS C:\> $virtualNetwork = New-AzApiManagementVirtualNetwork -Location "West US" -SubnetResourceId "/subscriptions/a8ff56dc-3bc7-4174-b1e8-3726ab15d0e2/resourceGroups/ContosoGroup/providers/Microsoft.Network/virtualNetworks/westUsVirtualNetwork/subnets/backendSubnet"
PS C:\> New-AzApiManagement -ResourceGroupName "ContosoGroup" -Location "West US" -Name "ContosoApi" -Organization "Contoso" -AdminEmail "admin@contoso.com" -VirtualNetwork $virtualNetwork -VpnType "Internal" -Sku "Premium"
```

Ez a parancs egy prémiumszintű API-kezelési szolgáltatást hoz létre egy Azure virtuális hálózat alhálózatában, amelynek belső átjáró végpontja van egy fő régióval az Egyesült Államok nyugati részén.

### 6. példa: API-kezelési szolgáltatás létrehozása és TLS 1.0 protokoll engedélyezése
```powershell
...
Ez a parancs létrehoz egy szokásos termékváltozat api-kezelési szolgáltatást, és engedélyezi a TLS 1.0-t az Előlapi ügyfélprogramban az ApiManagement Átjáró és a Backend ügyfélprogram között az ApiManagement átjáró és a Backend között.

## PARAMETERS

### -AdditionalRegions
Az Azure API Management további telepítési régiói.
...
### -AdminEmail
Az API-kezelő rendszer által küldött összes értesítés eredeti e-mail-címét adja meg.
...
### -Kapacitás
Az Azure API Felügyeleti szolgáltatás termékváltozat-kapacitását határozza meg.
...
Egyéni állomásnév-konfigurációk. Az alapértelmezett érték $null. Ha $null adja meg az alapértelmezett állomásnevet.
...
### -EnableClientCertificate
Flag only meant to be used for Consumption SKU ApiManagement Service. Ez azt kényszeríti, hogy az ügyfél tanúsítványa minden, az átjárónak szóló kérésnél érvénybe lép. Ez lehetővé teszi a tanúsítvány hitelesítését is az átjáró házirendjében.
...
### -Location
Megadja az Api Management szolgáltatás létrehozására használt helyet.
...
### -Name
Megadja az API-kezelés telepítésének nevét.
...
Az API-kezelés ezt a címet használja a fejlesztői portálon az e-mail-értesítésekben.
...
### -ResourceGroupName
Annak az erőforráscsoportnak a nevét adja meg, amely alatt ez a parancsmag API-kezelési telepítést hoz létre.
...
### -Termékváltozat
Az API-kezelési szolgáltatás rétegét határozza meg.
Érvényes értékek: 
- Fejlesztő 
- Normál 
- Premium Az alapértelmezett beállítás a Fejlesztőeszközök.
...
### -SslSetting
Az ApiManagement szolgáltatás Ssl-beállítása. Az alapértelmezett érték $null
...
Azure Active Directory-identitás létrehozása és hozzárendelése ehhez a kiszolgálóhoz olyan kulcskezelési szolgáltatásokhoz, mint az Azure KeyVault.
...
A szolgáltatásra telepítve a belső hitelesítésszolgáltató által kibocsátott tanúsítványok. Az alapértelmezett érték $null.
...
Felhasználói identitásokat rendelhet ehhez a kiszolgálóhoz, hogy olyan kulcskezelő szolgáltatásokkal használva használja őket, mint az Azure KeyVault.
...
### -VpnType
Az ApiManagement-telepítés virtuális hálózattípusa. Érvényes értékek: 
- "Nincs" (alapértelmezett érték. Az ApiManagement nem része semmilyen virtuális hálózatnak")
- "Külső" (Az ApiManagement-telepítés egy internetes végponttal rendelkező virtuális hálózaton belül van beállítva)
- "Belső" (az ApiManagement-telepítés egy intranetes végponttal rendelkező virtuális hálózaton belül van beállítva)
