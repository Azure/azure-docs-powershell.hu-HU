...
## SYNOPSIS
Létrehoz egy konfigurációs tárolót a megadott paraméterekkel.
...
### 1. példa: Alkalmazáskonfigurációs áruház létrehozása
```powershell
...
Ez a parancs létrehoz egy alkalmazáskonfigurációs áruházat.

### 2. példa: Appkonfiguráció létrehozása a "UserAssigned" IdentityType-val
```powershell
...
Ez a parancs létrehoz egy appkonfigurációt, és hozzárendel hozzá egy felhasználó által hozzárendelt felügyelt identitást.
A cmk (cusomer managed key) engedélyezéséhez kövesse az alábbi `Update-AzAppConfigurationStore` lépéseket.

### 3. példa: Alkalmazáskonfiguráció létrehozása a "SystemAssigned" IdentityType-val 
```powershell
...
Ez a parancs létrehoz egy appkonfigurációt, és engedélyezi az erőforráshoz társított, rendszerhez rendelt felügyelt identitást.
A CMK (cusomer managed key) engedélyezéséhez kövesse az `Update-AzAppConfigurationStore` alábbi lépéseket.

### 4. példa: Appkonfiguráció létrehozása úgy, hogy az IdentityType beállítása "SystemAssigned, UserAssigned"
```powershell
PS C:\> $assignedIdentity = New-AzUserAssignedIdentity -ResourceGroupName azpwsh-manual-test -Name assignedIdentity
...
Engedélyezheti a rendszer által hozzárendelt felügyelt identitást, és egyidejűleg adhat felhasználói identitásokat is.
A CMK (cusomer managed key) engedélyezéséhez kövesse az `Update-AzAppConfigurationStore` alábbi lépéseket.
...
### -Location
Az erőforrás helye.
Ez az erőforrás létrehozása után nem módosítható.
