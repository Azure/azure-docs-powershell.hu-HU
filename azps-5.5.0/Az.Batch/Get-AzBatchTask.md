...
## LEÍRÁS
A **Get-AzBatchTask** parancsmag Azure Batch-feladatokat kap egy kötegfeladathoz.
Adjon meg egy feladatot a *JobId vagy* a *Job paraméter* alapján.
Egyetlen feladat végrehajtásához adja meg az *Azonosító paramétert.*
A Szűrő  paraméter megadásával lekértheti az OData-szűrőnek megfelelő feladatokat.

## PÉLDÁK

### 1. példa: Feladat lekérte azonosító szerint
```
...
Ez a parancs a Feladat01 feladat alatt a Feladat03 azonosítóval kapja meg a feladatot.
A Get-AzBatchAccountKey parancsmag használatával kontextust rendelhet a $Context változóhoz.

### 2. példa: Az összes befejezett tevékenység lekérte egy adott feladatból
```
...
Ez a parancs az id Job02 azonosítójú feladatból kapja meg az elkészült feladatokat.

## PARAMETERS

### -BatchContext
Azt a **BatchAccountContext példányt** adja meg, amely a parancsmagot használja a Batch szolgáltatással való interakcióhoz.
Ha a Get-AzBatchAccount parancsmaggal szerezze be a BatchAccountContext parancsmagot, akkor az Azure Active Directory-hitelesítést fogja használni a Batch szolgáltatás használata során. Ha ehelyett megosztott kulcsú hitelesítést használ, a Get-AzBatchAccountKey parancsmaggal töltse ki a BatchAccountContext objektumot a hozzáférési kulcsokkal. Megosztott kulcs hitelesítése esetén a rendszer alapértelmezés szerint az elsődleges hozzáférési kulcsot használja. A használni használt kulcs módosítása a BatchAccountContext.KeyInUse tulajdonsággal.
...
A paraméter értékét megadva lekérte a fő entitás társított entitását.
...
Ha nem ad meg szűrőt, ez a parancsmag a Batch-fiók vagy -feladat összes feladatát visszaadja.
...
### -JobId
Annak a feladatnak az azonosítóját adja meg, amely a parancsmag által lekért feladatokat tartalmazza.
...
### -MaxCount
Azt adja meg, hogy legfeljebb hány tevékenység adható vissza.
