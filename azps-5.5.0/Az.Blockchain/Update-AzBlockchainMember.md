...
### 1. példa: Blockchain-tag frissítése
```powershell
...
Ez a parancs frissíti a blockchain egyik tagját.

### 2. példa: Blockchain-tag frissítése
```powershell
...
### -FirewallRule
Beállítja vagy beállítja a tűzfal szabályait.
Ennek létrehozásáról a FIREWALLRULE tulajdonságokat és a kivonattáblát a NOTES (JEGYZETEK) című szakaszban láthatja.
...
### -Password
Beállítja a tranzakciós csomópont dns-végpontjának alapvető hitelesítésjelszóját.
...
FIREWALLRULE <IFirewallRule[]>: Beállítja vagy beállítja a tűzfal szabályait.
  ...
  - `[StartIPAddress <String>]`: Beállítja vagy beállítja a tűzfalszabály tartományának kezdő IP-címét.
...
INPUTOBJECT: <IBlockchainIdentity> Identity Parameter
  ...
  - `[SubscriptionId <String>]`: A Microsoft Azure-előfizetést egyedileg azonosító előfizetésazonosítót kap. Az előfizetésazonosító minden szolgáltatási hívás URI-jának része.
