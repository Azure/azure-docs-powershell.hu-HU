...
## LEÍRÁS
A **New-AzDiskUpdateConfig parancsmag** létrehoz egy konfigurálható lemezfrissítési objektumot.
...
Az első parancs egy 10 GB-os helyi üres lemezfrissítési objektumot hoz létre Premium_LRS tárfiók típusában. Emellett beállítja a Windows operációs rendszer típusát, és engedélyezi a titkosítási beállításokat. A második és a harmadik parancs a lemezfrissítési objektum lemeztitkosítási kulcsának és kulcstitkosítási kulcsának beállításait adja meg.
Az utolsó parancs átveszi a lemezfrissítési objektumot, és frissíti a "Disk01" nevű meglévő lemez nevét az "ResourceGroup01" erőforráscsoportban.
...
Ez a parancs 10 GB-os lemezméretre frissíti az "ResourceGroup01" erőforráscsoport "Disk01" nevű meglévő lemezét.

## PARAMETERS

### -TetszetvesEnabled
Lehetővé teszi, hogy a program a kiépített teljesítménycélon túlra sorozatos sorozatot létesítsen a lemezen. A sorozatfelvétel alapértelmezés szerint le van tiltva. Az Ultra lemezre nem érvényes.
...
### -DiskIOPSReadOnly
Az összes olyan VM-hez engedélyezett IOPS-fájlok száma, amelyek a megosztott lemezhez ReadOnly-ként vannak rögzítve. Egy művelet 4 és 256 bájt közötti átvitelt tud.
...
### -DiskIOPSReadWrite
Az ehhez a lemezhez engedélyezett IOPS-k száma; csak az UltraSSD-lemezhez van beállítva. Egy művelet 4 és 256 bájt közötti átvitelt tud.
...
### -DiskMBpsReadOnly
Az a teljes átviteli sebesség (MBps), amely az összes olyan virtuális gép számára engedélyezett, amely a megosztott lemezhez ReadOnly átvitelt rögzít. A MBps másodpercenként több millió bájtot jelent – a MB itt a 10-eshatalmak ISO-ként való meghatalmazását használja.
...
### -DiskMBpsReadWrite
A lemezhez engedélyezett sávszélesség; csak az UltraSSD-lemezhez van beállítva. A MBps másodpercenként több millió bájtot jelent – a MB itt a 10-eshatalmak ISO-ként való meghatalmazását használja.
...
### -MaxSharesCount
A lemezhez egyszerre csatolhat virtuális gépek maximális száma. Az egynél nagyobb érték olyan lemez, amely egyszerre több virtuális gépre is fel lehet szerelve.
