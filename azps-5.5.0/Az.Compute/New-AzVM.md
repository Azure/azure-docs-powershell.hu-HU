...
A virtuális gép konfigurálása más parancsmagokkal is használható, például a Set-AzVMOperatingSystem, a Set-AzVMSourceImage, az Add-AzVMNetworkInterface és a Set-AzVMOSDisk.
...
### 3. példa: Virtuális gép létrehozása nyilvános IP-cím nélküli piactéri képből
```
$VMLocalAdminUser = "LocalAdminUser"
$VMLocalAdminSecurePassword = ConvertTo-SecureString <password> -AsPlainText -Force
$LocationName = "westus"
...
### -AddressPrefix
A virtuális hálózat címelőtagja, amely a VM-hez jön létre.
...
### -EncryptionAtHost
A EncryptionAtHost tulajdonságot a felhasználó használhatja a kérésben a gazdatitkosítás engedélyezéséhez vagy letiltásához a virtuális gép vagy virtuális gép méretarányának beállítására. Ez lehetővé teszi a titkosítást az összes lemezen, beleértve magának az állomásnak az Erőforrás/Ideiglenes lemezét is. Alapértelmezett: A gazdaszámítógép titkosítása le lesz tiltva, hacsak ez a tulajdonság nincs igazra állítva az erőforrásra nézve.
