...
## LEÍRÁS
A **Set-AzDataFactoryPipelineActivePeriod** parancsmag konfigurálja az Azure Data Factory-folyamat által feldolgozott adatszeletek aktív időszakát.
Ha a Set-AzDataFactorySliceStatus-parancsmag használatával módosítja egy adatkészlet szeletének állapotát, ellenőrizze, hogy a szelet kezdési és záró ideje a folyamat aktív időszakában van-e.
A folyamat létrehozása után megadhatja az adatok feldolgozásának időszakát.
Egy folyamat aktív időszakának megadása meghatározza az adatszeletek feldolgozásának időtartamát  az egyes Data Factory-adatkészletek rendelkezésre állási tulajdonságai alapján.

## PÉLDÁK

### 1. példa: Az aktív időszak konfigurálása
```
...
Ez a parancs beállítja a DPWikisample nevű folyamat adatszeletei aktív időszakát.
A parancs értékként biztosítja az adatszeletek kezdő és záró pontjait.
A parancs egy számértéket ad $True.

## PARAMETERS

### -AutoResolve
Azt jelzi, hogy ez a parancsmag automatikus feloldást használ.
...
Ez a parancsmag módosítja egy olyan folyamat aktív időszakát, amely a paraméter által megadott adatüzembe tartozik.
...
Ez a parancsmag módosítja egy olyan folyamat aktív időszakát, amely a paraméter által megadott adatüzemhez tartozik.
...
Az adatok feldolgozása történik, vagy az adatszeletek feldolgozása ezen az időszakon belül történik.
...
### -ForceRecalculate
Azt jelzi, hogy a parancsmag kényszerített újraszámítást használ.
...
Ez a parancsmag beállítja a paraméter által megadott folyamat aktív időszakát.
...
Ez a parancsmag módosítja egy olyan folyamat aktív időszakát, amely a paraméter által megadott csoporthoz tartozik.
...
Az adatok feldolgozása történik, vagy az adatszeletek feldolgozása ezen az időszakon belül történik.
*A StartDateTime* értéket ISO8601 formátumban kell megadni.
