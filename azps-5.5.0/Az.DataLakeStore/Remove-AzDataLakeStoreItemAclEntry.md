...
## LEÍRÁS
A **Remove-AzDataLakeStoreItemAclEntry** parancsmag eltávolít egy bejegyzést (ACE) egy adattavatárban lévő fájl vagy mappa hozzáférés-vezérlési listájából (ACL).

## PÉLDÁK

### 1. példa: Felhasználói bejegyzés eltávolítása
```
...
Ez a parancs eltávolítja a Patti Fuller-felhasználó ACE-ét a ContosoADL-fiókból.

### 2. példa: Felhasználó bejegyzésének eltávolítása rekurzív módon
```
...
### 3. példa: Acl-objektum használatával rekurzív ACE-engedélyek eltávolítása
```
...
Ez a parancs eltávolítja a Patti Fuller-hez szükséges felhasználói ACE-t a legfelső szintűről, és rekurzív módon törli a ContosoADL-fiókhoz szükséges alkönyvtárakat és fájlokat.
...
### -AceType
Az eltávolítható ACE típusát határozza meg.
...
### -Acl
Az eltávolítható bejegyzéseket tartalmazó ACL-objektumot adja meg.
...
### -Alapértelmezett
Azt jelzi, hogy ez a művelet eltávolítja az alapértelmezett ACE-t a megadott ACL-ről.
...
### -Id
Annak az AzureActive Directory-felhasználónak, csoportnak vagy szolgáltatásnévnek az objektumazonosítóját adja meg, amelyről el szeretné távolítani az ACE-et.
...
### -Path
Annak az elemnek a Data Lake Store-elérési útját adja meg, amelyből el szeretné távolítani az ACE-t, a gyökérkönyvtártól (/).
...
### -Recurse
Azt jelzi, hogy az ACL-et rekurzív módon el kell távolítani a gyermek alkönyvtáraiból és fájljaiból.
...
### -ShowProgress
Ha a folyamat elért, akkor a folyamat állapota 2010-et mutat. Csak akkor alkalmazható, ha a rekurzív Acl eltávolítás kész.
