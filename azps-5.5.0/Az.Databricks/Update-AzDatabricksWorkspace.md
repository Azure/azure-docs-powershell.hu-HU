...
### 1. példa: Az Adatkapcsolatok munkaterület címkéinek frissítése
```powershell
...
Ez a parancs frissíti az Adatkapcsolatok munkaterület címkéit.

### 2. példa: Titkosítás engedélyezése Datab ssl-munkaterületen
```powershell
...
A titkosítás engedélyezése a Datab workspace-munkaterületen három lépésből áll:
1.
Frissítse a munkaterületet `-PrepareEncryption` (ha nem így hozták létre).
1.
Keresse `StorageAccountIdentityPrincipalId` meg az utolsó lépés kimenetét.
Adjon meg kulcsengedélyeket a tőketitkos számára.
1.
Frissítse ismét a munkaterületet a titkosítási kulcs információinak kitöltéséhez:
    - `-EncryptionKeySource`
    - `-EncryptionKeyVaultUri`
    - `-EncryptionKeyName`
    - `-EncryptionKeyVersion`

### 3. példa: Titkosítás letiltása Datab és Datab több munkaterületen
```powershell
...
A titkosítás letiltásához egyszerűen állítsa a `-EncryptionKeySource` `'Default'` .
...
Lehetséges értékek (a kis- és a nagy- és a kis- és a nagy- és a kis- és
