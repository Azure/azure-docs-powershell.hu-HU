...
A \` \` MyResourceGroupName erőforráscsoport Tartomány1 csoportjában sorolja fel az Eseményrács tartománytémakra vonatkozó témaköröket (ha vannak ilyen), amelyek kielégítik az \` $odataFilter 10 tartománytémát \` egyszerre. Ha több találat érhető el, a $result. A NextLink nem lesz $null. A tartománytémakémák következő oldalának lehívása érdekében a felhasználó várhatóan újra Get-AzEventGridDomainTopic és az eredményt használja. Az előző hívásból kapott NextLink. A hívónak meg kell állnia, amikor az eredmény álljon meg. A NextLink $null.
