...
### EventhubPropertiesSet (alapértelmezett)
```
...
## LEÍRÁS
A New-AzEventHub parancsmag létrehoz egy új Azure Event Hubot.
Az Eventhub létrehozásához a Leírás tulajdonságainak rögzítése tulajdonsággal, kérjük, kövesse az alábbi lépéseket (2. példa). 

## PÉLDÁK

### 1. példa: Új EventHub létrehozása
```powershell
PS C:\> New-AzEventHub -ResourceGroupName MyResourceGroupName -NamespaceName MyNamespaceName -Name MyEventHubName
```

Létrehoz egy MyEventHubName nevű eseményközpontot egy 3 napos üzenetmegőrzési időtartammal és két partícióval a \` \` \` WestUS-helyen, a \` \` MyResourceGroupName \` erőforráscsoporttal.

### 2. példa: Az Eventhub frissítése a "CaptureDescription" névvel
```powershell
...
Létrehozza a MyEventHubName nevű eseményközpontot egy 3 napos üzenetmegőrzési időtartammal, 2 partícióval és CaptureDescription tulajdonsággal a \` \` \` WestUS-helyen, a \` \` MyResourceGroupName \` erőforráscsoporttal.
