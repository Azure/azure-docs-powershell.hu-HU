...
### 3. példa: Kafka-kompatibilis névtér
```powershell
...
Létrehoz egy Event Hubs névteret MyNamespaceName a myLocation megadott földrajzi helyen, a \` \` \` \` \` MyResourceGroupName erőforráscsoportban, ahol a Kafka és az \` Automatikusinflate beállítás engedélyezve van.
