...
### 1. példa: Függvényalkalmazás letöltése név szerint, és törlése.
```powershell
PS C:\> Get-AzFunctionApp -Name MyAppName -ResourceGroupName MyResourceGroupName | Remove-AzFunctionApp -Force
```

Ez a parancs név szerint kap egy függvényalkalmazást, és törli azt.

### 2. példa: Függvényalkalmazás törlése név szerint.
```powershell
PS C:\> Remove-AzFunctionApp -Name MyAppName -ResourceGroupName MyResourceGroupName -Force
```

Ez a parancs név szerint töröl egy függvényalkalmazást.
...
### -Force
A parancsmag eltávolítására kényszeríti a függvényalkalmazást megerősítés kérése nélkül.
