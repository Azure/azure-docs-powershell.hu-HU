...
## SYNOPSIS
Frissíti a függvényalkalmazást.
...
### 1. példa: A függvényalkalmazás-üzemeltető csomag frissítése.
```powershell
PS C:\> Update-AzFunctionApp -Name MyUniqueFunctionAppName -ResourceGroupName MyResourceGroupName -PlanName NewPlanName 
```

Ez a parancs frissíti a függvényalkalmazás-üzemeltetési tervet.

### 2. példa: SystemAssigned felügyelt identitás beállítása egy függvényalkalmazáshoz.
```powershell
PS C:\> Update-AzFunctionApp -Name MyUniqueFunctionAppName -ResourceGroupName MyResourceGroupName -IdentityType SystemAssigned 
```

Ez a parancs egy SystemAssigned felügyelt identitást állít be egy függvényalkalmazáshoz.

### 3. példa: Az Alkalmazásstatikás függvényalkalmazás frissítése.
```powershell
...
Ez a parancs frissíti az Application Insights függvényt.

### 3. példa: Felügyelt identitás eltávolítása egy függvényalkalmazásból.
```powershell
PS C:\> Update-AzFunctionApp -Name MyUniqueFunctionAppName -ResourceGroupName MyResourceGroupName -IdentityType None 
```

Ez a parancs eltávolít egy felügyelt identitást egy függvényalkalmazásból.

## PARAMETERS

### -ApplicationInsightsKey
Az App Insights hozzáadható eszközkulcsa.
...
A függvényalkalmazáshoz hozzáadható meglévő App Insights-projekt neve.
...
### -AsJob
A parancsmagot háttérként futtatja.
...
### -IdentityID
A függvényalkalmazáshoz társított felhasználói identitások listáját adja meg.
...
### -IdentityType
A függvényalkalmazáshoz használt identitás típusát határozza meg.
A "Nincs" típus eltávolítja az identitásokat a függvényalkalmazásból.
A paraméter elfogadható értékei: - SystemAssigned - UserAssigned - None
...
### -NoWait
Elindítja a műveletet, és azonnal, a művelet befejezése előtt visszatér.
Ha meg szeretné állapítani, hogy a művelet sikeresen befejeződött-e, használjon más mechanizmust.
...
INPUTOBJECT: <ISite> 
  ...
  - `[Config <ISiteConfig>]`: Az app konfigurációja.
    ...
    - `[ActionType <AutoHealActionType?>]`: Előre megadott teendő.
    ...
    - `[ExperimentRampUpRule <IRampUpRule[]>]`: A felfutási szabályok listája.
      - `[ActionHostName <String>]`: Annak a tárolóhelynek a állomásneve, amelyre a rendszer átirányítja a forgalmat, ha úgy dönt. Például: myapp-stage.azurewebsites.net.
      - `[ChangeDecisionCallbackUrl <String>]`: Egyéni döntési algoritmus is meg lehet adni a TiPCallback webhelybővítményben, amely URL-címet is meg lehet adni. Lásd a TiPCallback webhelykiterjesztést az állványhoz és a szerződésekhez.         https://www.siteextensions.net/packages/TiPCallback/
      - `[ChangeIntervalInMinute <Int32?>]`: Az ReroutePercentage átértékelhető intervallumát adja meg percben.
      - `[ChangeStep <Double?>]`: Automatikus felfutás esetén ez a lépés a hozzáadás/eltávolítás, amíg el nem éri a <code>ReroutePercentage</code> \n <code>MinReroutePercentage</code> vagy         <code>MaxReroutePercentage</code> . A webhely metrikákat a .\nCustom döntési algoritmusban megadott N percenként ellenőrzi a <code>ChangeIntervalInMinutes</code> TiPCallback webhelybővítmény, amely az URL-címet meg lehet <code>ChangeDecisionCallbackUrl</code> adni.
    ...
    - `[Http20Enabled <Boolean?>]`: Http20Enabled: Úgy konfigurál egy webhelyet, hogy az ügyfelek http2.0-on keresztül csatlakozznak
    - `[HttpLoggingEnabled <Boolean?>]`: <code>true</code> ha a HTTP-naplózás engedélyezve van; ellenkező esetben <code>false</code> .
