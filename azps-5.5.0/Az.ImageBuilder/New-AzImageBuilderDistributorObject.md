...
### 1. példa: Felügyelt kép terjesztő létrehozása
```powershell
...
Ez a parancs egy felügyelt kép terjesztőt hoz létre.

### 2. példa: VHD terjesztő létrehozása
```powershell
...
Ez a parancs egy VHD terjesztőt hoz létre.

### 3. példa: Megosztott kép terjesztő létrehozása
```powershell
...
Ez a parancs egy megosztott kép terjesztőt hoz létre.

## PARAMETERS

### -ArtifactTag
Címkék, amelyek a terjesztő által létrehozott/frissített összetevőre lesznek alkalmazva.
...
### -ExcludeFromLatest
Flag that indicates that created image version should be excluded from latest.
Kihagyja az alapértelmezett (hamis) beállítást.
...
### -GalleryImageId
A Megosztott képgyűjtemény képének erőforrás-azonosítója.
...
### -ImageId
A Felügyelt lemez lemezképének erőforrás-azonosítója.
...
### -Location
A kép Azure-beli helyének egyeznie kell, ha a kép már létezik.
...
### -ReplikációsRégió
Azon régiók listája, amelyekbe a képet replikálni fogja.
...
### -RunOutputName
A társított RunOutput-hoz használt név.
...
### -SharedImageDistributor
Terjesztheti a megosztott képgyűjteményen keresztül.
...
### -StorageAccountType
A megosztott kép tárolásához használt tárfiók típusa.
Kihagyja az alapértelmezett (Standard_LRS).
...
### -VhdDistributor
Terjesztés VHD-n keresztül egy tárfiókban.
