...
## LEÍRÁS
Az **Update-AzKeyVaultKey** parancsmag frissíti egy kulcs szerkeszthető attribútumát egy kulcstárban.
...
Az első parancs a **Get-Date** parancsmag használatával hoz létre **DateTime-objektumot.** Ez az objektum a jövőben két évet ad meg. A parancs ezt a dátumot a $Expires tárolja.
További információért írja be a `Get-Help Get-Date` .
A második parancs létrehoz egy változót a nagy súlyosságú és könyvelési címkeértékek tárolására.
Az utolsó parancs módosítja az ITSoftware nevű kulcsot. A parancs engedélyezi a kulcsot, beállítja a lejárati idejét a $Expires, és beállítja a $Tags.
...
Ez a parancs törli az ITSoftware nevű kulcs adott verziójának összes címkéét.
...
### -Enable
Az igaz érték engedélyezi a kulcsot és a hamis értéket, letiltja a kulcsot.
Ha nincs megadva, a meglévő engedélyezett/letiltott állapot változatlan marad.
...
### -Lejár
A kulcs lejárati ideje az UTC-ben.
...
### -KeyOps
A billentyűvel elvégezhető műveletek.
Ha nincs megadva, a kulcs meglévő főbb műveletei változatlanok maradnak.
...
### -NotBefore
Az a UTC-idő, amely előtt a kulcs nem használható.
...
Ha ez a kapcsoló meg van adva, a frissített kulcskötegobjektumot adja vissza.
...
### -Tag
A hashtable a kulcscímkéket jelöli.
Ha nincs megadva, a kulcs meglévő címkéi változatlanok maradnak.
...
### -Verzió
Főverzió.
A parancsmag a tárolónévből (jelenleg kijelölt környezetből, kulcsnévből és kulcsverzióból) egy kulcs teljes tartománynevét építi fel.
