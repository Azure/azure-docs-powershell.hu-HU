...
### 1. példa: Meglévő fürt frissítése név szerint
```powershell
...
A fenti parancs frissíti a Kusto-fürt "testnewkustocluster" termékváltozatát, amely a "testrg" erőforráscsoportban található.

### 2. példa: Meglévő fürt frissítése név szerint
```powershell
...
A fenti parancs egy ügyfél által kezelt kulccsal frissíti a "testrg" erőforráscsoportban található "testnewkustocluster" fürtöt.
...
Logikai érték, amely azt jelzi, hogy a fürt lemezei titkosítottak-e.
...
Logikai érték, amely azt jelzi, hogy engedélyezve van-e a dupla titkosítás.
...
### -EnablePurge
Logikai érték, amely azt jelzi, hogy a véglegesen megadott műveletek engedélyezve vannak-e.
...
Logikai érték, amely azt jelzi, hogy engedélyezve van-e a folyamatos átvitel.
...
A "SystemAssigned, UserAssigned" típus implicit módon létrehozott identitást és felhasználó által hozzárendelt identitásokat is tartalmaz.
A "Nincs" típus az összes identitást eltávolítja.
...
A Kusto-fürthöz társított felhasználói identitások listája.
A felhasználói identitásszótár kulcshivatkozásai ARM következő űrlapon található erőforrás-azonosítók lesznek: '/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.ManagedIdentity/userAssignedIdentities/{identityName}'.
...
A kulcs tárolókulcsának verziója.
...
A felhasználó által hozzárendelt identitás (ARM erőforrás-azonosító), amely hozzáféréssel rendelkezik a kulcshoz.
...
A felépítésről a NOTES (MEGJEGYZÉSEK) szakaszban láthatja a LANGUAGEEXTENSIONVALUE tulajdonságokat, és létrehozhat egy kivonattáblát.
...
Logikai érték, amely azt jelzi, hogy az optimalizált automatikusskála funkció engedélyezve van-e.
...
A maximálisan engedélyezett példányok száma.
...
A minimálisan engedélyezett példányok száma.
...
A sablon definiált verziója, például 1.
...
A fürt külső bérlői.
Ennek létrehozásáról a TRUSTEDEXTERNALTENANT tulajdonságokat és a kivonattáblát a NOTES (JEGYZETEK) című szakaszban láthatja.
...
Adatkezelési nyilvános IP-cím erőforrás-azonosítója.
...
A motorszolgáltatás nyilvános IP-cím erőforrás-azonosítója.
...
Az alhálózati erőforrás azonosítója.
...
LANGUAGEEXTENSIONVALUE <ILanguageExtension[]>: A nyelvi bővítmények listája.
  - `[Name <LanguageExtensionName?>]`: A nyelvbővítmény neve.

TRUSTEDEXTERNALTENANT <ITrustedExternalTenant[]>: A fürt külső bérlői.
  - `[Value <String>]`: Külső bérlő guid azonosítója.
