...
### GetWithDefaultParameters (alapértelmezett)
```
...
## LEÍRÁS
A **Get-AzMetric** parancsmag egy adott erőforrás metrikus értékeit kapja meg.

## PÉLDÁK

### 1. példa: Mérőszám az összesített kimenettel
```
...
Ez a parancs a webhely3 webhely metrikus értékeit kapja 1 perces időszemétben.

### 2. példa: Részletes kimenetet mutató mutató lekérte
```
...
### 3. példa: Részletes kimenet lekérte egy adott mutatóhoz
```
...
Ez a parancs részletes kimenetet kap a Kérelmek mutatóhoz.

### 4. példa: Adott dimenziószűrővel megadott metrika összesített kimenetének leselkedése
```
...
Ez a parancs összegzi a PageViews mérőszám összesített kimenetét a megadott dimenziószűrővel és összegzési típussal.

## PARAMETERS

### -AggregationType
A lekérdezés összegzési típusa
...
Alapértelmezés szerint a kimenet összegzi a kimenetet.
...
### -MetricFilter
A metrikus dimenziószűrőt adja meg a lekérdezési metrikákhoz.
...
### -MetricNamespace
A lekérdező metrikák névterét adja meg.
...
### -OrderBy
Az eredmények rendezési irányát és a rendezés irányát adja meg (például: összeg asc).
...
### -ResultType
A visszaadott eredmény típusát (metaadatok vagy adatok) adja meg.
...
### -TimeGrain
A metrika időszemétét **IdőSpan** objektumként adja meg az óó:pp:mm formátumban.
...
### -Top
A beolvasható rekordok maximális számát (default:10) adja meg a $filter.
...
A támogatott metrikákról további információt a következő oldalon talál: https://docs.microsoft.com/en-us/azure/azure-monitor/platform/metrics-supported
