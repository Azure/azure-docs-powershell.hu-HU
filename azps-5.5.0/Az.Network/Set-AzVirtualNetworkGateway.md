...
### ServerConfigurationUpdateResourceWithTags
```
Set-AzVirtualNetworkGateway -VirtualNetworkGateway <PSVirtualNetworkGateway> [-GatewaySku <String>]
 [-GatewayDefaultSite <PSLocalNetworkGateway>] [-VpnClientAddressPool <String[]>]
 [-VpnClientProtocol <String[]>] [-VpnClientRootCertificates <PSVpnClientRootCertificate[]>]
...
### 1. példa: Virtuális hálózati átjáró ASN-ének frissítése
```
PS C:\>$Gateway = Get-AzVirtualNetworkGateway -ResourceGroupName "ResourceGroup001" -Name "Gateway001"
PS C:\> Set-AzVirtualNetworkGateway -VirtualNetworkGateway $Gateway -Asn 1337
```

Az első parancs egy Átjáró01 nevű virtuális hálózati átjárót kap, amely az ResourceGroup001 erőforráscsoporthoz tartozik, és az $Gateway A második parancs frissíti a változóban tárolt virtuális hálózati átjárót $Gateway.
A parancs az ASN-t is 1337-re állítja.
...
Az első parancs egy Átjáró01 nevű virtuális hálózati átjárót kap, amely az ResourceGroup001 erőforráscsoporthoz tartozik, és az $Gateway A második parancs a Megadott ipsec-paramétereknek megfelelő Vpn ipsec házirendobjektumot hozza létre.
A harmadik parancs frissíti a változó típusú $Gateway.
A parancs emellett beállítja a virtuális hálózat átjárója $vpnclientipsecpolicy objektumban megadott egyéni vpn ipsec házirendet is.

### 3. példa: Címkék hozzáadása/frissítése meglévő virtuális hálózati átjáróhoz
```
PS C:\>$Gateway = Get-AzVirtualNetworkGateway -ResourceGroupName "ResourceGroup001" -Name "Gateway001"
...
Az első parancs egy Átjáró01 nevű virtuális hálózati átjárót kap, amely az ResourceGroup001 erőforráscsoporthoz tartozik, és az $Gateway A második parancs frissíti az Átjáró01 virtuális hálózati átjárót a @{ testtagKey="SomeTagKey"; testtagValue="SomeKeyValue" } címkével.

### 4. példa: AAD-hitelesítési konfiguráció hozzáadása/frissítése meglévő virtuális hálózati átjáró VpnClient-éhez
```
PS C:\>$Gateway = Get-AzVirtualNetworkGateway -ResourceGroupName "ResourceGroup001" -Name "Gateway001"
...
Az első parancs egy Átjáró01 nevű virtuális hálózati átjárót kap, amely az ResourceGroup001 erőforráscsoporthoz tartozik, és a $Gateway A második parancs frissíti az Átjáró01 virtuális hálózati átjárót az AAD-hitelesítési konfigurációval: paraméter:aadTenantUri, aadAudienceId, aadIssuerUri for VpnClient.
A harmadik parancs eltávolítja az AAD-hitelesítési konfigurációt a virtuális hálózati átjáró VpnClient szolgáltatásában.

### 5. példa: IpConfigurationBgpPeeringAddresses hozzáadása/frissítése meglévő virtuális hálózati átjáróhoz
```
PS C:\>$Gateway = Get-AzVirtualNetworkGateway -ResourceGroupName "ResourceGroup001" -Name "Gateway001"
...
Az első parancs egy Átjáró01 nevű virtuális hálózati átjárót kap, amely az ResourceGroup001 erőforráscsoporthoz tartozik, és az $Gateway A második parancs a virtuális hálózati átjáró átjárójának értékét (Gateway01 IpConfiguration Id) változó ipconfigurationId1 változóhoz rendeli hozzá.
A harmadik parancs hozzárendeli a címlistát a címlistához1.
A negyedik parancs létrehozott egy PSIpConfigurationBgpPeeringAddress objektumot.
Az ötödik parancs ezt az új PSIpConfigurationBgpPeeringAddress címet IpConfigurationBgpPeeringAddresses-re állítsa be, és frissítse az átjárót.
...
### -Asn
A virtuális hálózati átjáró ASN-je, amely az IPsec-alagutakon belüli BGP-munkamenetek beállításához használatos
...
### -GatewayDefaultSite
Az a hely, ahol kényszeríteni lehet az alagutat.
Ha meg van adva egy alapértelmezett webhely, az átjáró virtuális hálózatának összes internetes adata arra a webhelyre lesz irányítva.
...
### -GatewaySku
A virtuális hálózati átjáró termékváltozata
...
A virtuális hálózati átjáró objektum, amelyből ki kell alapozni a módosításokat.
Ezt a lekérdezést a Get-AzVirtualNetworkGateway
...
### -VpnClientAddressPool
A VPN-ügyfél IP-címeinek kiosztható címterülete.
Ez nem lehet átfedésben a virtuális hálózattal vagy a helyi tartományokkal.
...
IPSec-házirendek listája a P2S VPN-ügyféloldali protokollokhoz.
...
### -VpnClientProtocol
A P2S VPN-ügyféloldali alagutassági protokollok listája
...
### -VpnClientRevokedCertificates
A visszavont VPN-ügyféltanúsítványok listája.
Ha egy ilyen tanúsítványt bemutató VPN-ügyfél nem rendelkezik ilyen tanúsítvánnyal, a szolgáltatás azt jelenti, hogy el kell mennie.
...
### -VpnClientRootCertificates
A VPN-ügyfélalkalmazások főtanúsítványai, amelyek a VPN-ügyfélhitelesítéshez használhatók.
A virtuális magánhálózati ügyfélnek az ilyen főtanúsítványok egyikében létrehozott tanúsítványokat kell bemutatnia.
