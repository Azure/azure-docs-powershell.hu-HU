...
## LEÍRÁS
A **New-AzRecoveryServicesAsrPolicy** parancsmag létrehoz egy Azure-webhely-helyreállítási replikációs házirendet.
A replikációs házirend olyan replikációs beállítások megadására használható, mint például a replikáció gyakorisága és a helyreállítási pontok száma.
...
A megadott paraméterekkel elindítja a replikációs házirend létrehozását, és visszaadja a művelet nyomon követéséhez használt ASR-feladatot.
...
Az alkalmazás konzisztens helyreállítási pontjainak létrehozására használt gyakoriság (óra) megadása.
...
### -Hitelesítés
A használt hitelesítés típusát határozza meg.
Érvényes értékek:
...
### -AzureToAzure
A Kapcsoló paraméter az Azure–Azure-házirendek létrehozásának forgatókönyvét határozza meg.
...
### -AzureToVMware
A Kapcsoló paraméter az Azure-nak a vMWare-házirendek létrehozására vonatkozó forgatókönyvét határozza meg.
...
### -Compression
Azt adja meg, hogy engedélyezve kell-e lennie a tömörítésnek.
...
### -HyperVToAzure
A paraméter váltása a házirend megadásához a Hyper-V virtuális gépek Azure-ba való replikálása érdekében használható
...
### -MultiVmSyncStatus
TöbbVm-szinkronizálási állapotot ad meg a házirendhez.
...
Megadja a megőrizni szükséges helyreállítási pontokat.
...
Őrizze meg a helyreállítási pontokat adott ideig órában.
...
### -ReplicaDeletion
Azt adja meg, hogy a virtuális replika-gépet törölni kell-e, amikor letiltja a VMM által felügyelt webhelyről egy másikra történő replikációt.
...
Másodpercben megadja a replikációs gyakorisági intervallumot.
Érvényes értékek:
...
### -ReplicationMethod
A replikációs módszer megadása.
Érvényes értékek:
...
### -ReplicationProvider
A házirend replikációs szolgáltatóját adja meg.
...
A feladat kezdése után legalább 24 óranek kell lennie.
...
Az RPO küszöbértéke percek alatt, amelyeknél figyelmeztetést kell alkalmazni.
...
### -VmmToVmm
A paraméter váltása a házirend megadásához a VMM-kiszolgáló által kezelt Hyper-V webhelyek közötti replikáláshoz használható.
...
### -VMwareToAzure
Switch parameter specify that the replication policy being used to replikálása VMware virtual machines and/or Physical servers to Azure.
