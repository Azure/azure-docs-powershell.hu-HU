...
## LEÍRÁS
Frissíti az alkalmazott `Reservation` hatókört.
...
A megadott AppliedScopeType értéket `Reservation` "Egyszeres" és InstanceFlexibility "Be" típusra frissíti.
...
A megadott AppliedScopeType értéket `Reservation` "Megosztott" és InstanceFlexibility "Kikapcsolva" értékre frissíti.
...
### -InstanceFlexibility
Ha van ilyen, akkor frissíti a `Reservation` . Ha nincs megadva, a meglévő érték változatlan marad.
...
### -ReservationId
A `Reservation` frissítenie kell azonosítója
...
### -ReservationOrderId
A `ReservationOrder` frissítenie kell azonosítója
