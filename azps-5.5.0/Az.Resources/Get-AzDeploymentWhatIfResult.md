...
## LEÍRÁS
A **Get-AzDeploymentWhatIfResult** parancsmag a ARM sablont What-If az aktuális előfizetési hatókörű sablontelepítés eredményéhez. Visszaadja a módosítások listáját, jelezve, hogy mely erőforrások lesznek frissítve, ha a telepítést a tényleges erőforrások változtatása nélkül alkalmazza. Az eredmény formátumának megadásához használja a *ResultFormat paramétert.*

## PÉLDÁK

### 1. példa: Új What-If az előfizetés hatókörében
```powershell
...
Ez a parancs egy egyéni sablonfájl és egy lemezen található paraméterfájl használatával What-If találatot az aktuális előfizetés hatókörében.
A parancs a *Hely paraméterrel* adja meg, hogy hol tárolja a telepítési adatokat.
...
A parancs a *ResultFormat* paraméter segítségével úgy What-If, hogy az tartalmazza a teljes erőforrás-hasznos adatokat.

### 2. példa: Új What-If az előfizetés hatókörében a ResourceIdOnlyval
```powershell
PS C:\> Get-AzDeploymentWhatIfResult `
    -DeploymentName "deploy-01" `
    -Location "West US" `
...
Ez a parancs What-If az előfizetés aktuális hatókörében egy egyéni sablonfájl és egy paraméterfájl használatával a lemezen.
...
### -ExcludeChangeType
Azon erőforrás-módosítási típusok vesszővel elválasztott listája, amelyek nem tartoznak az What-If eredménybe.
...
### -Name
Annak a telepítésnek a neve, amelyből létre fog hozni. Ha nincs megadva, akkor a sablonfájl alapértelmezett neve, ha sablonfájlt ad meg; sablonobjektumok (például "2013123140835") beállítása.
...
Olyan fájl, amely a sablon paramétereit tartalmazza.
