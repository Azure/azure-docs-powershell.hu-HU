...
Új csoport létrehozása a beállítással és a `DisplayName` `ParentId` `null` beállítással. Ugyanaz lesz, mint a csoport szülője és szülője `DisplayName` `GroupName` lesz a bérlő.  
...
Ebben az esetben a csoport szülője lesz a bérlő, és a megadott értékre `DisplayName` lesz beállítva.

### 3. példa: Felügyeleti csoport létrehozása szülőnévvel és megjelenítendő névvel
```
...
### 4. példa: Felügyeleti csoport létrehozása szülőobjektummal (szülőobjektum használatával)
```
...
### -ParentId
A felügyeleti csoport szülőazonosítója
