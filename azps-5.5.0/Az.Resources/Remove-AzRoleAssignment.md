...
A feladat objektuma, vagyis a tőketnévnek meg kell adni.
...
A szerepkört, amelyhez a főnevet hozzárendelték, a RoleDefinitionName paraméterrel kell megadni.
Előfordulhat, hogy a hozzárendelés hatóköre meg van adva, és ha nincs megadva, alapértelmezés szerint az előfizetés hatóköre, azaz megpróbálja törölni a megadott fő és szerepkörhöz tartozó hozzárendelést az előfizetés hatókörében.
A hozzárendelés hatóköre az alábbi paraméterek egyikével lehet megadni.
a.
Hatókör – Ez a teljesen minősített hatókör az /subscriptions/ \<subscriptionId\> b tartománytól kezdve.
ResourceGroupName – Az előfizetés alatt bármelyik erőforráscsoport neve.
c.
ResourceName, ResourceType, ResourceGroupName és (opcionális) ParentResource – Az előfizetés alatt egy adott erőforrást azonosít.
...
Ha a ResourceName, a ResourceType és (nem kötelező) ParentResource paraméterekkel együtt használja, a parancs hierarchikus hatókört épít fel egy erőforrást azonosító relatív URI-ként.
...
A ResourceGroupName, az ResourceType és (opcionális) ParentResource paraméterekkel együtt kell használni egy hierarchikus hatókör felépítéséhez egy relatív URI-ként, amely azonosítja az erőforrást, és törli az adott hatókörben lévő hozzárendelést.
