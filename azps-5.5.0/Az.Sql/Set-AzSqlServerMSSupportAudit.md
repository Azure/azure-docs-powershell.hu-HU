...
## SYNOPSIS
Egy Azure SQL-kiszolgáló Microsoft támogatási műveleteinek naplózási beállításait módosítja.
...
## LEÍRÁS
A **Set-AzSqlServerMSSupportAudit** parancsmag módosítja egy Azure SQL-kiszolgáló Microsoft támogatási műveleteinek naplózási beállításait.
A parancsmagot a *ResourceGroupName* és *a ServerName* paraméterrel használhatja a kiszolgáló azonosításához.
Ha a blobtár a naplók célhelye, a *StorageAccountResourceId* paramétert megadva határozza meg a naplók tárfiókját.

## PÉLDÁK

### 1. példa: Azure SQL-kiszolgáló blobtárolójának engedélyezése a Microsoft támogatási műveletvizsgálati házirendjában
```powershell
...
### 2. példa: Azure SQL-kiszolgáló blobtárolójának letiltása a Microsoft támogatási műveletvizsgálati házirendjában
```powershell
PS C:\>Set-AzSqlServerMSSupportAudit -ResourceGroupName "ResourceGroup01" -ServerName "Server01" -BlobStorageTargetState Disabled
```

### 3. példa: Azure SQL-kiszolgáló eseményközpontjának engedélyezése a Microsoft támogatási műveletvizsgálati házirendjában
```powershell
...
### 4. példa: Azure SQL-kiszolgáló eseményközpontjának letiltása a Microsoft támogatási műveletvizsgálati házirendjában
```powershell
PS C:\>Set-AzSqlServerMSSupportAudit -ResourceGroupName "ResourceGroup01" -ServerName "Server01" -EventHubTargetState Disabled
```

### 5. példa: Azure SQL-kiszolgáló naplóelemzési házirendjának engedélyezése a Microsoft támogatási műveleteinek naplózásához
```powershell
...
### 6. példa: Azure SQL-kiszolgáló naplóelemzési házirend letiltása a Microsoft támogatási műveleteinek naplózásához
```powershell
...
### 7. példa: Egy Azure SQL-kiszolgáló naplóelemzési házirendjának letiltása a Microsoft által támogatott műveletek naplózási folyamatán keresztül
```powershell
...
### 8. példa: Tiltsa le egy Azure SQL-kiszolgáló támogatási műveletrekordjainak blobtárolóba küldését, és engedélyezze számukra a naplózási elemzések küldését.
```powershell
...
### 9. példa: A Microsoft támogatási műveletek naplórekordjainak küldésének engedélyezése egy Azure SQL-kiszolgálóhoz blobtároló, eseményközpont és naplóelemzés céljából.
```powershell
...
Azt jelzi, hogy a blobtároló a Microsoft támogatási műveleteinek naplórekordjainak célhelyét jelenti-e.
...
### -EventHubTargetState
Azt jelzi, hogy az eseményközpont a Microsoft támogatási műveleteinek naplórekordjainak célhelyét jelenti-e.
...
Azt jelzi, hogy a naplóelemzés a Microsoft támogatási műveleteinek naplórekordjainak célhelyét jelenti-e.
...
### -PassThru
Megadja, hogy kimenetként adja-e meg a Microsoft támogatási műveletek naplózási házirendét a parancsmag végrehajtásának végén.
...
### -ServerObject
A Kiszolgálóobjektum, amely a Microsoft támogatási műveleteinek naplózási házirendját kezeli.
...
### -WorkspaceResourceId
A Log Analytics munkaterületének munkaterület-azonosítója (a Log Analytics munkaterületének erőforrás-azonosítója), amelyre a Microsoft támogatási műveleteinek naplóit el szeretné küldeni. Példa: /subscriptions/4b9e8510-67ab-4e9a-95a9-e2f1e570ea9c/resourceGroups/insights-integration/providers/Microsoft.OperationalInsights/workspaces/viruela2
