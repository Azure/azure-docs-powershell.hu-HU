...
Ez a parancs beállít egy titkosítási kulcsforrást egy új létrehozott Keyvaulttal.
...
### 13. példa: Tárfiók frissítése a RoutingPreference beállítással
```powershell
...
Ez a parancs frissíti a tárfiókot a RoutingPreference beállítással: A PublishMicrosoftEndpoint mint false, a PublishInternetEndpoint mint true, a RoutingChoice pedig MicrosoftRouting.

## PARAMETERS

### -AccessTier
A parancsmag által módosított Tárfiók hozzáférési rétegét adja meg.
...
Azt jelzi, hogy a tárfiók támogatja-e az 5 TiB-kapacitásnál nagyobb fájlmegosztásokat. A fiók engedélyezése után a funkció nem tiltható le. Jelenleg csak az LRS és a ZRS replikációs típusai támogatottak, ezért a fiókok georedundáns fiókokra történő konvertálása nem lenne lehetséges. További információ: https://go.microsoft.com/fwlink/?linkid=2086047
...
### -KeyvaultEncryption
Azt jelzi, hogy a Tárhelyszolgáltatás-titkosítás használata során használni kell-e a Microsoft KeyVaultot a titkosítási kulcsokhoz. Ha a KeyName, a KeyVersion és a KeyVaultUri mind be van állítva, a KeySource értéke Microsoft.Keyvault lesz, függetlenül attól, hogy be van-e állítva ez a paraméter. 
...
### -KeyVaultUri
Ha a -KeyvaultEncryption paraméter megadásával használja a kulcstitkosítást, ezzel a beállítással megadhatja a kulcstár URI-ját.
...
### -MinimumTlsVersion
A tárterület-kérelmekben megengedett minimális TLS-verzió.
...
### -RoutingChoice
Az Routing Choice a felhasználó által választott hálózati útválasztási típust határozza meg. Lehetséges értékek: "MicrosoftRouting", "InternetRouting"
...
- Premium_LRS – Prémium helyi redundáns tárterület.
- Standard_GZRS – Georedundáns zónaredundáns tárolás.
- Standard_RAGZRS – Olvassa el a georedundáns zóna-redundáns tárhelyet.
A fióktípusok Standard_ZRS és Premium_LRS más típusú fiókokra nem változtatható.
