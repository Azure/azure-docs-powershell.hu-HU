...
## LEÍRÁS
A **New-AzVmss** parancsmag létrehoz egy virtuálisgép-mérethalmazt (VMSS) az Azure-ban.
...
* Az ötödik parancs a **New-AzVirtualNetworkSubnetConfig** parancsmaggal hoz létre egy alhálózatot, és az eredményt a $SubNet.
...
* A nyolcadik és a hetedik parancs a **New-AzPublicIpAddress** és **a Get- AzureRmPublicIpAddress** paranccsal hozza létre és szerezze be az információkat a nyilvános IP-címről.
* A parancsok az adatokat a $PubIP.
* A tizedik parancs a **New- AzureRmLoadBalancerFrontendIpConfig** parancsmaggal hoz létre egy frontend terheléselegyenítőt, és az eredményt a $Frontend.
* A tizenegyedik parancs a **New-AzLoadBalancerBackendAddressPoolConfig** paranccsal hoz létre egy háttércímkészlet-konfigurációt, és az eredményt a következő nevű változóban $BackendAddressPool.
...
### -BackendPoolName
Az ebben a méretkészletben a terheléselosztásban használt háttércímkészlet neve.  Ha nem ad meg értéket, a rendszer létrehoz egy új háttérkészletet, amely neve megegyezik a méretkészlet nevével.
...
### -EncryptionAtHost
Ez a paraméter engedélyezi a titkosítást az összes lemezen, beleértve magában az erőforrás-/ideiglenes lemezen is. Alapértelmezett: A gazdaszámítógép titkosítása le lesz tiltva, hacsak ez a tulajdonság nincs igazra állítva az erőforráshoz.
...
### -EvictionPolicy
Az alacsony prioritású virtuális gép méretarányának beállítása kiviction házirend.  Csak a "Deallocate" és a "Delete" érték támogatott.
...
### -FrontendPoolName
Az előlapi címkészlet neve, amely a Scale Set terheléselosztásban használható.  Ha nem ad meg értéket, létrejön egy új előlapi címkészlet, amely a méretaránykészlet nevével megegyezik.
...
### -Location
Az Azure-hely, ahol ez a méretaránykészlet fog létrejönni.  Ha nincs megadva érték, a hely a paraméterekben hivatkozott egyéb erőforrások helyéről lesz kikövetkeztetve.
...
### -Priority
A virtuális gép prioritása a méretaránykészletben.  Csak a "Regular", a "Spot" és a "Low" támogatott értékeket támogatja.
...
### -PublicIpAddressName
Az ezzel a méretkészletet használva használt nyilvános IP-cím neve.  Ha nem ad meg értéket, létrejön egy új nyilvános IPAddress, a méretaránykészlet nevével.
...
### -VirtualNetworkName
Az ezzel a méretkészletet használva használt virtuális hálózat neve.  Ha nem ad meg értéket, létrejön egy új virtuális hálózat a méretaránykészlet nevével.
...
### -VMScaleSetName
A parancsmag által létrehozott VMSS neve.
...
### -VnetAddressPrefix
A méretaránykészlethez használt virtuális hálózat címelőtagja.  Ha nem ad meg értéket, akkor a virtuális hálózat címének alapértelmezett előtagbeállítása (192.168.0.0/16).
