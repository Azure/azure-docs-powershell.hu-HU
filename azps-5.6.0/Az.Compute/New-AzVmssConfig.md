...
### -EvictionPolicy
A virtuális gépek kiviction házirendét adja meg a méretaránykészletben.
...
### -Location
Megadja azt az Azure-helyet, ahol a VMSS létrejön.
...
### -MaxPrice
Azt a maximális árat adja meg, amelyért fizet a spot VM/VMSS-ekért. Ez az ár amerikai dollárban van meg. Ezt az árat összehasonlítjuk a virtuális gép méretének aktuális spotárával. Ezenkívül az árakat a helyszíni VM/VMSS létrehozása/frissítése során hasonlítják össze, és a művelet csak akkor lesz sikeres, ha a maxPrice nagyobb, mint az aktuális azonnali ár. A maxPrice akkor is használatos a helyszíni VM/VMSS kivoktatására, ha a jelenlegi spotár meghaladja a vm/VMSS létrehozását követően a maxPrice értéken. A lehetséges értékek a nullánál nagyobb decimális értékek. Példa: 0,01538.  -1 azt jelzi, hogy a helyszíni VM/VMSS ár okokból nem szükséges. Az alapértelmezett maximális ár -1, ha nem Ön biztosítja.
...
### -Priority
A virtuális machien prioritása a méretaránykészletben.  Csak a "Regular", a "Spot" és a "Low" támogatott értékeket támogatja.
...
A virtuális gép által törölt virtuális gépnek a konfigurálható időtartamnak (percben) jóvá kell hagynia az ütemezett esemény megszüntetését, mielőtt az esemény automatikusan jóváhagyásra kerül (időkorra).
