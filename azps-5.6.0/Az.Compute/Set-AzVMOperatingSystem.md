...
### 1. példa: Az operációs rendszer tulajdonságainak beállítása új virtuális géphez
```
...
A harmadik parancs az Erőforráscsoport11 erőforráscsoportBan az AvailabilitySet03 nevű elérhetőségi halmazt kapja meg, majd az objektumot a $AvailabilitySet változóban tárolja.
...
### 2. példa: Új virtuális gép operációs rendszerének tulajdonságainak beállítása engedélyezett gyorsjavítással
```
$SecurePassword = ConvertTo-SecureString "Password" -AsPlainText -Force
$Credential = New-Object System.Management.Automation.PSCredential ("FullerP", $SecurePassword); 
$AvailabilitySet = Get-AzAvailabilitySet -ResourceGroupName "ResourceGroup11" -Name "AvailabilitySet03" 
...
A parancs a $Credential.
A parancs bizonyos paraméterekhez a korábbi parancsokban hozzárendelt változókat használja.
A parancs engedélyezi a gyorsjavítást a virtuális gépen.

### 3. példa: Az operációs rendszer tulajdonságainak beállítása új Linux-virtuális géphez
```
$SecurePassword = ConvertTo-SecureString "Password" -AsPlainText -Force
$Credential = New-Object System.Management.Automation.PSCredential ("FullerP", $SecurePassword); 
$AvailabilitySet = Get-AzAvailabilitySet -ResourceGroupName "ResourceGroup11" -Name "AvailabilitySet03" 
...
A parancs nevet és méretet rendel a virtuális géphez.
A virtuális gép a 2016-ban tárolt rendelkezésre állási $AvailabilitySet.
A következő két parancs értékeket rendel a változókhoz, amelyek az alábbi parancsban használhatók.
Az utolsó parancs beállítja az operációs rendszer tulajdonságait a $VirtualMachine.
A parancs a $Credential.
A parancs bizonyos paraméterekhez a korábbi parancsokban hozzárendelt változókat használja.
A parancs beállítja a javítás mód értékét a virtuális gépen az "AutomaticByPlatform" értékre.
...
### -CustomData
Megadja a virtuális gépnek át adható karakterláncot. További információt az [Azure-alapú virtuális gépek egyéni adataiban található.](https://docs.microsoft.com/en-us/azure/virtual-machines/custom-data)
**Megjegyzés: Nem ajánlott bizalmas adatokat egyéni adatokban tárolni.**
...
### -EnableHotpatching
Lehetővé teszi az ügyfeleknek, hogy újraindítás nélkül javítják az Azure-virtuális gépeiket. Az enableHotpatching engedélyezéséhez a "provisionVMAgent" értéknek igazra, a "patchMode" pedig az "AutomaticByPlatform" értékre kell állítania.
...
**AutomaticByPlatform** – a virtuális gépet automatikusan frissíti az operációs rendszer. A provisionVMAgent és a WindowsConfiguration.enableAutomaticUpdates tulajdonságnak teljesülnie kell
...
A lehetséges értékek a [TimeZoneInfo.GetSystemTimeZones](https://docs.microsoft.com/dotnet/api/system.timezoneinfo.getsystemtimezones) [által visszaadott](https://docs.microsoft.com/dotnet/api/system.timezoneinfo.id?#System_TimeZoneInfo_Id) időzónákból származó TimeZoneInfo.Id is beállíthatók.
