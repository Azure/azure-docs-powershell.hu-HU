...
## SYNOPSIS
Az exportálás létrehozására vagy frissítésére vonatkozó művelet.
A frissítési művelethez be kell állítani a legújabb eTag-et a kérelemben.
A get művelettel beszerezheti a legújabb eTag-et.
A létrehozási művelethez nincs szükség eTag gombra.
...
### 1. példa: AzCostManagementExport frissítése hatókör és név szerint
```powershell
...
### 2. példa: AzCostManagementExport frissítése inputObject szerint
```powershell
...
### -ConfigurationColumn
Az exportálásban szerepeltetniandó oszlopnevek tömbje.
Ha nincs megtéve, akkor az exportálás az összes elérhető oszlopot tartalmazza.
Az elérhető oszlopok az ügyfélcsatornától függően változhatnak (lásd a példákat).
...
### -DataSetGranularity
Az exportálás sorai részletessége.
Jelenleg csak a "Naponta" támogatott.
...
### -DefinitionTimeframe
Az exportáláshoz szükséges adatok behúzásának időkerete.
Ha egyéni, akkor meg kell adni egy adott időszakot.
...
Felhívjuk a figyelmét arra, hogy a "Usage" (Használat) egyenértékű a "ActualCost" értékekkel, és olyan exportokra alkalmazható, amelyek még nem szolgáltatnak adatokat a szolgáltatásfoglalások díjára vagy amortizációs összegére.
...
Annak a tárolónak a neve, amelybe exportálni fogja az adatokat.
...
Annak a tárfióknak az erőforrás-azonosítója, amelybe exportálni fogják az etikát.
...
Annak a könyvtárnak a neve, amelybe az exportálást feltölti a rendszer.
...
### -ETag
Az erőforrás eTagja.
Az egyidejű frissítési esetek kezeléséhez ez a mező határozza meg, hogy a felhasználó frissíti-e a legújabb verziót.
...
Jelenleg csak a "Csv" támogatott.
...
### -ScheduleStatus
Az exportálás ütemezésének állapota.
Ha "Inaktív" állapotban van, az exportálás ütemezése fel van függesztve.
