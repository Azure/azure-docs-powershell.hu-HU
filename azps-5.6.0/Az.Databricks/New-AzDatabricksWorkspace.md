...
### 1. példa: Adatkapcsolati munkaterület létrehozása
```powershell
...
Ez a parancs adatforrás-munkaterületet hoz létre.

### 2. példa: Adatkapcsolatú munkaterület létrehozása testre szabott virtuális hálózattal
```powershell
...
Ez a parancs létrehoz egy Datab workspace with customized virtual network in a resource group.

### 3. példa: Adatkapcsolati munkaterület létrehozása titkosítás engedélyezésével
```powershell
...
Ez a parancs létrehoz egy Datab workspace-munkaterületet, és beállítja a titkosításra való felkészüléshez.
A titkosításra vonatkozó Update-AzDatabricksWorkspace további beállításokért tanulmányozza az itt látható példákat.
...
### -EnableNoPublicIP
Az az érték, amelyet ehhez a mezőhöz kell használni.
...
A felügyelt erőforráscsoport neve.
...
### -PrepareEncryption
Készítse elő a munkaterületet titkosításra.
Engedélyezi a Felügyelt identitást felügyelt tárfiókhoz.
...
### -PrivateSubnetName
A virtuális hálózaton belüli privát alhálózat neve.
...
### -PublicSubnetName
A virtuális hálózaton belüli nyilvános alhálózat neve.
...
Logikai érték, amely azt jelzi, hogy a DBFS-gyökérfájlrendszer engedélyezve lesz-e másodlagos titkosítási réteggel, valamint platform felügyelt kulcsokkal a nyugott adatokhoz.
...
### -VirtualNetworkId
Annak a virtuális hálózatnak az azonosítója, ahol létre kell hoznunk ezt az adathalmazcsoportot.
