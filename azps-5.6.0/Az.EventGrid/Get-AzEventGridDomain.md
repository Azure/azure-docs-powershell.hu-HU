...
## LEÍRÁS
A Get-AzEventGridDomain parancsmag vagy egy megadott Event Grid tartomány adatait, vagy az aktuális Azure-előfizetés összes Eseményrács tartományának listáját kapja.
Ha meg van biztosítanak egy tartománynevet, egyetlen Event Grid tartomány adatait ad vissza a rendszer.
Ha a tartománynév nem szerepel a listában, a visszaadott tartománylista fog eredményül adni. A listában visszaadott elemek számát a Felső paraméter vezérli. Ha a felső érték nincs megadva vagy nincs $null, akkor a lista az összes visszaadott tartományelemet egyszerre fogja tartalmazni. Ellenkező esetben a Top a listában visszaadott elemek maximális számát jelzi.
Ha további tartományok is elérhetők, a következő hívásban a NextLink értékét kell használni a következő tartománylap lehívására.
...
A \` \` \` MyResourceGroupName erőforráscsoport Eseményrács tartomány1 tartományának adatait \` tartalmazza.
...
A MyResourceGroupName erőforráscsoport Eseményrács tartomány1 tartományának adatait a \` \` \` \` ResourceId beállítással kapja meg.
...
List all Event Grid domains in resource group \` MyResourceGroupName \` without pagination (all domains are returned in one shot)
...
Sorolja fel a MyResourceGroupName erőforráscsoport eseményrács tartományát (ha van ilyen), amely megfelel $odataFilter \` \` 10 tartománynak. Ha több találat érhető el, a $result. A NextLink nem lesz $null. A tartományok következő oldalának lehívása érdekében a felhasználó várhatóan újra hívja Get-AzEventGridDomain és az eredményt használja. Az előző hívásból kapott NextLink. A hívónak meg kell állnia, amikor az eredmény álljon meg. A NextLink $null.
...
Az Azure-előfizetés összes Eseményrács tartományának felsorolása tördelés nélkül (minden tartományt egy képben ad vissza)
...
Az Azure-előfizetésben sorolja fel azokat az Eseményrács tartományokat (ha vannak), amelyek $odataFilter 20 tartományt. Ha több találat érhető el, a $result. A NextLink nem lesz $null. A tartományok következő oldalának lehívása érdekében a felhasználó várhatóan újra hívja Get-AzEventGridDomain és az eredményt használja. Az előző hívásból kapott NextLink. A hívónak meg kell állnia, amikor az eredmény álljon meg. A NextLink $null.
...
### -NextLink
A következő behozni szükséges erőforrások lapjára mutató hivatkozás.
Ezt az értéket az első parancsmag-Get-AzEventGrid akkor lehet beszerezni, ha még több erőforrás áll rendelkezésre a lekérdezhető erőforrásokhoz.
...
### -ODataQuery
A lista eredményeinek szűréséhez használt OData-lekérdezés.
A szűrés jelenleg csak a Név tulajdonságban engedélyezett. A támogatott műveletek közé tartoznak a következők: CONTAINS, eq (egyenlő), ne (nem egyenlő), ÉS, VAGY és NEM.
...
### -Top
A lekért erőforrások maximális száma.
Az érvényes érték 1 és 100 között lehet.
Ha a felső érték meg van adva, és további eredmények is elérhetők, az eredményben egy hivatkozás fog jelenni, amely a következő lekérdezni fogja a NextLinkben lekérdezni fog egy lapot.
Ha nincs megadva a felső érték, a teljes erőforráslistát egyszerre adja vissza a visszaadott érték.
