...
### 1. példa: Fogyasztási PowerShell-függvényalkalmazás létrehozása Közép-Usa-ban.
```powershell
...
Ez a parancs létrehoz egy fogyasztási PowerShell-függvényalkalmazást az Egyesült Államok középső felében.

### 2. példa: Egy szolgáltatástervben üzemeltetett PowerShell-függvényalkalmazás létrehozása.
```powershell
PS C:\> New-AzFunctionApp -Name MyUniqueFunctionAppName `
                          -ResourceGroupName MyResourceGroupName `
                          -PlanName MyPlanName `
...
Ez a parancs létrehoz egy PowerShell-függvényalkalmazást, amely egy szolgáltatástervben fog működni.

### 3. példa: Függvényalkalmazás létrehozása privát ACR-kép használatával.
```powershell
PS C:\> New-AzFunctionApp -Name MyUniqueFunctionAppName `
                          -ResourceGroupName MyResourceGroupName `
                          -PlanName MyPlanName `
...
Ez a parancs egy függvényalkalmazást hoz létre egy privát ACR-kép használatával.
...
Tiltsa le az alkalmazáselemzési erőforrások létrehozását a függvényalkalmazás létrehozása során.
Naplók nem érhetők el.
...
### -DockerImageName
Csak Linux.
Container image name from Docker Registry, pl. publisher/image-name:tag.
...
A tároló beállításjegyzékének felhasználóneve és jelszava.
A privát registries esetén kötelező megadni.
...
A paraméter elfogadható értékei: - SystemAssigned - UserAssigned
...
### -OSType
A függvényalkalmazást szolgáltató operációs rendszer.
