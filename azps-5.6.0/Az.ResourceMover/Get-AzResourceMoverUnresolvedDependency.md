...
### 1. példa: Egy áthelyezési gyűjtemény azonosítatlan függő erőforrásainak listájának lekérte.
```powershell
...
Az Áthelyezési gyűjtemény azonosítatlan függő erőforrásainak listája.
...
### -DependencyLevel
A függőségi szintet határozza meg.
...
Például: $apply=filter(count eq 2).
...
### -Orderby
OData-sorrend lekérdezések szerint beállítás.
Használhatja például a $orderby=Count desc használhatja.
