...
## LEÍRÁS
Létrehoz egy új Sablon specifikációs verziót a megadott ARM sablontartalommal. A tartalom egy nyers JSON-karakterláncból **(FromJsonStringParameterSet** paraméterkészlet használatával) vagy egy megadott JSON-fájlból **(FromJsonFileParameterSet** paraméterkészlet használatával) is lehet.  

Ha a fő sablon specifikációja még nem létezik, akkor a Sablon specifikációja verzióval együtt jön létre. Ha a Sablon specifikációja már létezik a megadott névvel, akkor a megadott verzió frissül (a többi meglévő verzió megőrzve marad).
...
Létrehoz egy új "v1.0" sablonverziót a "myTemplateSpec" nevű sablon specifikációban. A megadott verzió a $templateJson sablon tartalmának ARM lesz.

 **Megjegyzés:** A ARM sablon nem művelet, mivel nem tartalmaz tényleges erőforrásokat.
...
Létrehoz egy új "v2.0" sablonverziót a "myTemplateSpec" nevű sablon specifikációban. A megadott verzióban a helyi fájl tartalma "be van myTemplateContent.js" lesz, mint a ARM sablon tartalma.
...
### -Force
Ne kérjen megerősítést egy meglévő verzió felülírásakor.
...
### -Tag
Címkék hashtable for the new template spec resource(s).
