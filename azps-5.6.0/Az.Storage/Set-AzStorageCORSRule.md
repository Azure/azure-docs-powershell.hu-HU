...
## LEÍRÁS
A **Set-AzStorageCORSRule** parancsmag beállítja a több forrásból való erőforrás-megosztási (CORS) szabályokat egy adott Típusú Azure Storage-szolgáltatáshoz.
A parancsmag társzolgáltatásának típusai a blob, a tábla, a várólista és a fájl.
Ez a parancsmag felülírja a meglévő szabályokat.
Az aktuális szabályokért használja a Get-AzStorageCORSRule parancsmagot.
...
Az első parancs egy szabálytömböt rendel a $CorsRules változóhoz.
Ez a parancs a kódblokk több során túlnyúló normál kiterjesztést használ.
A második parancs hozzárendeli a $CorsRules Blob szolgáltatástípushoz.

### 2. példa: CORS-szabály tulajdonságainak módosítása blobszolgáltatásban
```
...
Az első parancs a Blob típus aktuális CORS-szabályait kapja meg a **Get-AzStorageCORSRule** parancsmag használatával.
A parancs a szabályokat a tömbváltozóban $CorsRules tárolja.
A második és a harmadik parancs módosítja az első szabályt a $CorsRules.
Az utolsó parancs a blob$CorsRules szolgáltatástípushoz rendeli a szabályokat.
A módosított szabályok felülírják az aktuális KORI-szabályokat.
...
### -CorsRules
A CORS-szabályok tömbje.
A meglévő szabályokat beolvashatja a Get-AzStorageCORSRule parancsmag használatával.
...
### -ServiceType
Azt az Azure Storage szolgáltatástípust adja meg, amelyhez ez a parancsmag szabályokat rendel.
