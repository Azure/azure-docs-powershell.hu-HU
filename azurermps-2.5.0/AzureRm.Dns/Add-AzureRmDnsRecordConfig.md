...
## Leírás
A **Add-AzureRmDnsRecordConfig** PARANCSMAG egy DNS-rekordot ad hozzá egy **Recordset** objektumhoz.
...
A SOA rekordok akkor jönnek létre, amikor létrejön egy DNS-zóna, és törlődik a DNS-zóna törlésekor.
Nem adhat hozzá vagy távolíthat el SOA rekordokat, de szerkesztheti őket.

A **rekordhalmaz** -objektumot paraméterként vagy a pipeline operátor segítségével átadhatja erre a parancsmagra.

## Példák

### 1. példa: rekord felvétele a rekordba
```
...
Ez a példa egy rekordot ad hozzá egy meglévő rekordhoz.

### 2. példa: AAAA rekord felvétele a Record set-ba
```
...
Ebben a példában az AAAA-rekordot egy meglévő rekorddal egészíti ki.

### 3. példa: CNAME rekord felvétele a Record set-ba
```
...
Ez a példa CNAME rekordot ad hozzá egy meglévő rekordhoz.
Mivel a CNAME rekord legfeljebb egy rekordban szerepelhet, kezdetben üres rekordnak kell lennie, vagy a meglévő rekordokat el kell távolítani a Remove-AzureRmDnsRecordConfig.

### 4. példa: MX rekord hozzáadása a Record set-hoz
```
...
Ez a példa MX rekordot ad hozzá egy meglévő rekordhoz.
A "@" rekord a zóna APEX nevű rekordjait jelzi.

### Példa 5: NS rekord felvétele a Record set-ba
```
...
Ebben a példában egy NS rekordot szúr be egy meglévő rekordba.

### Példa 6: PTR rekord hozzáadása a rekordhoz
```
...
Ebben a példában egy PTR rekordot szúr be egy meglévő rekordba.

### 7. példa: SRV rekord hozzáadása a rekordhoz
```
...
Ebben a példában egy SRV rekordot ad hozzá egy meglévő rekordhoz.

### 8. példa: TXT rekord hozzáadása a Record set-hoz
```
...
Ez a példa egy TXT rekordot szúr be egy meglévő rekordba.
...
### -RecordSet
A szerkesztendő **Recordset** objektumot adja meg.
...
Ez a **rekordhalmaz** kapcsolat nélküli megjelenítése, és a változtatások nem változtatják meg a DNS-válaszokat addig, amíg az Set-AzureRmDnsRecordSet parancsmagot nem futtatja.

## KIMENETEK

### Microsoft. Azure. Command. DNS. DnsRecordSet
Ez a parancsmag a módosított **Recordset** objektumot számítja ki.
Az átadott objektumot a program közvetlenül módosítja.
