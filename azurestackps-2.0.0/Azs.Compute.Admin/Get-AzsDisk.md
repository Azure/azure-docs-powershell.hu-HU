...
### Példa 1: az összes lemez beolvasása 
```powershell
PS C:\> Get-AzsDisk
```

Paraméterek nélkül a `Get-AzsDisk` program felsorolja az összes lemezt.

### 2. példa: lemez beszerzése név alapján
```powershell
...
Adjon meg egy lemezt a `Name` lekéréséhez.

### 3. példa: megadott számú lemez beszerzése
```powershell
...
A `Count` megadott számú lemezek beolvasásához használja a paramétert.

### Példa 4: az összes lemez beolvasása adott helyen
```powershell
...
Az `ScaleUnit` vagy `VolumeLabel` paraméter használata adott helyen lévő összes lemez listázásához
...
### -ScaleUnit
Az a méretarány, amelyhez az erőforrás tartozik.
...
Az a megosztás, amelyhez az erőforrás tartozik.
...
### -UserSubscriptionId
Az a felhasználói előfizetés azonosítója, amelyhez az erőforrás tartozik.
...
### -VolumeLabel
Annak a kötetnek a Kötetcímke, amelyhez az erőforrás tartozik.
...
### Microsoft. Azure. PowerShell. parancsmagok. ComputeAdmin. modellek. Api20180730Preview. IDisk
