...
1. [Telepítse az Azure PowerShellt](install-az-ps.md).

2. A telepítés sikerességének ellenőrzéséhez futtassa a `Get-InstalledModule Az -AllVersions` parancsot a parancssorról.
...
1. Gépelje be: `Connect-AzAccount`. Az `-Environment` argumentum lehetővé teszi egy másik régióban vagy felhőben való hitelesítést. Például az Azure Chinához való csatlakozáshoz:

    ```powershell-interactive
    Connect-AzAccount -Environment AzureChinaCloud
    ```

2. Kap egy jogkivonatot, amelyet a https://microsoft.com/devicelogin címen használhat. Nyissa meg ezt az oldalt a böngészőjében, és írja be a jogkivonatot az Azure-beli hitelesítő adatokkal való bejelentkezéshez és az Azure PowerShell engedélyezéséhez. 

Miután bejelentkezett egy Azure-fiókba, az Azure PowerShell parancsmagjaival elérheti és kezelheti az előfizetésben lévő erőforrásokat. A bejelentkezési folyamattal és az elérhető hitelesítési módszerekkel kapcsolatos további információért tekintse meg az [Azure PowerShell-lel való bejelentkezést](authenticate-azureps.md) bemutató cikket.
...
A `New-AzVM` parancsmag egy leegyszerűsített szintaxist biztosít, amely megkönnyíti az új virtuális gépek létrehozását. Mindössze két paraméterértéket kell megadnia: a virtuális gép nevét, valamint a virtuális gép helyi rendszergazdai fiókjához tartozó hitelesítő adatok egy készletét.
...
Felmerülhet a kérdés, hogy milyen más elemek jönnek még létre, illetve hogy milyen lesz a virtuális gép konfigurációja. Először is lássuk az erőforráscsoportokat.
...
A **cloud-shell-storage-westus** erőforráscsoport a Cloud Shell első használatakor jön létre. A **SampleVM** erőforráscsoportot a `New-AzVM` parancsmag hozta létre.
...
Hozzunk létre egy MyResourceGroup nevű erőforráscsoportot az Azure uswest2 régiójában. Ehhez írja be a következő parancsot:
...
A `Get-AzResource` parancsmag használatával listázhatja az Azure-ban futó erőforrásokat. Az alábbi példa az új erőforráscsoportban létrehozott erőforrásokat mutatja be.
...
Az Azure-fiók tisztítása érdekében érdemes lehet törölnie a példában létrehozott erőforrásokat. A `Remove-Az*` parancsmagokkal törölheti a már nem szükséges erőforrásokat. A létrehozott Windows rendszerű virtuális gép eltávolításához futtassa az alábbi parancsot:
...
Egyszerre több erőforrást is törölhet. A következő parancs például törli a teljes „MyResourceGroup” erőforráscsoportot, amelyet eddig az összes mintához használtunk.
Ezzel a csoportban található összes erőforrást is törli.
...
A feladat végrehajtása több percet is igénybe vehet az erőforrások számától és típusától függően.
