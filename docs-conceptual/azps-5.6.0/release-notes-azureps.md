...
## <a name="560---march-2021"></a>5.6.0 – március 2021
#### <a name="azaccounts"></a>Az.Accounts
* Az Azure. Identity frissítésével javítsa ki azt a problémát, amelyet a Connect-AzAccount nem sikerül az ADFS hitelesítő adatainak használatakor [#13560]

#### <a name="azautomation"></a>Az.Automation
* Kijavítva azt a problémát, amelyet a karakterlánc nem szerializálhat megfelelően. [#14215]
* A Python3 Runbook-típus támogatása hozzáadva

#### <a name="azcompute"></a>Az.Compute
* A (z) "-EnableHotpatching" paraméter hozzáadva a Windows rendszerű gépekhez készült "set-AzVMOperatingSystem" parancsmaghoz. 
* A "-PatchMode" paraméter hozzáadva a Linux-paraméterekhez a "set-AzVMOperatingSystem" parancsmagban. 
* [A változás megszakítása] A nyilvános előzetes verzióban lévő felhasználók a virtuális gép vendég javítási funkciójának módosítása.
    - A "RebootStatus" tulajdonság el lett távolítva a "Microsoft. Azure. Management. számítás. models. LastPatchInstallationSummary" objektumból. 
    - A "StartedBy" tulajdonság el lett távolítva a "Microsoft. Azure. Management. számítás. models. LastPatchInstallationSummary" objektumból.
    - Átnevezte a "kbid" tulajdonságot "KbId" értékre a "Microsoft. Azure. Management. számítás. models. VirtualMachineSoftwarePatchProperties" objektumban. 
    - A "javítások" tulajdonság átnevezve a "availablePatches" értékre a "Microsoft. Azure. Management. számítás. models. VirtualMachineAssessPatchesResult" objektumban. 
    - Átnevezte a "Microsoft. Azure. Management. számítás. models. SoftwareUpdateRebootBehavior" objektumot a következőre: "Microsoft. Azure. Management. számítás. models. VMGuestPatchRebootBehavior".
    - Átnevezte a "Microsoft. Azure. Management. számítás. models. InGuestPatchMode" objektumot a következőre: "Microsoft. Azure. Management. számítás. models. WindowsVMGuestPatchMode".
* [A változás megszakítása] Az összes "Tárolószolgáltatás" parancsmag el lett távolítva. A Container Service API-t a 2020 januárjában elavulták. 
...
* Rögzített hitelesítés a következőhöz: `Connect-AzContainerRegistry`

#### <a name="azcosmosdb"></a>Az. CosmosDB
* Bevezette a NetworkAclBypass és a NetworkAclBypassResourceIds adatbázis-fiók parancsmagokhoz.
* A ServerVersion paraméter bevezetve a frissítéshez – AzCosmosDBAccount.
* Mentésben engedélyezni kívánttal és BackupRetention bevezetve az adatbázis-fiók parancsmagokhoz

#### <a name="azdatafactory"></a>Az.DataFactory
* Az ADF .net SDK verziójának frissítése a 4.14.0-be

#### <a name="azmigrate"></a>Az. áttelepítés
* Az. migrálása
* Az az. migráló modulban parancsmagként bejelentkezett Initialize-AzMigrateReplicationInfrastructure a jelenleg futó külső parancsfájlból.
* A New-AzMigrateServerReplication paraméterei New-AzMigrateDiskMapping a kis-és nagybetűk megkülönböztetését.
* Az új v3 kulcsok kezeléséhez hozzáadta a méretezési berendezés módosításának támogatását.

#### <a name="azrecoveryservices"></a>Az.RecoveryServices
* NULL értékű keresés hozzáadva a cél Storage-fiókhoz a fájlmegosztás-visszaállításban.

#### <a name="azresources"></a>Az.Resources
* Azure-erőforrások üzembe helyezésének támogatása a bicep nyelven
* A New-AzTenantDeployment és a New-AzManagementGroupDeployment TemplateSpec üzemelő példányokkal kapcsolatos hibák kijavítva
* Támogatás hozzáadva a "-QueryString" paraméterhez a "test-az * Deployments" parancsmagokban
* Kijavítottuk a problémát dinamikus paraméterekkel, ha az "új-az * üzemelő példányok" a "-QueryString" használatával van használatban
* A "-TemplateParameterObject" paraméter támogatása a "-TemplateSpecId" paraméter használatakor az "új-az * Deployments" parancsmagokban
* A nem létező sablon-specifikáció üzembe helyezésére tett kísérlet során megjelenő pontatlan hibaüzenet kijavítva

#### <a name="azstorage"></a>Az.Storage
* Frissítve a Microsoft. Azure. Management. Storage 19.0.0 az új API 2021-01-01-es verziójának támogatásához.
* Támogatott erőforrás-hozzáférési szabály a NetworkRuleSet-ben
    - Update-AzStorageAccountNetworkRuleSet
    - Add-AzStorageAccountNetworkRule
    - "Remove-AzStorageAccountNetworkRule"
* Támogatott blob-verzió és blob-típus hozzáfűzése a felügyeleti házirendben
    - 'Add-AzStorageAccountManagementPolicyAction'
    - "New-AzStorageAccountManagementPolicyFilter"
    - 'Set-AzStorageAccountManagementPolicy'
* Támogatott létrehozási/frissítési fiók a AllowSharedKeyAccess
    - New-AzStorageAccount
    - Set-AzStorageAccount
* Támogatott titkosítási hatókör létrehozása a RequireInfrastructureEncryption
    - New-AzStorageEncryptionScope
* Támogatott másolási blokk blob szinkronban, titkosítási hatókörrel
    - "Copy-AzStorageBlob"
* Kijavítva a hiba, amely Get-AzStorageBlobContent helytelen címtár-elválasztó karaktert használ a Linux és MacOS rendszereken [#14234]

#### <a name="azwebsites"></a>Az.Websites
* A "publish-AzWebApp" egyéni időtúllépési beállításának bevezetése 
* App Service Environment-támogatás hozzáadva
...
* @alunmj, Kis helyesírás, formázás módosításai (#14155)
* @chakra146, Add-AzLoadBalancerInboundNatPoolConfig.md frissítése (#14231)
* Martin Ehrnst ( @ehrnst ), rögzített egy elírást a parancsmagban (#14112)
* Jan David Narkiewicz ( @jdnark )
  * Példák az örökölt parancsmagot és a New-AzAksCluster aliast használó New-AzAksra. Módosult példák a New-AzAksCluster használatára, amely a jelen dokumentációs oldal által célként megadott parancsmag. (#14088)
  * Írja be a következőt: SshKeyVaule SshKeyValue (#14087)
* Ivan Kulezic ( @kukislav ), SQL mi karbantartási konfigurációs példák hozzáadása (#14216)
* @webguynj, Set-AzNetworkSecurityRuleConfig.md frissítése (#14176)
* A Dils ( @yannickdils ) új cmdline adott hozzá a példához, amely alkalmazza a Load Balancer (#14185) módosításait.

## <a name="550---february-2021"></a>5.5.0 – február 2021
#### <a name="azaccounts"></a>Az.Accounts
* A nyomon követett CloudError-kód kivételben
* "ContextCleared" esemény lett végrehajtva, ha a "Clear-AzContext" művelet végrehajtása megtörtént

#### <a name="azaks"></a>Az.Aks
* A parancsmag hibájának finomítására szolgáló hibaüzenetek.
* Frissített kivételek kezelése Azure PowerShell kapcsolódó kivételek használatához.
* Kijavítva azt a problémát, amellyel a felhasználó nem használhatja a megadott szolgáltatásnevet a Kubernetes-fürt létrehozásához. [#13938]

#### <a name="azautomation"></a>Az.Automation
* Kijavítva a "Pscustomobject formájában kapja" és a "Array" feldolgozásának hibáját.

#### <a name="azcompute"></a>Az.Compute
* A "-EnableAutomaticUpgrade" paraméter hozzáadva a következőhöz: "set-AzVmExtension" és "Add-AzVmssExtension".
* A FilterExpression paraméter el lett távolítva a Get-AzVMImage parancsmag dokumentációból. 
* Elavult üzenet hozzáadva a Tárolószolgáltatás-parancsmagokhoz:
    ...
    - "Remove-AzContainerServiceAgentPoolProfile"
    - "Update-AzContainerService"
* A "-BurstingEnabled" paraméter hozzáadva a következőhöz: "New-AzDiskConfig" és "New-AzDiskUpdateConfig"
* A "-GroupByApplicationId" és a "-GroupByUserAgent" paraméterek hozzá lettek adva az "export-AzLogAnalyticThrottledRequest" és az "export-AzLogAnalyticRequestRateByInterval" parancsmagokhoz.
* A "VMParameterSet" paraméter hozzá lett adva a "Get-AzVMExtension" parancsmaghoz. Új "-VM" paramétert adott meg ehhez a paraméterhez. 

#### <a name="azcontainerregistry"></a>Az.ContainerRegistry
* Parancsmagok hozzáadva a támogatott adattár-, jegyzékfájl-és címkéző műveletekhez:
...
Támogatott – EnableNoPublicIP Databricks-munkaterület létrehozásakor

#### <a name="azfrontdoor"></a>Az.FrontDoor
* FrontDoorId hozzáadva a tulajdonságokhoz
* JSON-Kizárások hozzáadása és a felügyelt szabályok RequestBodyCheck támogatása

#### <a name="azhdinsight"></a>Az.HDInsight
* Új "-EnableComputeIsolation" és "-ComputeIsolationHostSku" paraméter hozzáadva a (z) "New-AzHDInsightCluster" parancsmaghoz a számítási elkülönítési funkció támogatásához
* A "ComputeIsolationProperties" és a "ConnectivityEndpoints" tulajdonság hozzáadva a (z) AzureHDInsightCluster osztályban.

#### <a name="azkeyvault"></a>Az.KeyVault
* A kulcs típusának és a görbe nevének megadása támogatott a kulcsok BYOK-fájlon keresztüli importálásakor

#### <a name="aznetwork"></a>Az.Network
* Új parancsmagok lettek hozzáadva a "virtuális útválasztó" nevű régi terméknév helyett a "Route Server" nevű új névvel.
    - "New-AzRouteServer"
    - "Get-AzRouteServer"
    ...
    - Az elavult attribútum figyelmeztetést adott a régi parancsmagokhoz.
* Hibajavítás a ExpressRouteLink MacSecConfig. Új "SciState" tulajdonság hozzáadva a következőhöz: "PSExpressRouteLinkMacSecConfig"
* A Get-AzVirtualNetworkGatewayConnectionIkeSa formátumának frissített formázása és a táblázatos nézetek formázása

#### <a name="azpolicyinsights"></a>Az.PolicyInsights
* A PowerShellben végrehajtott, a kérelmek számát megnövelő módosítások visszavonása. Nem sikerült eltávolítani a támogatási lapozás helytelen utasítását

#### <a name="azrecoveryservices"></a>Az.RecoveryServices
* módosított szabályzat-ellenőrzési korlátok a Backup szolgáltatás esetében.
* Zóna-redundancia hozzáadva a Recovery Service-tárolók számára. 
* Azure Site Recovery a VMware-ből az Azure-ba és az Azure-szolgáltatók felé irányuló HyperV-elhelyezési csoportok támogatása.
* Azure Site Recovery támogatja a VMware-ből az Azure-ba való rendelkezésre állási zónát és az Azure-szolgáltatók HyperV.
* Azure Site Recovery UseManagedDisk támogatása az Azure-szolgáltatóhoz való HyperV

#### <a name="azresources"></a>Az.Resources
* A rendszerbiztonsági tag el lett távolítva New-AzRoleAssignment és Set-AzRoleAssignment, mert az aktuális leképezés bizonyos forgatókönyvek megszegését okozta

#### <a name="azsql"></a>Az.Sql
* MaintenanceConfigurationId hozzáadva a következőhöz: "New-AzSqlDatabase", "set-AzSqlDatabase", "New-AzSqlElasticPool" és "set-AzSqlElasticPool"
* Rögzített regresszió a "set-AzSqlServerAudit" paraméterben, ha a PredicateExpression argumentum meg van megadva

#### <a name="azstorage"></a>Az.Storage
* Támogatott RoutingPreference-beállítások a Storage-fiók létrehozása/frissítése során
    - New-AzStorageAccount
    - Set-AzStorageAccount
* Az Azure. Storage. Blobok frissítése a 12.8.0-be
* Frissített Azure. Storage. files. shares to 12.6.0
* Az Azure. Storage. files. DataLake frissítése a 12.6.0-be

#### <a name="azwebsites"></a>Az.Websites
* Key Vault-tanúsítvány WebAppba való importálásának támogatása.

### <a name="thanks-to-our-community-contributors"></a>Köszönjük a közösség alábbi tagjainak
* @atul-ram, Set-AzEventHub.md frissítése (#13921)
* Christoph Bergmeister [MVP] ( @bergmeister ), set-AzDataLakeGen2AclRecursive.MD-hibajavítási hibák elhárítása (Directiry-> könyvtár) (#14082)
* Alexander Schmidt ( @devdeer-alex ), rögzített hivatkozás a hozzájárulási irányelvekhez (#14009)
* @JiangYuchun, Get-AzApplicationGatewayAuthenticationCertificate.md frissítése (#13972)
* Sebastian Olsen ( @Spacebjorn ), javított példa parancs (#13901)

## <a name="540---january-2021"></a>5.4.0 – január 2021
#### <a name="azaccounts"></a>Az.Accounts
* A (z) [#13745] hibakeresési üzenetben szereplő helyes ügyfél-kérelmek azonosítójának megjelenítése
* Gyakori Azure PowerShell kivétel típusa
* Támogatott Storage API 2019-06-01

#### <a name="azautomation"></a>Az.Automation
* Kijavítva a probléma, hogy a leírás nem lett feltöltve az Update Management-ütemtervek esetében

#### <a name="azcosmosdb"></a>Az. CosmosDB
* Az "az. CosmosDB" modul általános elérhetősége
* New-AzCosmosDBAccount parancsmag korlátozása a meglévő adatbázis-fiókok frissítési meghívásához.
* A AnalyticalStorageTTL bemutatása a SqlContainer-ben.

#### <a name="aziothub"></a>Az.IotHub
* Az SAS-jogkivonat létrehozásával kapcsolatos regresszió javítva

#### <a name="azkeyvault"></a>Az.KeyVault
* Kijavítva egy probléma a Secret Management modulban

#### <a name="azlogicapp"></a>Az.LogicApp
* Kijavítottuk a "Get-AzLogicAppTriggerHistory" és a "Get-AzLogicAppRunAction" nevű problémát, amely csak az eredmények első oldalának beolvasását kéri [#9141]

#### <a name="azmonitor"></a>Az.Monitor
* Parancsmagok hozzáadva az adatgyűjtés szabályaihoz: 
    - "Get-AzDataCollectionRule"
    - "New-AzDataCollectionRule"
    - "Set-AzDataCollectionRule"
    - "Update-AzDataCollectionRule"
    - "Remove-AzDataCollectionRule"
* Az adatgyűjtési szabályok társításához hozzáadott parancsmagok
...
* Frissített parancsmagok a ConnectionMode beállításának engedélyezéséhez Virtual Network átjáró kapcsolatain.
    - New-AzVirtualNetworkGatewayConnection
    - Set-AzVirtualNetworkGatewayConnection
* Frissített "New-AzFirewallPolicyApplicationRule" parancsmag:
    - Hozzáadott paraméter célcímet
    - Hozzáadott paraméter TerminateTLS
* Új parancsmagok lettek hozzáadva Azure Firewall Premium-funkciókhoz
    - "New-AzFirewallPolicyIntrusionDetection"
    - "New-AzFirewallPolicyIntrusionDetectionBypassTraffic"
    - "New-AzFirewallPolicyIntrusionDetectionSignatureOverride"
* Frissítve New-AzFirewallPolicy parancsmag:
    - Paraméter hozzáadva – SkuTier
    - Paraméter hozzáadva – identitás
    - Paraméter hozzáadva – UserAssignedIdentityId
    - Paraméter hozzáadva – IntrusionDetection
    - Paraméter hozzáadva – TransportSecurityName
    - Paraméter hozzáadva – TransportSecurityKeyVaultSecretId
* Új parancsmag hozzáadva az IKE biztonsági társítások beolvasásához Virtual Network átjáró kapcsolataihoz.
    - "Get-AzVirtualNetworkGatewayConnectionIkeSa"
* Több hitelesítési támogatás hozzáadva a p2sVpnGateway-hez
    - Frissített New-AzVpnServerConfiguration és Update-AzVpnServerConfiguration, hogy több hitelesítési paramétert lehessen beállítani.
* Frissítve a New-AzVpnGateway és a New-AzP2sVpnGateway parancsmaggal:
    - Hozzáadott paraméter EnableRoutingPreferenceInternetFlag

#### <a name="azrecoveryservices"></a>Az.RecoveryServices
* A régiók közötti visszaállítási szolgáltatás hozzáadva.  
* Letiltotta a munkaterhelés konfigurálását, ha a célként megadott tétel egy rendelkezésre állási csoport.

#### <a name="azresources"></a>Az.Resources
* A-QueryString paraméter támogatása az új-az * Deployments parancsmagok esetében

#### <a name="azsql"></a>Az.Sql
* A "Start-AzSqlInstanceDatabaseLogReplay" parancsmag szinkron, hozzáadva – AsJob jelző

#### <a name="azstorage"></a>Az.Storage
* Javítsa a Continuationtoken argumentumot használja soha nem null értéket, ha a blob listázása a-IncludeVersion
    - Get-AzStorageBlob

#### <a name="azwebsites"></a>Az.Websites
* App Service felügyelt tanúsítványok támogatása
    - "New-AzWebAppCertificate"
    - "Remove-AzWebAppCertificate"
* Kijavítva a hiba, amely miatt a Docker-jelszó el lesz távolítva a appSettings a következőben: "set-AzWebApp" és "set-AzWebAppSlot"

### <a name="thanks-to-our-community-contributors"></a>Köszönjük a közösség alábbi tagjainak
* Ivan Akcheurov ( @ivanakcheurov ), set-AzSecurityWorkspaceSetting.MD frissítése (#13877)
* @javiermarasco, Frissítés példa (#13837)
* @jhaprakash26, Set-AzVirtualNetwork.md frissítése (#13857)
* Michael Holmes ( @MichaelHolmesWP ), Update New-AzStorageTableStoredAccessPolicy.MD (#13871)
* Michael James ( @mikejwhat ), lehetővé teszi, hogy a Get-AzLogicAppTriggerHistory és a Get-AzLogicAppRunAction több mint 30 eredményt hagyjon (#13846)
* @Willem-J-an, Javítsa ki azt a hibát, ami miatt a Docker-jelszó el lesz távolítva a appSettings a set-AzWebApp (slot) alkalmazásban (#13866)

## <a name="530---december-2020"></a>5.3.0 – 2020. december
#### <a name="azaccounts"></a>Az.Accounts
* Javítottuk a problémát, amely miatt a Windows PowerShell figyelmen kívül hagyja a HTTP-proxyt [#13647]
* Továbbfejlesztettük a hosszú ideig futó műveletek hibakeresési naplóját a létrehozott modulokban

#### <a name="azautomation"></a>Az.Automation
* Kijavítottuk azt a hibát, amely miatt a Start-AzAutomationRunbook paraméterei nem tudják JSON-sztringgé konvertálni a PSObject tördelt sztringet [#13240]
* Kijavítottuk a New-AzAutomationUpdateManagementAzureQuery parancsmaghoz tartozó helykiegészítőt

#### <a name="azcompute"></a>Az.Compute
* Az új VMParameterSet paraméterkészlet új VM paraméterét hozzáadtuk a Get-AzVMDscExtensionStatus és Get-AzVMDscExtension parancsmagokhoz. 

#### <a name="azdatabricks"></a>Az.Databricks
* Kijavítottunk egy problémát, amely miatt a New-AzDatabricksVNetPeering parancsmag a teljes üzembe helyezés előtt visszatér (https://github.com/Azure/autorest.powershell/issues/610)

#### <a name="azdatafactory"></a>Az.DataFactory
* Kijavítottuk az Invoke-AzDataFactoryV2Pipeline parancs SupportsShouldProcess paraméterrel kapcsolatos hibáját

#### <a name="azdesktopvirtualization"></a>Az.DesktopVirtualization
* Hozzáadtuk a StartVMOnConnect tulajdonságot a gazdagépkészlethez.

#### <a name="azhdinsight"></a>Az.HDInsight
* Hozzáadott tulajdonságok: Fqdn és EffectiveDiskEncryptionKeyUrl az AzureHDInsightHostInfo osztályban.

#### <a name="azkeyvault"></a>Az.KeyVault
* Hozzáadtuk a Get-AzKeyVaultSecret parancsmaghoz az új -AsPlainText új paramétert, amely egyszerű szövegként közvetlenül visszaadja a titkos kódot [#13630]
* Támogatott szelektív visszaállítás egy felügyelt HSM teljes biztonsági másolatból [#13526]
* Kijavítottunk néhány kisebb hibát [#13583] [#13584]
* A Get-Secret hiányzó visszaadott objektumait hozzáadtuk a SecretManagement modulhoz
* Kijavítottunk egy problémát, amely miatt a tároló az alapértelmezett hozzáférési szabályzat nélkül jöhetett létre [#13687]

#### <a name="azkusto"></a>Az.Kusto
* Az API-verzió a 2020-09-18-as verzióra frissült.

#### <a name="aznetwork"></a>Az.Network
* Kijavítottuk a remove peering és remove connection parancsmagok ExpressRouteCircuit-forgatókönyvben jelentkező hibáját
    - Remove-AzExpressRouteCircuitPeeringConfig és Remove-AzExpressRouteCircuitConnectionConfig

#### <a name="azpolicyinsights"></a>Az.PolicyInsights
* A Get-AzPolicyState tördelt eredményeinek visszaadása mostantól támogatott

#### <a name="azrecoveryservices"></a>Az.RecoveryServices
* Engedélyeztük a SoftDelete funkció használatát az SQL-hez.
* Kijavítottuk az SQL AG-visszaállítást és eltávolítottuk a tárolónév-ellenőrzést.
* Az Azure Files-beli biztonsági mentési elem tárolónév-formátuma megváltozott.
* Hozzáadtuk a CMK funkció támogatását a Recovery Services-tárolóhoz.

#### <a name="azresources"></a>Az.Resources
* Kijavítottunk egy NullRef kivétellel kapcsolatos problémát a New-AzureManagedApplication és a Set-AzureManagedApplication parancsmagokban.
* Frissítettük az Azure Resource Manager SDK-t, hogy mostantól a legfrissebb DeploymentScripts GA API-verziót használja: 2020-10-01.

#### <a name="azservicefabric"></a>Az.ServiceFabric
* Add-AzServiceFabricNodeType kijavítva. Csomóponttípus hozzáadva a Service Fabric-fürthöz a virtuálisgép-méretezési csoport létrehozása előtt.

#### <a name="azsql"></a>Az.Sql
* Az InstanceFailoverGroup parancs paraméterleírása javítva.
* Frissítettük a logikát, amelyben a rendszer a schemaName, a tableName és a columnName tulajdonságokat az SQL-adatbesorolási parancsok azonosítójából bontja ki.
* Javítottuk a Get-AzSqlDatabaseImportExportStatus Status és StatusMessage mezőit a dokumentációnak való megfelelés érdekében
* Hozzáadtuk a Microsoft támogatási műveletek (DevOps) következő naplózási parancsmagjait: Get-AzSqlServerMSSupportAudit, Set-AzSqlServerMSSupportAudit, Remove-AzSqlServerMSSupportAudit

#### <a name="azstorage"></a>Az.Storage
* A tárfiók create/update/get/list EncryptionScope tulajdonsága mostantól támogatott
    - New-AzStorageEncryptionScope
    - Update-AzStorageEncryptionScope
    - Get-AzStorageEncryptionScope
* Tárolólétrehozás és blobfeltöltés támogatása a titkosítási hatókör beállításával
    - New-AzRmStorageContainer
    - New-AzStorageContainer
    - Set-AzStorageBlobContent

### <a name="thanks-to-our-community-contributors"></a>Köszönjük a közösség alábbi tagjainak
* Andreas Wolter (@AndreasWolter), eltávolított marketingnyelv, jobb példaszűrő (#13671)
* Tidjani Belmansour (@BelRarr), Get-AzBillingInvoice.md frissítése (#13634)
* David Klempfner (@DavidKlempfner)
  * Helyesírási hiba javítva (#13677)
  * PSMetricNoDetails.cs frissítése (#13676)
* @kilobyte97, a remove parancsmag konfigurációtörléssel kapcsolatos hibájának javítása (#13655)
* @kongou-ae, Set-AzFirewall.md frissítése (#13727)
* @MasterKuat, a cím és a kód felcserélésének javítása a dokumentációban (#13666)
* NickT (@nukeulis), Set-AzContext.md frissítése (#13702)
* @PaulHCode, Start-AzJitNetworkAccessPolicy.md frissítése – a példa kijavítása a megfelelő parancsmag megjelenítéséhez (#13713)
* Ryan Borstelmann (@ryanborMSFT), eltávolított előfizetés-azonosító (#13715)
* Shashikant Shakya (@shshakya), Set-AzSqlDabase.md frissítése (#13674)
* Sebastian Olsen (@Spacebjorn), Get-AzRecoveryServicesBackupItem.md frissítése (#13719)

## <a name="520---december-2020"></a>5.2.0 – 2020. december
#### <a name="azaccounts"></a>Az.Accounts
* Megoldott az ExpiresOn időpont elemzése a nyers tokenből, ha nem lehetett lekérni a mögöttes kódtárból
* Továbbfejlesztett figyelmeztető üzenet, ha az interaktív hitelesítés nem érhető el

#### <a name="azapimanagement"></a>Az.ApiManagement
* [Kompatibilitástörő változás] A New-AzApiManagementProduct alapértelmezés szerint nem rendelkezik előfizetési korláttal.

#### <a name="azcompute"></a>Az.Compute
* Szerkesztve lett a Get-AzVm, hogy a túl sok erőforrás miatti szabályozás ellenőrzése előtt el legyen végezve a -Name szerinti szűrés. 
* Új Start-AzVmssRollingExtensionUpgrade parancsmag.

#### <a name="azcontainerregistry"></a>Az.ContainerRegistry
* Támogatott Name paraméter és folyamatbemeneti érték a Get-AzContainerRegistryUsage parancsmaghoz [#13605]
* Továbbfejlesztett kivételek a Connect-AzContainerRegistry parancsmag esetében

#### <a name="azdatafactory"></a>Az.DataFactory
* Az ADF .Net SDK a 4.13.0-s verzióra frissült

#### <a name="azhealthcareapis"></a>Az.HealthcareApis
* Hozzá lett adva a felhasználó által kezelt kulcsok támogatása

#### <a name="aziothub"></a>Az.IotHub
* Ki lett javítva egy SAS-tokennel kapcsolatos hiba.

#### <a name="azkeyvault"></a>Az.KeyVault
* Az „Összes” lehetőség támogatása a Key Vault hozzáférési szabályzatok beállításakor
* A SecretManagement modul új verziója támogatott [#13366]
* Támogatott a ByteArray, String, PSCredential és Hashtable használata a SecretValue esetén a SecretManagement modulban [#12190]
* [Kompatibilitástörő változás] Újra lett tervezve a felügyelt HSM-hez kapcsolódó parancsmagok API-felülete.

#### <a name="azmonitor"></a>Az.Monitor
* Módosítva lett a New-AzAutoscaleProfile Rule paramétere az üres lista elfogadásához. [#12903]
* Új parancsmagok lettek hozzáadva a diagnosztikai beállítások rugalmasabb létrehozásának támogatásához:
    * Get-AzDiagnosticSettingCategory
    * New-AzDiagnosticSetting
    * New-AzDiagnosticDetailSetting

#### <a name="azrecoveryservices"></a>Az.RecoveryServices
* Módosult a súgószöveg és a paraméterkészlet neve a Restore-AzRecoveryServicesBackupItem parancsmag esetében.

#### <a name="azresources"></a>Az.Resources
* A -Tag paraméter mostantól támogatott a Set-AzTemplateSpec és a New-AzTemplateSpec parancsmag esetében
* A Tag paraméter megjelenítése mostantól támogatott a sablonspecifikációk alapértelmezett formázója esetében 

#### <a name="azservicefabric"></a>Az.ServiceFabric
* Hozzá lett adva egy példa a Set-AzServiceFabricSetting parancsmaghoz a SettingsSectionDescription paraméterrel
* Frissültek az alkalmazásokhoz kapcsolódó parancsmagok, amelyek jelzik, hogy a támogatás csak az ARM által üzembe helyezett erőforrásokra vonatkozik
* Elavultként lettek megjelölve az Add-AzureRmServiceFabricClusterCertificate és Remove-AzureRmServiceFabricClusterCertificate fürttanúsítvány-parancsmagok

#### <a name="azsql"></a>Az.Sql
* A SecondaryType paraméter hozzá lett adva a következőkhöz: 
    - New-AzSqlDatabase
    - Set-AzSqlDatabase
    - „New-AzSqlDatabaseSecondary”
* A HighAvailabilityReplicaCount paraméter hozzá lett adva a következőkhöz: 
    - New-AzSqlDatabase
    - Set-AzSqlDatabase
* A ReadReplicaCount mostantól a HighAvailabilityReplicaCount aliasa a következőkben: 
    - New-AzSqlDatabase
    - Set-AzSqlDatabase

#### <a name="azstorage"></a>Az.Storage
* A feltöltési Azure-fájlméret akár 4 TiB-ig támogatott
    - Set-AzStorageFileContent
* Az Azure.Storage.Blobs a 12.7.0-s verzióra frissült
* Az Azure.Storage.Files.Shares a 12.5.0-s verzióra frissült
* Az Azure.Storage.Files.DataLake a 12.5.0-s verzióra frissült

#### <a name="azstoragesync"></a>Az.StorageSync
* Hozzá lett adva a Sync-rétegzési szabályzat funkció letöltési szabályzattal és helyi gyorsítótárazási móddal

#### <a name="azwebsites"></a>Az.Websites
* Ismétlődő hozzáférés-korlátozási szabályok elkerülése

### <a name="thanks-to-our-community-contributors"></a>Köszönjük a közösség alábbi tagjainak
* Andrew Dawson (@dawsonar802), A Get-AzKeyVaultCertificate.md frissítése – Tanúsítvány beszerzése és pfx-szakaszként való mentése a PowerShell Core-ban való használathoz (#13557)
* @iviark, Egészségügyi célú API-khoz kapcsolódó PowerShell BYOK-frissítések (#13518)
* John Duckmanton (@johnduckmanton), A TagPatchOperation helyesírásának javítása (#13508)
* Michael James (@mikejwhat)
  * A Get-AzLogicAppRunHistory súgójának rendszerezése (#13513)
* Richard de Zwart (@mountain65)
  * Az Update-AzAppConfigurationStore.md frissítése (#13485)
  * A New-AzCosmosDBAccount.md frissítése (#13490)
* @SteppingRazor, New-AzApiManagementProduct: A SubscriptionsLimit paraméter alapértelmezett értékének módosítása None (Nincs) értékre (#13457)
* Steve Burkett (@SteveBurkettNZ), WorkspaceResourceId paraméter – elírás javítása egy példában (#13589)

## <a name="510---november-2020"></a>5.1.0 – 2020. november
#### <a name="azaccounts"></a>Az.Accounts
* Kijavítottunk egy hibát, amely miatt a Connect-AzAccount -DeviceCode parancs használata figyelmen kívül hagyja a TenantId azonosítót [#13477]
* Hozzáadtunk egy új, Get-AzAccessToken parancsmagot
* Kijavítottunk egy problémát, amely hibát okozott, ha a felhasználói profil elérési útja elérhetetlen volt
* Kijavítottunk egy problémát, amely írási objektumhibát okozott a Connect-AzAccount parancsmag futtatása közben [#13419]
* A ContainerRegistryEndpointSuffix paramétert adtuk a következőkhöz: Add-AzEnvironment, Set-AzEnvironment 
* A bejelentkezés <kbd>CTRL</kbd>+<kbd>C</kbd> billentyűkombinációval való megszakításának támogatása
* Kijavítottunk egy hibát, amely miatt a Connect-AzAccount -KeyVaultAccessToken parancsmag nem működött [#13127]
* Kijavítottuk a null értékű hivatkozást és a kis- és nagybetűk megkülönböztetését mellőző metódust az Invoke-AzRestMethod parancsmagban

#### <a name="azaks"></a>Az.Aks
* Kijavítottunk egy hibát, amely miatt a felhasználó nem használhatott szolgáltatásnevet új Kubernetes-fürt létrehozásához. [#13012]

#### <a name="azappconfiguration"></a>Az.AppConfiguration
* Az Az.AppConfiguration modul általános elérhetősége

#### <a name="azdatafactory"></a>Az.DataFactory
* Továbbfejlesztettük a New-AzDataFactoryV2LinkedServiceEncryptedCredential parancs hibaüzenetét

#### <a name="azdatalakestore"></a>Az.DataLakeStore
* Az ADLS-adatsík SDK frissítése az 1.2.4-alpha verzióra. Módosítások: https://github.com/Azure/azure-data-lake-store-net/blob/preview-alpha/CHANGELOG.md#version-124-alpha

#### <a name="azdesktopvirtualization"></a>Az.DesktopVirtualization
* Új MSIX csomag-parancsmagokat és frissített alkalmazás-parancsmagokat adtunk hozzá.

#### <a name="azeventhub"></a>Az.EventHub
* Javítottuk a fürtparancsokat a címke nélküli EventHub-fürtön
* Frissítettük a PartnerNamespace ás az AzEventHubGeoDRConfiguration parancsok súgószövegét 

#### <a name="azhdinsight"></a>Az.HDInsight
* ResourceProviderConnection és PrivateLink paramétereket adtunk hozzá a New-AzHDInsightCluster parancsmaghoz a továbbítók és a Private Link funkció támogatása érdekében
* AmbariDatabase paramétert adtunk hozzá a New-AzHDInsightCluster parancsmaghoz az egyedi Ambari-adatbázis funkció támogatása érdekében
* Az AmbariDatabase elfogadott értéket hozzáadtuk az Add-AzHDInsightMetastore parancsmag MetastoreType paraméteréhez

#### <a name="aziothub"></a>Az.IotHub
* A címkék engedélyezve vannak az IoT Hub létrehozási parancsmagjában.

#### <a name="azkeyvault"></a>Az.KeyVault
* Kulcstartócímke frissítésének támogatása

#### <a name="azlogicapp"></a>Az.LogicApp
* Ki lett javítva a hiba, amely miatt a Get-AzLogicAppRunHistory csak az eredmények első oldalát kérte le

#### <a name="aznetwork"></a>Az.Network
* Frissítettük az alábbi parancsmagokat: 
    - New-AzLoadBalancerFrontendIpConfigCommand, Set-AzLoadBalancerFrontendIpConfigCommand, Add-AzLoadBalancerFrontendIpConfigCommand:
        - PublicIpAddressPrefix tulajdonság hozzáadva
        - PublicIpAddressPrefixId tulajdonság hozzáadva
* Új tulajdonságokat adtunk hozzá a következő parancsmagokhoz a globális terheléselosztás engedélyezése érdekében
    - New-AzLoadBalancer:
        - Sku Tier tulajdonság hozzáadva
    - New-AzPublicIpAddress:
        - Sku Tier tulajdonság hozzáadva
    - New-AzPublicIpPrefix:
        - Sku Tier tulajdonság hozzáadva
    - New-AzLoadBalancerBackendAddressConfig:
        - LoadBalancerFrontendIPConfigurationId tulajdonság hozzáadva
* A következő parancsmagokra vonatkozó figyelmeztetések érvénytelenítésének tervezése frissült  -New-AzVirtualHubRoute   -New-AzVirtualHubRouteTable   -Add-AzVirtualHubRoute   -Add-AzVirtualHubRouteTable   -Get-AzVirtualHubRouteTable   -Remove-AzVirtualHubRouteTable
* Hozzáadtuk a következő parancsmagokra vonatkozó figyelmeztetések érvénytelenítésének megtervezését a RouteTable argumentumon:   New-AzVirtualHub   -set-AzVirtualHub   -Update-AzVirtualHub
* A -MinScaleUnits és a -MaxScaleUnits argumentumok mostantól választhatók a set-AzExpressRouteGateway parancsmagban
* Új parancsmagok lettek hozzáadva a kölcsönös hitelesítés és az SSL-profilok támogatásához az Application Gatewayen
...
#### <a name="azrecoveryservices"></a>Az.RecoveryServices
* A szabályzat BackupTime tulajdonságának meghatározása UTC szerint értendő.
* A kompatibilitástörő változás figyelmeztetésének módosítása a Get-AzRecoveryServicesBackupJobDetails parancsmagban.
* A Set-AzRecoveryServicesBackupProtectionPolicy parancsmag mintaszkript-súgószövegének frissítése.

#### <a name="azresources"></a>Az.Resources
* Kijavítottunk egy hibát, amely miatt a What-If két, kis- és nagybetűs erőforráscsoport-hatókört jelenített meg
* Frissítettük az Export-AzResourceGroup parancsmagot az SDK használatára.
* Kulturális környezeti információkat adtunk hozzá az elemzési módszerekhez

#### <a name="azsql"></a>Az.Sql
* Kijavítottuk azokat a hibákat, amelyek miatt a Set-AzSqlDatabaseAudit parancsmag nem támogatta a rugalmas skálázású adatbázisokat, valamint az adatbázis-kiadást nem lehetett meghatározni
* MaintenanceConfigurationId azonosítót adtunk hozzá a New-AzSqlInstance és Set-AzSqlInstance parancsmagokhoz
* Kijavítottuk a GetAzureSqlDatabaseReplicationLink.cs hibáját, amely miatt a PartnerServerName paramétert a kulcs helyett érték alapján ellenőrizte a rendszer

#### <a name="azwebsites"></a>Az.Websites
* Új hozzáférés-korlátozási funkciók kaptak támogatást: ServiceTag, több IP-cím és HTTP-fejlécek

### <a name="thanks-to-our-community-contributors"></a>Köszönjük a közösség alábbi tagjainak
* John Q. Martin (@johnmart82), a tűzfal előfeltétel-információinak hozzáadása (#13385)
* Manikandan Duraisamy (@madurais-msft), a PublicSubnetName argumentum javítva (#13417)
* @mahortas, A -HostNames paraméterértékek frissítve (#13349)
* @MariachiForHire, támogatott TrafficAnalyticsInterval-értékek hozzáadva (#13304)
* Michael James (@mikejwhat), a Get-AzLogicAppRunHistory mostantól több mint 30 bejegyzést adhat vissza (#13393)
* Shashikant Shakya (@shshakya), Restore-AzSqlInstanceDatabase.md frissítése (#13404)
