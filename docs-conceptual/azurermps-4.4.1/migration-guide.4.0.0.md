...
- Az ```SubscriptionName``` tulajdonság a következőre módosult: ```Name```
...
- Az ```TenantId``` tulajdonság a következőre módosult: ```Id```
...
- Az ```Domain``` tulajdonság a következőre módosult: ```Directory```
